    ...
    def on_delete(req, resp, id_, mid):
        ...
        try:
            ...
            try:
                ...
                cursor.execute(" DELETE FROM tbl_shopfloors_meters WHERE shopfloor_id = %s AND meter_id = %s ",
                               (id_, mid))
                cnx.commit()
            finally:
                if cursor:
    ...
    def on_delete(req, resp, id_, sid):
        ...
        try:
            ...
            try:
                ...
                cursor.execute(" DELETE FROM tbl_shopfloors_sensors WHERE shopfloor_id = %s AND sensor_id = %s ",
                               (id_, sid))
                cnx.commit()
            finally:
                if cursor:
    ...
    def on_delete(req, resp, id_, cid):
        ...
        try:
            ...
            try:
                ...
                cursor.execute(" DELETE FROM tbl_shopfloors_commands WHERE shopfloor_id = %s AND command_id = %s ",
                               (id_, cid))
                cnx.commit()
            finally:
                if cursor:
        ...
        try:
            ...
            try:
                ...
                if row is None:
                    ...
                    # Note: The original code had a redundant check here, kept as is per instructions
                    cursor.execute(" SELECT name "
                                   " FROM tbl_shopfloors "
                                   " WHERE id = %s ", (id_,))
                    if cursor.fetchone() is None:
                ...
                if row is None:
                    ...
                    if meta_result['working_calendars'] is not None and len(meta_result['working_calendars']) > 0:
                        for working_calendar in meta_result['working_calendars']:
                            ...
                            add_row = (" INSERT INTO tbl_shopfloors_working_calendars "
                                       " (shopfloor_id, working_calendar_id) "
                                       " VALUES (%s, %s) ")
                            cursor.execute(add_row, (new_id, working_calendar['id'],))
                    cnx.commit()
            finally:
