...
class MicrogridEVChargerCollection:
    ...
    def on_get(req, resp):
        ...
        query = (" SELECT id, name, uuid, microgrid_id, power_point_id, meter_id, capacity "
                 ...
                 " ORDER BY id ")
        cursor.execute(query)
        rows_microgrid_evchargers = cursor.fetchall()

        result = list()
        if rows_microgrid_evchargers is not None and len(rows_microgrid_evchargers) > 0:
            for row in rows_microgrid_evchargers:
