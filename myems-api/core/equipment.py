        ...
        try:
            ...
            try:
                ...
                update_row = (" UPDATE tbl_equipments "
                              " SET name = %s, is_input_counted = %s, is_output_counted = %s, is_enabled = %s, "
...
class MeterEquipmentCollection:
        ...
        try:
            ...
            try:
                ...
                if meter_type == 'meters':
                    relation_table = 'tbl_equipments_meters'
                    meter_col = 'meter_id'
                    meter_tbl = 'tbl_meters'
                elif meter_type == 'virtualmeters':
                    relation_table = 'tbl_equipments_virtual_meters'
                    meter_col = 'virtual_meter_id'
                    meter_tbl = 'tbl_virtual_meters'
                else:  # offlinemeters
                    relation_table = 'tbl_equipments_offline_meters'
                ...
                # Query all equipments that this meter is bound to
                query = f""" SELECT e.id, e.name 
                             FROM {relation_table} em, tbl_equipments e 
                             WHERE em.equipment_id = e.id AND em.{meter_col} = %s 
                             ORDER BY e.id """
                cursor.execute(query, (id_,))
                rows = cursor.fetchall()
            ...
            try:
                ...
                if row is None:
                    ...
                    meta_result = {
                                   ...
                                   "is_enabled": bool(row[5]),
                                   "cost_center": cost_center_dict.get(row[6], None),
                                   "efficiency_indicator": Decimal(row[7]) if row[7] is not None else None,
                                   "svg": svg_dict.get(row[8], None),
                                   "camera_url": row[9],
                                   "description": row[10],
                                   "commands": None,
