...
class EnergyStorageContainerUserCollection:
    @staticmethod
    def __init__():
        """Initializes Class"""
        pass
    ...
    def on_get(req, resp, id_):
        ...
        query = (" SELECT u.id, u.name, u.uuid "
                 " FROM tbl_energy_storage_containers m, tbl_energy_storage_containers_users mu, "
                 + config.myems_user_db['database'] + ".tbl_users u "
                 " WHERE mu.energy_storage_container_id = m.id AND u.id = mu.user_id AND m.id = %s "
                 " ORDER BY u.id ")
        cursor.execute(query, (id_,))
        rows = cursor.fetchall()
        result = list()
    ...
    def on_post(req, resp, id_):
        ...
        query = (" SELECT id "
                 " FROM tbl_energy_storage_containers_users "
                 " WHERE energy_storage_container_id = %s AND user_id = %s")
        cursor.execute(query, (id_, user_id,))
        if cursor.fetchone() is not None:
            cursor.close()
            cnx.close()
            raise falcon.HTTPError(status=falcon.HTTP_400, title='API.ERROR',
                                   description='API.ENERGY_STORAGE_CONTAINER_USER_RELATION_EXISTS')
        add_row = (" INSERT INTO tbl_energy_storage_containers_users (energy_storage_container_id, user_id) "
                   " VALUES (%s, %s) ")
        ...
        resp.status = falcon.HTTP_201
        resp.location = '/energystoragecontainers/' + str(id_) + '/users/' + str(user_id)


class EnergyStorageContainerUserItem:
    @staticmethod
    def __init__():
        """Initializes Class"""
        pass
    ...
    def on_delete(req, resp, id_, uid):
        ...
        cursor.execute(" SELECT id "
                       " FROM tbl_energy_storage_containers_users "
                       " WHERE energy_storage_container_id = %s AND user_id = %s ", (id_, uid))
        if cursor.fetchone() is None:
            cursor.close()
            cnx.close()
            raise falcon.HTTPError(status=falcon.HTTP_404, title='API.NOT_FOUND',
                                   description='API.ENERGY_STORAGE_CONTAINER_USER_RELATION_NOT_FOUND')
