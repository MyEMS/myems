        ...
        try:
            ...
            try:
                ...
                if meter_type == 'meters':
                    relation_table = 'tbl_stores_meters'
                    meter_col = 'meter_id'
                    meter_tbl = 'tbl_meters'
                elif meter_type == 'virtualmeters':
                    relation_table = 'tbl_stores_virtual_meters'
                    meter_col = 'virtual_meter_id'
                    meter_tbl = 'tbl_virtual_meters'
                else:  # offlinemeters
                    relation_table = 'tbl_stores_offline_meters'
                ...
                # Query all stores that this meter is bound to
                query = f""" SELECT s.id, s.name 
                             FROM {relation_table} sm, tbl_stores s 
                             WHERE sm.store_id = s.id AND sm.{meter_col} = %s 
                             ORDER BY s.id """
                cursor.execute(query, (id_,))
                rows = cursor.fetchall()
