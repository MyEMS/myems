...
class MicrogridWindturbineCollection:
    ...
    def on_get(req, resp):
        ...
        query = (" SELECT id, name, uuid, microgrid_id, capacity "
                 ...
                 " ORDER BY id ")
        cursor.execute(query)
        rows_microgrid_windturbines = cursor.fetchall()

        result = list()
        if rows_microgrid_windturbines is not None and len(rows_microgrid_windturbines) > 0:
            for row in rows_microgrid_windturbines:
    ...
    def on_post(req, resp):
        ...
        if 'name' not in new_values['data'].keys() or \
            ...
            raise falcon.HTTPError(status=falcon.HTTP_400, title='API.BAD_REQUEST',
                                   description='API.INVALID_MICROGRID_WINDTURBINE_NAME')
        ...
        if cursor.fetchone() is not None:
            ...
            cnx.close()
            raise falcon.HTTPError(status=falcon.HTTP_400, title='API.BAD_REQUEST',
                                   description='API.MICROGRID_WINDTURBINE_NAME_IS_ALREADY_IN_USE')

        add_values = (" INSERT INTO tbl_microgrids_windturbines "
        ...
        resp.location = '/microgridwindturbines/' + str(new_id)


class MicrogridWindturbineItem:
    @staticmethod
    def __init__():
        """Initializes MicrogridWindturbineItem"""
        pass
    ...
    def on_get(req, resp, id_):
        if not id_.isdigit() or int(id_) <= 0:
            raise falcon.HTTPError(status=falcon.HTTP_400, title='API.BAD_REQUEST',
                                   description='API.INVALID_MICROGRID_WINDTURBINE_ID')
        ...
        if row is None:
            raise falcon.HTTPError(status=falcon.HTTP_404, title='API.NOT_FOUND',
                                   description='API.MICROGRID_WINDTURBINE_NOT_FOUND')
        else:
            microgrid = microgrid_dict.get(row[3])
            meta_result = {"id": row[0],
                           "name": row[1],
    ...
    def on_delete(req, resp, id_):
        ...
        if cursor.fetchone() is None:
            ...
            raise falcon.HTTPError(status=falcon.HTTP_404, title='API.NOT_FOUND',
                                   description='API.MICROGRID_WINDTURBINE_NOT_FOUND')
        # todo: delete objects associated with this microgrid windturbine
        # delete microgrid windturbine itself
        cursor.execute(" DELETE FROM tbl_microgrids_windturbines "
                       " WHERE id = %s ", (id_,))
        cnx.commit()
    ...
    def on_put(req, resp, id_):
        ...
        update_row = (" UPDATE tbl_microgrids_windturbines "
