    ...
    def on_get(req, resp, id_):
        ...
        try:
            ...
            try:
                ...
                rows_contacts = cursor.fetchall()
                if rows_contacts is not None and len(rows_contacts) > 0:
                    for row_contact in rows_contacts:
                        contact_dict[row_contact[0]] = {"id": row_contact[0],
                                                        "name": row_contact[1],
                                                        "uuid": row_contact[2]}
                ...
                rows_cost_centers = cursor.fetchall()
                if rows_cost_centers is not None and len(rows_cost_centers) > 0:
                    for row_cc in rows_cost_centers:
                        cost_center_dict[row_cc[0]] = {"id": row_cc[0],
                                                       "name": row_cc[1],
                                                       "uuid": row_cc[2]}
                ...
                if rows_points is not None and len(rows_points) > 0:
                    for row_point in rows_points:
                        point_dict[row_point[0]] = {"id": row_point[0],
                                                    "name": row_point[1]}
...
class EnergyStoragePowerStationContainerCollection:
    ...
    def on_post(req, resp, id_):
        ...
        resp.status = falcon.HTTP_201
        resp.location = '/energystoragepowerstations/' + str(id_) + '/containers/' + str(energy_storage_container_id)
        ...
        try:
            ...
            try:
                ...
                cursor.execute(" SELECT id "
                               ...
                               " AND energy_storage_container_id = %s ", (id_, sid))
                if cursor.fetchone() is None:
                    raise falcon.HTTPError(status=falcon.HTTP_404, title='API.NOT_FOUND',
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    add_row = (" INSERT INTO tbl_energy_storage_power_stations_users "
                               " (energy_storage_power_station_id, user_id) "
                               " VALUES (%s, %s) ")
