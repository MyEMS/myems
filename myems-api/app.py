...
SERVICE_NAME = os.getenv('MYEMS_SERVICE_NAME') or os.getenv('SERVICE_NAME') or 'myems-api'
MYEMS_HANDLER_ATTR = '_myems_managed_handler'
STARTUP_CONTEXT_ID = str(uuid.uuid4())
REQUEST_ID_CONTEXT = ContextVar('request_id', default='-')
CONTEXT_ID_CONTEXT = ContextVar('context_id', default=STARTUP_CONTEXT_ID)
CONTEXT_TYPE_CONTEXT = ContextVar('context_type', default='startup')


def parse_log_level():
    log_level_name = (os.getenv('MYEMS_LOG_LEVEL') or os.getenv('LOG_LEVEL') or 'INFO').upper()
...
class JsonFormatter(logging.Formatter):
    def format(self, record):
        payload = {
            ...
            'context_id': CONTEXT_ID_CONTEXT.get(),
            'context_type': CONTEXT_TYPE_CONTEXT.get(),
        }
...
def configure_bootstrap_logging():
    ...
    setattr(stdout_handler, MYEMS_HANDLER_ATTR, True)

    stderr_handler = logging.StreamHandler(sys.stderr)
    stderr_handler.setLevel(logging.WARNING)
    stderr_handler.setFormatter(formatter)
    setattr(stderr_handler, MYEMS_HANDLER_ATTR, True)

    for handler in list(root_logger.handlers):
        if getattr(handler, MYEMS_HANDLER_ATTR, False):
            root_logger.removeHandler(handler)
            handler.close()

    root_logger.setLevel(min(log_level, logging.WARNING))
    root_logger.addHandler(stdout_handler)
    root_logger.addHandler(stderr_handler)


def bootstrap_excepthook(exc_type, exc_value, exc_traceback):
    if issubclass(exc_type, KeyboardInterrupt):
        return sys.__excepthook__(exc_type, exc_value, exc_traceback)

    logging.getLogger('myems.startup').error(
        'startup exception',
...
APP_LOGGER = logging.getLogger('myems.app')
REQUEST_LOGGER = logging.getLogger('myems.request')


class RequestContextFilter(logging.Filter):
    def filter(self, record):
        record.request_id = REQUEST_ID_CONTEXT.get()
        record.context_id = CONTEXT_ID_CONTEXT.get()
        record.context_type = CONTEXT_TYPE_CONTEXT.get()
        return True

def set_log_context(context_type='task', context_id=None):
    context_id = context_id or str(uuid.uuid4())
    context_id_token = CONTEXT_ID_CONTEXT.set(context_id)
    context_type_token = CONTEXT_TYPE_CONTEXT.set(context_type)
    return context_id, context_id_token, context_type_token


def reset_log_context(context_id_token, context_type_token):
    CONTEXT_ID_CONTEXT.reset(context_id_token)
    CONTEXT_TYPE_CONTEXT.reset(context_type_token)
...
class RequestLoggingMiddleware:
    def process_request(self, req, resp):
        ...
        req.context.request_started_at = time.monotonic()
        req.context.request_id_token = REQUEST_ID_CONTEXT.set(request_id)
        _, req.context.context_id_token, req.context.context_type_token = set_log_context(
    ...
    def process_response(self, req, resp, resource, req_succeeded):
        start_time = getattr(req.context, 'request_started_at', None)
        duration_ms = None
        ...
        extra = {
            ...
            'query_keys': sorted(req.params.keys()),
        }

        if not req_succeeded or (status_code is not None and status_code >= 500):
            REQUEST_LOGGER.error('request failed', extra=extra)
        elif status_code is not None and status_code >= 400:
            REQUEST_LOGGER.warning('request completed with client error', extra=extra)
        else:
            REQUEST_LOGGER.info('request completed', extra=extra)

        request_id_token = getattr(req.context, 'request_id_token', None)
        if request_id_token is not None:
            REQUEST_ID_CONTEXT.reset(request_id_token)
        context_id_token = getattr(req.context, 'context_id_token', None)
        context_type_token = getattr(req.context, 'context_type_token', None)
        if context_id_token is not None and context_type_token is not None:
            reset_log_context(context_id_token, context_type_token)
...
api = falcon.App(middleware=[RequestLoggingMiddleware(), cors, MultipartMiddleware()])
