"""
Tenant Load PDF Exporter

This module exports tenant load analysis (average load, maximum load, load factor) data to PDF format.
The content mirrors the Excel exporter (excelexporters/tenantload.py).
...
class TenantLoadPDFExporter:
    """
    Export tenant load data to PDF format.
    ...
    def _create_cover_page(self, pdf, name, period_type,
        ...
        # Title
        fig.text(0.5, 0.50, _('Tenant Data') + ' - ' + _('Load'),
...
def export(report, name, base_period_start_datetime_local,
    ...
    """
    exporter = TenantLoadPDFExporter(language)
