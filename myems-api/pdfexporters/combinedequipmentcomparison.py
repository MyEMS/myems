"""
Combined Equipment Comparison PDF Exporter
...
class CombinedEquipmentComparisonPDFExporter:
    """
    ...
    def _create_cover_page(self, pdf: PdfPages):
        ...
        fig.text(0.5, 0.50,_('Combined Equipment Data') + ' - ' +_('Equipment Comparison'),
                 fontsize=24, weight='bold', ha='center', va='center')

        # Info list
        unit = self.report.get('energy_category', {}).get('unit_of_measure', '')
        info_data = [
            [_('Combined Equipment Data') + '1:', self.equipment1_name],
            [_('Combined Equipment Data') + '2:', self.equipment2_name],
    ...
    def _create_consumption_summary_page(self, pdf: PdfPages):
        ...
        Excel has:
          - CombinedEquipment1 name as title, Consumption row with EC(unit) and total
          - CombinedEquipment2 name as title, Consumption row with EC(unit) and total
        """
        _ = self._
        unit = self.report.get('energy_category', {}).get('unit_of_measure', '')
        ...
        # --- CombinedEquipment1 consumption table ---
        ax1 = fig.add_axes([0.10, 0.55, 0.80, 0.30])
        ax1.axis('off')

        # CombinedEquipment1 name as title row
        eq1_table = ax1.table(
        ...
        # CombinedEquipment1 value table (EC header + total value)
        ...
        # --- CombinedEquipment2 consumption table ---
        ...
        # CombinedEquipment2 value table
...
def export(report, equipment1_name, equipment2_name, energy_category_name,
    ...
    """
    exporter = CombinedEquipmentComparisonPDFExporter(language)
