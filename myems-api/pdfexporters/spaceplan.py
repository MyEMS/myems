"""
Space Plan PDF Exporter

This module provides functionality to export space plan data to PDF format.
...
class SpacePlanPDFExporter:
    """
    Export space plan data to PDF format.
    ...
    def _create_combined_analysis_page(self, pdf: PdfPages):
        ...
        for i in range(ca_len):
            col_headers.append(names[i] + '   (' + _('Plan') + ' - ' + _('Actual') + ')(' + units[i] + ')')
        col_headers.append(_('Ton of Standard Coal') + '  (' + _('Plan') + ' - ' + _('Actual') + ') (TCE)')
        col_headers.append(_('Ton of Carbon Dioxide Emissions') + '  (' + _('Plan') + ' - ' + _('Actual') + ') (TCO2E)')
...
def export(report, name, base_period_start_datetime_local,
    ...
    """
    exporter = SpacePlanPDFExporter(language)
