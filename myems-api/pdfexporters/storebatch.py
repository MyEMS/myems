"""
Store Batch PDF Exporter

This module provides functionality to export store batch data to PDF format.
...
Note: the content mirrors the Excel exporter (excelexporters/storebatch.py)
...
class StoreBatchPDFExporter:
    """
    Export store batch data to PDF format.
    Generates a wide matrix table (rows = stores, columns = energy categories)
    ...
    def export(self,
        ...
        logger.info("Starting PDF generation for store batch")
    ...
    def _create_cover_page(self, pdf: PdfPages):
        ...
        # Title - _('Store Data') + ' - ' + _('Batch Analysis')
        fig.text(0.5, 0.50, _('Store Data') + ' - ' + _('Batch Analysis'),
    ...
    def _create_batch_data_table(self, pdf: PdfPages):
        """Create the main batch data table showing all stores with their
        ...
        Paginates if there are too many store rows.
        """
        _ = self._
        stores = self.report.get('stores', [])
        ...
        num_pages = (num_stores + max_rows_per_page - 1) // max_rows_per_page
        ...
        for page in range(num_pages):
            ...
            end_idx = min(start_idx + max_rows_per_page, num_stores)
...
def export(report, space_name, reporting_start_datetime_local,
    ...
    """
    exporter = StoreBatchPDFExporter(language)
