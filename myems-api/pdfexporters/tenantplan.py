"""
Tenant Plan PDF Exporter

This module exports tenant energy plan analysis (plan - actual) data to PDF format.
The content mirrors the Excel exporter (excelexporters/tenantplan.py).
...
class TenantPlanPDFExporter:
    """
    Export tenant plan data to PDF format.
    ...
    def _create_cover_page(self, pdf, name, period_type,
        ...
        # Title
        fig.text(0.5, 0.50, _('Tenant Data') + ' - ' + _('Energy Plan Analysis'),
    ...
    def _create_combined_analysis_page(self, pdf: PdfPages):
        ...
        plan_row = [_('Plan')]
        for i in range(ca_len):
            plan_row.append(str(round2(subtotals_saving[i], 2)))
...
def export(report, name, base_period_start_datetime_local,
    ...
    """
    exporter = TenantPlanPDFExporter(language)
