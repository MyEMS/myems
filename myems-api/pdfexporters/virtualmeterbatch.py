"""
Virtual Meter Batch PDF Exporter

This module provides functionality to export virtual meter batch data to PDF format.
It generates comprehensive reports showing energy consumption data for multiple
virtual meters within a specific space and time period.

Key Features:
- Multi-virtual-meter energy consumption comparison table
- Daily energy consumption breakdown per virtual meter
...
Note: the content mirrors the Excel exporter (excelexporters/virtualmeterbatch.py)
exactly - the same columns, the same rows and the same 2-decimal precision.
The batch report is a wide matrix (rows = virtual meters, columns = dates) and has
no chart, matching the Excel output.
"""
...
class VirtualMeterBatchPDFExporter:
    """
    Export virtual meter batch data to PDF format.
    Generates a wide matrix table (rows = virtual meters, columns = dates) matching
    ...
    def export(self,
        ...
        logger.info("Starting PDF generation for virtual meter batch")
    ...
    def _create_cover_page(self, pdf: PdfPages):
        ...
        # Title - _('Meter Data') + ' - ' + _('Virtual Meter Batch Analysis')
        fig.text(0.5, 0.50, _('Meter Data') + ' - ' + _('Virtual Meter Batch Analysis'),
    ...
    def _create_batch_data_table(self, pdf: PdfPages):
        """Create the main batch data table showing all virtual meters with their
        daily energy consumption and subtotal. Mirrors the Excel exporter columns:
        [ID, Name, Space, Energy Category, one column per date, Total].
        Paginates if there are too many virtual meter rows.
        """
        _ = self._
        ...
        # Resolve the date axis exactly like the Excel exporter does
        date_list = list(self.report.get('date_list') or [])
        if len(date_list) == 0:
            first_meter = meters[0]
            date_list = [item.get('date') for item in first_meter.get('daily_values', [])
                         if item.get('date')]

        n_dates = len(date_list)

        # Column headers: ID, Name, Space, Energy Category, dates..., Total
        col_headers = [_('ID'), _('Name'), _('Space'), _('Energy Category')]
        for day in date_list:
            col_headers.append(str(day))
        col_headers.append(_('Total'))
        ...
        # Column widths: fixed leading/trailing columns, dates share the middle
        fixed_w = 0.05 + 0.13 + 0.13 + 0.09 + 0.08  # ID + Name + Space + EC + Total
        remaining_w = 1.0 - fixed_w
        date_col_w = remaining_w / n_dates if n_dates > 0 else 0.1
        col_widths = [0.05, 0.13, 0.13, 0.09] + [date_col_w] * n_dates + [0.08]
        ...
        for page in range(num_pages):
            ...
            fig = plt.figure(figsize=self.page_size)
            if num_pages == 1:
                fig.suptitle(self.space_name + ' - ' + _('Virtual Meter Batch Analysis'),
            ...
            # Build table data; daily values and subtotal rounded to 2 decimals
            table_data = [col_headers]
            for meter in page_meters:
                daily_value_map = {item.get('date'): item.get('value')
                                   for item in meter.get('daily_values', [])
                                   if item.get('date')}
                row = [str(meter.get('id', '')),
                       str(meter.get('virtual_meter_name', '')),
                ...
                subtotal = meter.get('subtotal')
                row.append(str(round2(subtotal, 2)) if subtotal is not None else '')
...
def export(report, space_name, reporting_start_datetime_local,
    ...
    """
    exporter = VirtualMeterBatchPDFExporter(language)
