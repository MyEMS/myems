"""
Tenant Comparison PDF Exporter

This module exports tenant comparison analysis of two tenants data to PDF format.
The content mirrors the Excel exporter (excelexporters/tenantcomparison.py).
...
class TenantComparisonPDFExporter:
    """
    Export tenant comparison data to PDF format.
    ...
    def export(self,
        ...
        logger.info(f"Starting PDF generation for TenantComparison: {tenant1_name} vs {tenant2_name}")

        pdf_filename = self.generate_pdf(
            report, tenant1_name, tenant2_name, energy_category_name,
        ...
        logger.info(f"PDF generation completed in {elapsed:.2f}s for TenantComparison")
        return result
    ...
    def generate_pdf(self,
        ...
        if "reporting_period1" not in report.keys() or \
            ...
            with PdfPages(filename) as pdf:
                self._create_cover_page(pdf, tenant1_name, tenant2_name,
    ...
    def _create_cover_page(self, pdf, tenant1_name, tenant2_name,
                           energy_category_name,
        ...
        # Title
        fig.text(0.5, 0.50, _('Tenant Data') + ' - ' + _('Tenant Comparison'),
    ...
    def _create_combined_analysis_page(self, pdf: PdfPages):
        ...
        # ===== Create figure =====
        fig = plt.figure(figsize=self.page_size)
        fig.suptitle(self.tenant1_name + ' & ' + self.tenant2_name + ' - ' +
        ...
        summary_data = [
            ['', cat_name + ' (' + unit + ')'],
            [self.tenant1_name, str(total1)],
            [self.tenant2_name, str(total2)],
    ...
    def _create_detailed_data_page(self, pdf: PdfPages):
        ...
        for page in range(num_pages):
            ...
            fig = plt.figure(figsize=self.page_size)
            fig.suptitle(self.tenant1_name + ' and ' + self.tenant2_name + ' ' +
                         _('Detailed Data'),
                         fontsize=14, weight='bold', y=0.98)
            ...
            col_headers = [
                _('Datetime'),
                self.tenant1_name + ' ' + cat_name + ' (' + unit + ')',
                self.tenant2_name + ' ' + cat_name + ' (' + unit + ')',
                _('Difference')
            ]
    ...
    def _create_parameters_page(self, pdf: PdfPages):
        """Create parameters pages for both tenant1 and tenant2."""
        _ = self._

        for param_key, tenant_name in [('parameters1', self.tenant1_name),
                                       ('parameters2', self.tenant2_name)]:
            ...
            for batch in range(num_batches):
                ...
                fig = plt.figure(figsize=self.page_size)
                fig.suptitle(tenant_name + ' ' + _('Parameters') +
...
# Convenience function with the same signature as excelexporters.tenantcomparison.export
def export(report, tenant1_name, tenant2_name, energy_category_name,
           reporting_start_datetime_local, reporting_end_datetime_local,
           period_type, language):
    """
    Export report data to PDF and return base64 encoded string.
    ...
    """
    exporter = TenantComparisonPDFExporter(language)
    return exporter.export(report, tenant1_name, tenant2_name, energy_category_name,
                           reporting_start_datetime_local,
                           reporting_end_datetime_local,
                           period_type, language)
