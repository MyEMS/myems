"""
Shopfloor Energy Category PDF Exporter

This module exports shopfloor energy consumption breakdown by categories data to PDF format.
The content mirrors the Excel exporter (excelexporters/shopfloorenergycategory.py).
...
class ShopfloorEnergyCategoryPDFExporter:
    """
    Export shopfloor energy category data to PDF format.
    ...
    def _create_cover_page(self, pdf: PdfPages, name: str, period_type: str,
        ...
        fig.text(0.5, 0.50, _('Shopfloor Data') + ' - ' + _('Energy Category Analysis'),
    ...
    def _create_base_period_working_days_page(self, pdf: PdfPages):
        ...
        shopfloor_working_calendars = self.report.get('shopfloor', {}).get('working_calendars', [])

        for i in range(ca_len):
            label = names[i] + ' (' + units[i] + ')'
            nw_val = non_working[i] if i < len(non_working) else 0
            w_val = working[i] if i < len(working) else 0
            nw_display = str(nw_val) if len(shopfloor_working_calendars) > 0 and nw_val > 0 else '-'
            w_display = str(w_val) if len(shopfloor_working_calendars) > 0 and w_val > 0 else '-'
    ...
    def _create_parameters_page(self, pdf: PdfPages):
        ...
        Performance optimization: instead of creating 2 separate figure pages per parameter
        (table page + chart page), we batch 4 parameters per page, each showing a compact
        25-row table above and a small chart below. This dramatically reduces figure count.
        """
        _ = self._
...
def export(report, name, base_period_start_datetime_local,
    ...
    """
    exporter = ShopfloorEnergyCategoryPDFExporter(language)
