"""
Equipment Tracking PDF Exporter

This module provides functionality to export equipment tracking data to PDF format.
It generates comprehensive reports showing equipment tracking information
with a cover page and detailed data table.

Key Features:
- Equipment tracking data export to PDF
...
class EquipmentTrackingPDFExporter:
    """
    Export equipment tracking data to PDF format.
    ...
    def export(self,
        ...
        logger.info("Starting PDF generation for equipment tracking")

        pdf_filename = self.generate_pdf(report, space_name, language)
    ...
    def _create_cover_page(self, pdf: PdfPages):
        ...
        ax_table = fig.add_axes([0.25, 0.15, 0.50, 0.25])
        ax_table.axis('off')
    ...
    def _create_data_table(self, pdf: PdfPages):
        """Create equipment data table with pagination.
        ...
        for page in range(num_pages):
            ...
            fig = plt.figure(figsize=self.page_size)
            if num_pages == 1:
                fig.suptitle(self.space_name + ' - ' + _('Equipment Tracking'),
            ...
            for eq in page_equipments:
                row = [
                    str(eq.get('id', '')),
                    str(eq.get('equipment_name', '')),
                    str(eq.get('space_name', '')),
                    str(eq.get('cost_center_name', '')),
            ...
            # Column widths: ID=0.06, Name=0.22, Space=0.28, CostCenter=0.18, Description=0.26
            col_widths = [0.06, 0.22, 0.28, 0.18, 0.26]
...
def export(report, space_name, language):
    ...
    """
    exporter = EquipmentTrackingPDFExporter(language)
