"""
Shopfloor Plan PDF Exporter

This module exports store energy plan analysis (plan - actual) data to PDF format.
The content mirrors the Excel exporter (excelexporters/shopfloorplan.py).
...
class ShopfloorPlanPDFExporter:
    """
    Export shopfloor plan data to PDF format.
    ...
    def _create_cover_page(self, pdf, name, period_type,
        ...
        # Title
        fig.text(0.5, 0.50, _('Shopfloor Data') + ' - ' + _('Energy Plan Analysis'),
    ...
    def _create_combined_analysis_page(self, pdf: PdfPages):
        """Create combined analysis page: Reporting Period Plan table on top,
        ...
        # ===== Build Reporting Period Plan table data =====
        col_headers = ['']
        ...
        for i in range(ca_len):
            ...
            plan_row.append(str(round2(val, 2)) if val is not None else '')
        plan_row.append(str(round2(reporting_data.get('total_in_kgce_saving', 0) / 1000, 2)))
        plan_row.append(str(round2(reporting_data.get('total_in_kgco2e_saving', 0) / 1000, 2)))
        ...
        rp_table_data = [col_headers, plan_row, per_area_row, increment_row]
        ...
        fig = plt.figure(figsize=self.page_size)
        fig.suptitle(self.name + ' - ' + _('Reporting Period Plan'),
        ...
        # ===== Top: Reporting Period Plan table (full width) =====
    ...
    def _create_detailed_data_page(self, pdf: PdfPages):
        ...
        Paginated at 100 rows per page to handle large datasets.
        """
        _ = self._
...
def export(report, name, base_period_start_datetime_local,
    ...
    """
    exporter = ShopfloorPlanPDFExporter(language)
