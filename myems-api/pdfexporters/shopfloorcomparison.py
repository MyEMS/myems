"""
Shopfloor Comparison PDF Exporter

This module exports store comparison analysis of two stores data to PDF format.
The content mirrors the Excel exporter (excelexporters/shopfloorcomparison.py).
...
class ShopfloorComparisonPDFExporter:
    """
    Export shopfloor comparison data to PDF format.
    ...
    def export(self,
        ...
        logger.info(f"Starting PDF generation for StoreComparison: {store1_name} vs {store2_name}")

        pdf_filename = self.generate_pdf(
            report, store1_name, store2_name, energy_category_name,
        ...
        logger.info(f"PDF generation completed in {elapsed:.2f}s for StoreComparison")
        return result
    ...
    def generate_pdf(self,
        ...
        if "reporting_period1" not in report.keys() or \
            ...
            with PdfPages(filename) as pdf:
                self._create_cover_page(pdf, store1_name, store2_name,
    ...
    def _create_cover_page(self, pdf, store1_name, store2_name,
                           energy_category_name,
        ...
        # Title
        fig.text(0.5, 0.50, _('Shopfloor Data') + ' - ' + _('Shopfloor Comparison'),
                 fontsize=24, weight='bold', ha='center', va='center')

        # Info list
        info_data = [
            [_('Store') + '1:', store1_name],
            [_('Store') + '2:', store2_name],
    ...
    def _create_combined_analysis_page(self, pdf: PdfPages):
        ...
        # ===== Create figure =====
        fig = plt.figure(figsize=self.page_size)
        fig.suptitle(self.store1_name + ' & ' + self.store2_name + ' - ' +
        ...
        summary_data = [
            ['', cat_name + ' (' + unit + ')'],
            [self.store1_name, str(total1)],
            [self.store2_name, str(total2)],
    ...
    def _create_detailed_data_page(self, pdf: PdfPages):
        ...
        unit = self.unit
        cat_name = self.energy_category_name
        rows_per_page = 30  # 50/100 rows overflow the page, see doc 3.5
        ...
        for page in range(num_pages):
            ...
            fig = plt.figure(figsize=self.page_size)
            fig.suptitle(self.store1_name + ' and ' + self.store2_name + ' ' +
                         _('Detailed Data'),
                         fontsize=14, weight='bold', y=0.98)

            ax_table = fig.add_axes([0.06, 0.06, 0.88, 0.84])
            ax_table.axis('off')

            col_headers = [
                _('Datetime'),
                self.store1_name + ' ' + cat_name + ' (' + unit + ')',
                self.store2_name + ' ' + cat_name + ' (' + unit + ')',
                _('Difference')
            ]
    ...
    def _create_parameters_page(self, pdf: PdfPages):
        """Create parameters pages for both store1 and store2."""
        _ = self._

        for param_key, store_name in [('parameters1', self.store1_name),
                                       ('parameters2', self.store2_name)]:
            ...
            for batch in range(num_batches):
                ...
                fig = plt.figure(figsize=self.page_size)
                fig.suptitle(store_name + ' ' + _('Parameters') +
...
# Convenience function with the same signature as excelexporters.shopfloorcomparison.export
def export(report, store1_name, store2_name, energy_category_name,
           reporting_start_datetime_local, reporting_end_datetime_local,
           period_type, language):
    """
    Export report data to PDF and return base64 encoded string.
    Provides a unified signature matching the Excel exporter.
    """
    exporter = ShopfloorComparisonPDFExporter(language)
    return exporter.export(report, store1_name, store2_name, energy_category_name,
                           reporting_start_datetime_local,
                           reporting_end_datetime_local,
                           period_type, language)
