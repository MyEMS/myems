"""
Store Energy Category PDF Exporter

This module exports store energy consumption breakdown by categories data to PDF format.
The content mirrors the Excel exporter (excelexporters/storeenergycategory.py).
...
class StoreEnergyCategoryPDFExporter:
    """
    Export store energy category data to PDF format.
    ...
    def _create_cover_page(self, pdf: PdfPages, name: str, period_type: str,
        ...
        fig.text(0.5, 0.50, _('Store Data') + ' - ' + _('Energy Category Analysis'),
    ...
    def _create_base_period_working_days_page(self, pdf: PdfPages):
        ...
        store_working_calendars = self.report.get('store', {}).get('working_calendars', [])

        for i in range(ca_len):
            label = names[i] + ' (' + units[i] + ')'
            nw_val = non_working[i] if i < len(non_working) else 0
            w_val = working[i] if i < len(working) else 0
            nw_display = str(nw_val) if len(store_working_calendars) > 0 and nw_val > 0 else '-'
            w_display = str(w_val) if len(store_working_calendars) > 0 and w_val > 0 else '-'
...
def export(report, name, base_period_start_datetime_local,
    ...
    """
    exporter = StoreEnergyCategoryPDFExporter(language)
