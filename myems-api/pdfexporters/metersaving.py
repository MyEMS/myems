"""
Meter Saving PDF Exporter

This module provides functionality to export meter saving data to PDF format.
It generates comprehensive reports showing energy saving analysis for meters
with detailed breakdown and time-series data.

Key Features:
- Meter energy saving analysis
...
class MeterSavingPDFExporter:
    """
    Export meter saving data to PDF format.
    ...
    def generate_pdf(self,
        ...
        # saving value, Excel still produces a header-only file, so the PDF
        # produces a cover-only file (do not add, do not remove content).
        has_saving_data = \
            "reporting_period" in self.report.keys() and \
            "values_saving" in self.report['reporting_period'].keys() and \
            len(self.report['reporting_period']['values_saving']) > 0
        ...
        with PdfPages(filename) as pdf:
            ...
            if has_saving_data:
                # Reporting period saving summary table
                self._create_saving_page(pdf)
    ...
    def _create_cover_page(self, pdf: PdfPages, name: str, period_type: str,
        ...
        fig.text(0.5, 0.50, _('Meter Data') + ' - ' + _('Meter Saving'),
    ...
    def _create_saving_page(self, pdf: PdfPages):
        """Create the reporting period saving summary page.

        Mirrors the Excel block at rows 7~10: a title (name + Saving), a header
        row (empty, energy category, TCE, TCO2E Decreased) plus the Saving row
        and the Increment Rate row.
        """
        _ = self._
        ...
        table_data = [col_headers, saving_row, increment_row]

        fig = plt.figure(figsize=self.page_size)
        fig.suptitle(self.name + ' ' + _('Saving'),
    ...
    def _create_detailed_data_pages(self, pdf: PdfPages):
        ...
        chart_title = _('Reporting Period Saving') + ' - ' + category_label
        ...
        if not self.is_base_period_exists:
            ...
            base_chart_title = _('Base Period Saving') + ' / ' + chart_title
...
def export(report, name, base_period_start_datetime_local, base_period_end_datetime_local,
    ...
    """
    exporter = MeterSavingPDFExporter(language)
