"""
Space Efficiency PDF Exporter

This module provides functionality to export space efficiency data to PDF format.
...
class SpaceEfficiencyPDFExporter:
    """
    Export space efficiency data to PDF format.
    ...
    def _create_combined_analysis_page(self, pdf: PdfPages):
        """Create combined analysis page: Reporting Period Cumulative Efficiency table only (matching Excel)."""
        _ = self._
        ...
        # --- Build Reporting Period Cumulative Efficiency table data (matching Excel layout) ---
        rp_col_headers = ['']
        ...
        ax_rp = fig.add_axes([0.05, 0.10, 0.90, 0.80])
        ax_rp.axis('off')

        rp_num_cols = len(rp_col_headers)
        rp_col_widths = [0.15] + [0.85 / ca_len] * ca_len
    ...
    def _create_detailed_data_page(self, pdf: PdfPages):
        ...
        if not self.is_base_period_exists:
            ...
            for page in range(num_pages):
                ...
                for i in range(ca_len):
                    subtotal_row.append(str(round2(cumulations[i], 2)) if i < len(cumulations) and cumulations[i] is not None else '')
            ...
            for page in range(num_pages):
                ...
                for t_idx in range(start_row, end_row):
                    ...
                    for j in range(base_ca_len):
                        if t_idx < len(base_values[j]) and base_values[j][t_idx] is not None:
                    ...
                    for j in range(reporting_ca_len):
                        if t_idx < len(values[j]) and values[j][t_idx] is not None:
                ...
                subtotal_row = [_('Subtotal')]
                for i in range(base_ca_len):
                    subtotal_row.append(str(round2(base_cumulations[i], 2)) if i < len(base_cumulations) and base_cumulations[i] is not None else '')
    ...
    def _create_detailed_data_charts(self, pdf: PdfPages):
        ...
        logger.info(f"Detailed data charts: timestamps_len={len(timestamps)}, "
                    f"names={names}, values_len={len(values)}")
        if timestamps:
            logger.info(f"timestamps[0] type={type(timestamps[0])}, "
                        f"len={len(timestamps[0])}, first={timestamps[0][:2] if timestamps[0] else 'empty'}")
        if values:
            logger.info(f"values[0] type={type(values[0])}, len={len(values[0])}, "
                        f"sample={values[0][:3] if values[0] else 'empty'}")

        if not timestamps or len(timestamps[0]) == 0 or not names:
            logger.warning("No data for detailed data charts, skipping")
            return
        ...
        if not self.is_base_period_exists:
            ...
            logger.info(f"No base period: {num_categories} categories, {num_pages} pages")
            ...
            for page in range(num_pages):
                ...
                for idx, i in enumerate(page_categories):
                    try:
                        ...
                        logger.error(f"Error plotting chart {i} ({names[i]}): {e}")
            ...
            num_pages = (num_categories + charts_per_page - 1) // charts_per_page
            logger.info(f"With base period: {num_categories} categories, {num_pages} pages")
...
def export(report, name, base_period_start_datetime_local,
    ...
    """
    exporter = SpaceEfficiencyPDFExporter(language)
