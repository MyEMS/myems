"""
Shopfloor Energy Item PDF Exporter

This module exports store energy consumption breakdown by energy items data to PDF format.
The content mirrors the Excel exporter (excelexporters/shopfloorenergyitem.py).
...
class ShopfloorEnergyItemPDFExporter:
    """
    Export shopfloor energy item data to PDF format.
    ...
    def _create_cover_page(self, pdf, name, period_type,
        ...
        # Title
        fig.text(0.5, 0.50, _('Shopfloor Data') + ' - ' + _('Energy Item Analysis'),
    ...
    def _create_consumption_summary_page(self, pdf: PdfPages):
        ...
        for i in range(ca_len):
            cat_name = energy_category_names[i] if i < len(energy_category_names) else ''
            rp_col_headers.append(names[i] + ' ' + cat_name + ' (' + units[i] + ')')

        rp_consumption_row = [_('Consumption')]
        for i in range(ca_len):
            val = subtotals[i] if i < len(subtotals) else None
            rp_consumption_row.append(str(round2(val, 2)) if val is not None else '')
    ...
    def _create_category_group_pages(self, pdf: PdfPages):
        ...
        for category_name, item_indices in category_dict.items():
            ...
            fig = plt.figure(figsize=self.page_size)
            fig.suptitle(self.name + ' ' + category_name + ' (' + unit + ') ',
...
def export(report, name, base_period_start_datetime_local,
    ...
    """
    exporter = ShopfloorEnergyItemPDFExporter(language)
