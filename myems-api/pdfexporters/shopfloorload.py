"""
Shop Floor Load PDF Exporter

This module provides functionality to export shop floor load data to PDF format.
It generates comprehensive reports showing load analysis for shop floors
with detailed breakdown by energy categories and time periods.

Key Features:
- Shop floor load analysis (average load, maximum load, load factor)
...
class ShopfloorLoadPDFExporter:
    """
    Export shop floor load data to PDF format.
    ...
    def export(self,
        ...
        if pdf_filename and os.path.exists(pdf_filename):
            try:
                ...
                logger.info(f"PDF encoded: {len(binary_data)} bytes -> {len(result)} base64 chars")
            except Exception as e:
        ...
        logger.info(f"PDF generation completed in {elapsed:.2f}s for {name}, result length: {len(result)}")
        return result
    ...
    def _create_parameters_page(self, pdf):
        ...
        for batch in range(num_batches):
            ...
            for idx, pi in enumerate(valid_params):
                ...
                # Line chart (filter None/invalid values to prevent matplotlib crash)
                ax_chart = fig.add_subplot(gs[idx, 1])
                valid_xs, valid_ys = [], []
                for vi, vv in enumerate(data):
                    if vv is not None:
                        try:
                            valid_ys.append(float(vv))
                            valid_xs.append(vi)
                        except (TypeError, ValueError):
                            pass
                if valid_ys:
                    marker_step_p = max(1, len(valid_ys) // 20)
                    ax_chart.plot(valid_xs, valid_ys, linewidth=1.2,
                                  color='#5B9BD5', marker='o', markersize=3,
                                  markevery=marker_step_p, label=name)
                    ax_chart.fill_between(valid_xs, valid_ys, alpha=0.15, color='#5B9BD5')
...
def export(report, name, base_period_start_datetime_local,
    ...
    """
    exporter = ShopfloorLoadPDFExporter(language)
