...
- Child spaces data page (table + pie charts per energy category)
...
class SpaceCostPDFExporter:
    ...
    def _create_child_spaces_page(self, pdf):
        """Create child spaces data page matching Excel table + pie charts per energy category."""
        _ = self._
        ...
        child_names = child['child_space_names_array'][0]
        ...
        # Build table data: ID, Child Space, then each energy category
        col_headers = [_('ID'), _('Child Space')]
        for i in range(ca_len):
            col_headers.append(category_names[i] + ' (' + child_units[i] + ')')
        ...
        gs = gridspec.GridSpec(1 + num_pie_rows, 1,
                               height_ratios=[0.5] + [0.5] * num_pie_rows)
        ...
        for i in range(ca_len):
            col_widths.append(0.83 / max(ca_len, 1))
        ...
        # Pie charts - one per energy category, 3 per row
        for i in range(ca_len):
            row_idx = 1 + i // 3
            col_idx = i % 3
            if col_idx == 0:
            ...
            ax_pie.set_title(category_names[i] + ' (' + child_units[i] + ')',
                             fontsize=9, weight='bold')
