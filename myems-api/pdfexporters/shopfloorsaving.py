"""
Shopfloor Saving PDF Exporter

This module exports store energy saving analysis (baseline - actual) data to PDF format.
The content mirrors the Excel exporter (excelexporters/shopfloorsaving.py).
...
class ShopfloorSavingPDFExporter:
    """
    Export shopfloor saving data to PDF format.
    ...
    def _create_cover_page(self, pdf, name, period_type,
        ...
        # Title
        fig.text(0.5, 0.50, _('Shopfloor Data') + ' - ' + _('Energy Saving Analysis'),
    ...
    def _create_combined_analysis_page(self, pdf: PdfPages):
        ...
        saving_row = [_('Saving')]
        for i in range(ca_len):
            val = subtotals_saving[i] if i < len(subtotals_saving) else None
            saving_row.append(str(round2(val, 2)) if val is not None else '')
...
def export(report, name, base_period_start_datetime_local,
    ...
    """
    exporter = ShopfloorSavingPDFExporter(language)
