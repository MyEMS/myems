"""
Space Saving PDF Exporter

This module provides functionality to export space saving data to PDF format.
...
Key Features:
- Space energy saving analysis (baseline - actual)
- Base period vs reporting period comparison
- Saving breakdown by energy categories (TCE/TCO2E)
- Detailed data with line charts
- Multi-language support
- Base64 encoding for file transmission

The exported PDF file includes:
- Cover page with report metadata (Energy Saving Analysis)
- Combined analysis page (saving summary table + TCE/TCO2E breakdown tables with pie charts)
...
class SpaceSavingPDFExporter:
    """
    Export space saving data to PDF format.
    ...
    def generate_pdf(self,
        ...
        with PdfPages(filename) as pdf:
            ...
            # Combined analysis page (saving table + TCE/TCO2E breakdown tables with pie charts)
    ...
    def _create_combined_analysis_page(self, pdf: PdfPages):
        """Create combined analysis page: Reporting Period Saving table on top,
        TCE/TCO2E breakdown tables with pie charts on bottom.
        Content matches Excel export exactly.
        """
        _ = self._
        ...
        # ===== Build Reporting Period Saving table data =====
        col_headers = ['']
        for i in range(ca_len):
            col_headers.append(names[i] + '   (' + _('Baseline') + ' - ' + _('Actual') + ')(' + units[i] + ')')
        col_headers.append(_('Ton of Standard Coal') + '  (' + _('Baseline') + ' - ' + _('Actual') + ') (TCE)')
        col_headers.append(_('Ton of Carbon Dioxide Emissions') + '  (' + _('Baseline') + ' - ' + _('Actual') + ') (TCO2E)')

        saving_row = [_('Saving')]
        for i in range(ca_len):
            saving_row.append(str(round2(subtotals_saving[i], 2)))
        saving_row.append(str(round2(reporting_data.get('total_in_kgce_saving', 0) / 1000, 2)))
        saving_row.append(str(round2(reporting_data.get('total_in_kgco2e_saving', 0) / 1000, 2)))

        per_area_row = [_('Per Unit Area')]
        for i in range(ca_len):
            val = subtotals_per_unit_area_saving[i] if i < len(subtotals_per_unit_area_saving) else None
            per_area_row.append(str(round2(val, 2)) if val is not None else '')
        kgce_per_area = reporting_data.get('total_in_kgce_per_unit_area_saving', None)
        kgco2e_per_area = reporting_data.get('total_in_kgco2e_per_unit_area_saving', None)
        ...
        for i in range(ca_len):
            val = increment_rates_saving[i] if i < len(increment_rates_saving) else None
        ...
        rp_table_data = [col_headers, saving_row, per_area_row, increment_row]
        ...
        for i in range(ca_len):
            saving_tce = round2(subtotals_in_kgce_saving[i] / 1000, 3) if i < len(subtotals_in_kgce_saving) else 0
            proportion = (str(round2(subtotals_in_kgce_saving[i] / kgce_sum * 100, 2)) + '%'
                          if abs(kgce_sum) > 0 else '-')
            tce_table_data.append([str(i + 1), names[i], str(saving_tce), proportion])
        ...
        for i in range(ca_len):
            saving_co2e = (round2(subtotals_in_kgco2e_saving[i] / 1000, 3)
            ...
            co2e_table_data.append([str(i + 1), names[i], str(saving_co2e), proportion])

        # ===== Create figure with nested layout =====
        fig = plt.figure(figsize=self.page_size)
        fig.suptitle(self.name + ' - ' + _('Reporting Period Saving'),
                     fontsize=14, weight='bold', y=0.98)

        # Main layout: top = summary table, bottom = TCE/TCO2E breakdown
        gs_main = gridspec.GridSpec(2, 1, height_ratios=[0.30, 0.70], hspace=0.25)

        # ===== Top: Reporting Period Saving table (full width) =====
        ax_rp = fig.add_subplot(gs_main[0])
        ax_rp.axis('off')

        rp_num_cols = len(col_headers)
...
def export(report, name, base_period_start_datetime_local,
    ...
    """
    exporter = SpaceSavingPDFExporter(language)
