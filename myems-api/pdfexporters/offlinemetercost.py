"""
Offline Meter Cost PDF Exporter

This module provides functionality to export offline meter cost data to PDF format.
It generates comprehensive reports showing cost analysis for offline meters
with detailed analysis including base period comparison and time-series data.

Key Features:
- Offline meter cost analysis
...
class OfflineMeterCostPDFExporter:
    """
    Export offline meter cost data to PDF format.
    ...
    def _create_cover_page(self, pdf: PdfPages, name: str, period_type: str,
        ...
        fig.text(0.5, 0.50, _('Meter Data') + ' - ' + _('Offline Meter Cost'),
    ...
    def _create_consumption_page(self, pdf: PdfPages):
        """Create the reporting period costs summary page.

        Mirrors the Excel block at rows 8~10: a header row (empty, energy category,
        TCE, TCO2E) plus the Cost row and the Increment Rate row.
        """
        _ = self._
        ...
        table_data = [col_headers, cost_row, increment_row]
    ...
    def _create_detailed_data_pages(self, pdf: PdfPages):
        ...
        chart_title = _('Reporting Period Costs') + ' - ' + category_label
        ...
        if not self.is_base_period_exists:
            ...
            base_chart_title = _('Base Period Costs') + ' / ' + chart_title
...
def export(report, name, base_period_start_datetime_local, base_period_end_datetime_local,
    ...
    """
    exporter = OfflineMeterCostPDFExporter(language)
