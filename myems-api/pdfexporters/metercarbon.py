"""
Meter Carbon PDF Exporter

This module provides functionality to export meter carbon emissions data to PDF format.
It generates comprehensive reports showing carbon dioxide emissions for meters
with detailed analysis including base period comparison and time-series data.

Key Features:
- Meter carbon emissions analysis
...
- Meter carbon emissions summary
...
class MeterCarbonPDFExporter:
    """
    Export meter carbon emissions data to PDF format.
    ...
    def export(self,
        ...
        # Same empty-data condition as the Excel exporter: no file is produced
    ...
    def generate_pdf(self,
        ...
        self.energy_category_name = self.report['meter']['energy_category_name']
        self.unit_of_measure = self.report['meter']['unit_of_measure']

        # Check if base period exists
        self.is_base_period_exists = is_base_period_timestamp_exists(report['base_period'])
        ...
        with PdfPages(filename) as pdf:
            ...
            # Reporting period carbon dioxide emissions summary table
            self._create_consumption_page(pdf)
    ...
    def _create_cover_page(self, pdf: PdfPages, name: str, period_type: str,
        ...
        fig.text(0.5, 0.50, _('Meter Data') + ' - ' + _('Meter Carbon'),
    ...
    def _create_consumption_page(self, pdf: PdfPages):
        """Create the reporting period carbon dioxide emissions summary page.

        Mirrors the Excel block at rows 8~10: a header row (empty, energy category,
        TCE, TCO2E) plus the Carbon Dioxide Emissions row and the Increment Rate row.
        """
        _ = self._
        ...
        table_data = [col_headers, carbon_row, increment_row]
        ...
        # Header row is green (matches Excel row 8)
        for j in range(num_cols):
            table[0, j].set_facecolor(self.colors['table_header'])
            table[0, j].set_text_props(weight='bold')
        # Row labels are bold but not filled (matches Excel B9/B10)
    ...
    def _create_detailed_data_pages(self, pdf: PdfPages):
        ...
        chart_title = _('Reporting Period Carbon Dioxide Emissions') + ' - ' + category_label
        ...
        if not self.is_base_period_exists:
            ...
            base_chart_title = _('Base Period Carbon Dioxide Emissions') + ' / ' + chart_title
            ...
            for page in range(num_pages):
                ...
                # Line chart of the current page, base period and reporting period overlaid
                ax_chart = fig.add_axes([0.08, 0.11, 0.84, 0.26])
...
def export(report, name, base_period_start_datetime_local, base_period_end_datetime_local,
           reporting_start_datetime_local, reporting_end_datetime_local, period_type, language):
    """
    Export report data to PDF and return base64 encoded string.
    This function maintains the same interface as the Excel exporter.
    """
    exporter = MeterCarbonPDFExporter(language)
