"""
Shopfloor Prediction PDF Exporter

This module exports store energy prediction analysis data to PDF format.
The content mirrors the Excel exporter (excelexporters/shopfloorprediction.py).
...
class ShopfloorPredictionPDFExporter:
    """
    Export shopfloor prediction data to PDF format.
    ...
    def _create_base_period_working_days_page(self, pdf: PdfPages):
        ...
        self._draw_working_days_page(pdf, _('Base Period Consumption'),
                                     names, units, non_working, working)
    ...
    def _create_reporting_working_days_page(self, pdf: PdfPages):
        ...
        self._draw_working_days_page(pdf, _('Reporting Period Consumption'),
                                     names, units, non_working, working)

    def _draw_working_days_page(self, pdf, section_title, names, units, non_working, working):
        """Draw a single working/non-working days table page."""
        _ = self._

        fig, ax = plt.subplots(figsize=self.page_size)
        ax.axis('off')

        fig.suptitle(self.name + ' ' + section_title,
        ...
        for i in range(len(names)):
            ...
            table_data.append([label,
                               str(nw_val) if nw_val and nw_val > 0 else '-',
    ...
    def _create_cover_page(self, pdf, name, period_type,
        ...
        # Title
        fig.text(0.5, 0.50, _('Shopfloor Data') + ' - ' + _('Energy Prediction Analysis'),
    ...
    def _create_combined_analysis_page(self, pdf: PdfPages):
        ...
        tce_table_data = [[_('Energy Category'), _('Ton of Standard Coal(TCE) by Prediction')]]
        for i in range(ca_len):
        ...
        co2e_table_data = [[_('Energy Category'), _('Ton of Carbon Dioxide Emissions(TCO2E) by Prediction') ]]
        for i in range(ca_len):
        ...
        # Row labels are bold but not filled (matches meterprediction summary table style)
        ...
        ax_tce_pie.set_title(self.name + ' ' + _('Ton of Standard Coal(TCE) by Prediction'),
                             fontsize=8, weight='bold')
        ...
        ax_co2e_pie.set_title(self.name + ' ' + _('Ton of Carbon Dioxide Emissions(TCO2E) by Prediction'),
                              fontsize=8, weight='bold')
...
def export(report, name, base_period_start_datetime_local,
    ...
    """
    exporter = ShopfloorPredictionPDFExporter(language)
