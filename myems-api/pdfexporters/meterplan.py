"""
Meter Plan PDF Exporter

This module provides functionality to export meter plan data to PDF format.
It generates comprehensive reports showing energy plan analysis for meters
with detailed breakdown and time-series data.

Key Features:
- Meter energy plan analysis
...
class MeterPlanPDFExporter:
    """
    Export meter plan data to PDF format.
    ...
    def generate_pdf(self,
        ...
        # plan value, Excel still produces a header-only file, so the PDF
        ...
        with PdfPages(filename) as pdf:
            ...
            if has_plan_data:
                # Reporting period plan summary table
                self._create_plan_page(pdf)
    ...
    def _create_cover_page(self, pdf: PdfPages, name: str, period_type: str,
        ...
        fig.text(0.5, 0.50, _('Meter Data') + ' - ' + _('Meter Plan'),
    ...
    def _create_plan_page(self, pdf: PdfPages):
        """Create the reporting period plan summary page.

        Mirrors the Excel block at rows 7~10: a title (name + Plan), a header
        row (empty, energy category, TCE, TCO2E Decreased) plus the Plan row
    ...
    def _create_detailed_data_pages(self, pdf: PdfPages):
        ...
        chart_title = _('Reporting Period Plan') + ' - ' + category_label
        ...
        if not self.is_base_period_exists:
            ...
            base_chart_title = _('Base Period Plan') + ' / ' + chart_title
...
def export(report, name, base_period_start_datetime_local, base_period_end_datetime_local,
    ...
    """
    exporter = MeterPlanPDFExporter(language)
