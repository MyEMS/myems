"""
Combined Equipment Output PDF Exporter

This module provides functionality to export equipment output data to PDF format.
...
Key Features:
- CombinedEquipment output analysis
...
- Combined analysis page (reporting period output table + increment rate)
...
class CombinedEquipmentOutputPDFExporter:
    """
    Export equipment output data to PDF format.
    ...
    def _create_cover_page(self, pdf, name, period_type,
        ...
        # Title
        fig.text(0.5, 0.50, _('Combined Equipment Data') + ' - ' +_('Output'),
    ...
    def _create_combined_analysis_page(self, pdf: PdfPages):
        """Create combined analysis page: Reporting Period Output table only (matching Excel)."""
        _ = self._
        ...
        # --- Build Reporting Period Output table data (matching Excel layout) ---
        rp_col_headers = ['']
        for i in range(ca_len):
            rp_col_headers.append(names[i] + ' (' + units[i] + ')')

        rp_output_row = [_('Output')]
        for i in range(ca_len):
            rp_output_row.append(str(round2(subtotals[i], 2)))
        ...
        rp_table_data = [rp_col_headers, rp_output_row, rp_increment_row]

        fig = plt.figure(figsize=self.page_size)
        fig.suptitle(self.name + ' - ' + _('Reporting Period Output'),
        ...
        if not self.is_base_period_exists:
            ...
            for page in range(num_pages):
                ...
                for idx, i in enumerate(page_categories):
                    ...
                    ax_chart.set_title(_('Reporting Period Output') + ' - ' +
                                       names[i] + ' (' + units[i] + ')',
...
def export(report, name, base_period_start_datetime_local,
    ...
    """
    exporter = CombinedEquipmentOutputPDFExporter(language)
