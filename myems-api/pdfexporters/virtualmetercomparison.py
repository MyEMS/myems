"""
Virtual Meter Comparison PDF Exporter

This module provides functionality to export virtual meter comparison data to PDF format.
It generates comprehensive reports comparing energy consumption between two virtual meters
with detailed breakdown by time periods.

Key Features:
- Two-virtual-meter energy consumption comparison
- Consumption summary table (each virtual meter keeps its own energy category and unit)
...
- Consumption summary table (virtualmeter1, virtualmeter2)
- Line chart of reporting period consumption (virtualmeter1, virtualmeter2)
...
class VirtualMeterComparisonPDFExporter:
    """
    Export virtual meter comparison data to PDF format.
    Generates comprehensive reports comparing two virtual meters' energy consumption.
    Each virtual meter keeps its own energy category name and unit of measure.
    """
    ...
    def export(self,
        ...
        logger.info("Starting PDF generation for virtual meter comparison")
    ...
    def _create_cover_page(self, pdf: PdfPages):
        ...
        # Title
        fig.text(0.5, 0.50, _('Meter Data') + ' - ' + _('Virtual Meter Comparison'),
                 fontsize=24, weight='bold', ha='center', va='center')

        # Info list (matches Excel header: Name1/Name2/Period Type/Reporting Start/End)
        info_data = [
            [_('Name') + '1:', self.name1],
            [_('Name') + '2:', self.name2],
    ...
    def _create_consumption_summary_page(self, pdf: PdfPages):
        ...
        Excel has:
          - Meter1 name as title, Consumption row with EC1(unit1) and total1
          - Meter2 name as title, Consumption row with EC2(unit2) and total2
        ...
        # --- Meter1 consumption table ---
        ax1 = fig.add_axes([0.10, 0.55, 0.80, 0.30])
        ax1.axis('off')

        m1_table = ax1.table(
            cellText=[[self.name1], [_('Consumption')]],
            loc='center', cellLoc='center', colWidths=[1.0]
        )
        m1_table.auto_set_font_size(False)
        m1_table.set_fontsize(11)
        # Title row (meter name) - green header
        m1_table[0, 0].set_facecolor('#90EE90')
        m1_table[0, 0].set_text_props(weight='bold')
        # Consumption label row
        m1_table[1, 0].set_facecolor('#90EE90')
        m1_table[1, 0].set_text_props(weight='bold')
        _style_table_borders(m1_table, 2, 1)

        # Meter1 value table (EC header + total value)
        ax1v = fig.add_axes([0.55, 0.55, 0.35, 0.30])
        ax1v.axis('off')

        val1_table = ax1v.table(
            cellText=[[ec1_unit_label], [str(total1)]],
            loc='center', cellLoc='center', colWidths=[1.0]
        )
        ...
        # --- Meter2 consumption table ---
        ax2 = fig.add_axes([0.10, 0.10, 0.80, 0.30])
        ax2.axis('off')

        m2_table = ax2.table(
            cellText=[[self.name2], [_('Consumption')]],
            loc='center', cellLoc='center', colWidths=[1.0]
        )
        m2_table.auto_set_font_size(False)
        m2_table.set_fontsize(11)
        m2_table[0, 0].set_facecolor('#90EE90')
        m2_table[0, 0].set_text_props(weight='bold')
        m2_table[1, 0].set_facecolor('#90EE90')
        m2_table[1, 0].set_text_props(weight='bold')
        _style_table_borders(m2_table, 2, 1)
        ...
        val2_table = ax2v.table(
            cellText=[[ec2_unit_label], [str(total2)]],
            loc='center', cellLoc='center', colWidths=[1.0]
        )
    ...
    def _create_line_chart(self, pdf: PdfPages):
        ...
        Excel chart references 2 series (cols C-D): meter1 and meter2 values.
        Each series keeps its own energy category name and unit in the legend.
        """
        _ = self._
        rp1 = self.report.get('reporting_period1', {})
        rp2 = self.report.get('reporting_period2', {})
        ...
        # Plot meter1 line (filter None by paired index)
    ...
    def _create_detailed_data_table(self, pdf: PdfPages):
        ...
        Column headers keep each meter's own energy category name and unit.
        """
        _ = self._
        rp1 = self.report.get('reporting_period1', {})
        rp2 = self.report.get('reporting_period2', {})
        ...
        # Column headers (match Excel: name + energy category + " (" + unit + ")")
        col_headers = [
            _('Datetime'),
        ...
        for page in range(num_pages):
            ...
            fig = plt.figure(figsize=self.page_size)
            if num_pages == 1:
                fig.suptitle(self.name1 + ' and ' + self.name2 + ' ' +
                             _('Detailed Data'),
            ...
            # Last row (Total) styling - only when the Total row is on this page
...
def export(report, name1, name2,
    ...
    """
    exporter = VirtualMeterComparisonPDFExporter(language)
