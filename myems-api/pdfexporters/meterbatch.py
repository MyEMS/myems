"""
Meter Batch PDF Exporter

This module provides functionality to export meter batch data to PDF format.
It generates comprehensive reports showing energy consumption data for multiple
meters within a specific space and time period.

Key Features:
- Multi-meter energy consumption comparison table
- Daily energy consumption breakdown per meter
...
Note: the content mirrors the Excel exporter (excelexporters/meterbatch.py)
exactly - the same columns, the same rows and the same 2-decimal precision.
The batch report is a wide matrix (rows = meters, columns = dates) and has no
chart, matching the Excel output.
"""
...
class MeterBatchPDFExporter:
    """
    Export meter batch data to PDF format.
    Generates a wide matrix table (rows = meters, columns = dates) matching
    ...
    def export(self,
        ...
        logger.info("Starting PDF generation for meter batch")
    ...
    def _create_cover_page(self, pdf: PdfPages):
        ...
        # Title - _('Meter Data') + ' - ' + _('Batch Analysis') => "计量表数据 - 批量分析"
        fig.text(0.5, 0.50, _('Meter Data') + ' - ' + _('Batch Analysis'),
    ...
    def _create_batch_data_table(self, pdf: PdfPages):
        """Create the main batch data table showing all meters with their daily
        ...
        Paginates if there are too many meter rows.
        """
        _ = self._
        ...
        # Paginate: max rows per page (table-only page, matches equipment batch)
...
def export(report, space_name, reporting_start_datetime_local,
    ...
    """
    exporter = MeterBatchPDFExporter(language)
