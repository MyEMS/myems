"""
Shopfloor Batch PDF Exporter

This module provides functionality to export shopfloor batch data to PDF format.
It generates a comparison table showing energy consumption, carbon emission and
cost for all stores of a space within a reporting period.
...
Note: the content mirrors the Excel exporter (excelexporters/shopfloorbatch.py)
...
class ShopfloorBatchPDFExporter:
    """
    Export shopfloor batch data to PDF format.
    Generates a wide matrix table (rows = shopfloors, columns = energy categories)
    ...
    def export(self,
        ...
        logger.info("Starting PDF generation for shopfloor batch")
    ...
    def _create_cover_page(self, pdf: PdfPages):
        ...
        # Title - _('Shopfloor Data') + ' - ' + _('Batch Analysis')
        fig.text(0.5, 0.50, _('Shopfloor Data') + ' - ' + _('Batch Analysis'),
    ...
    def _create_batch_data_table(self, pdf: PdfPages):
        """Create the main batch data table showing all shopfloors with their
        consumption per energy category, carbon emissions and costs.
        Mirrors the Excel exporter columns:
        [ID, Name, Space, one column per energy category, Carbon Emissions, Costs].
        Paginates if there are too many shopfloor rows.
        """
        _ = self._
        shopfloors = self.report.get('shopfloors', [])
        ...
        # Column headers: ID, Name, Space, energy categories..., Carbon Emissions, Costs
        ...
        for page in range(num_pages):
            ...
            end_idx = min(start_idx + max_rows_per_page, num_shopfloors)
            page_shopfloors = shopfloors[start_idx:end_idx]
            ...
            for shopfloor in page_shopfloors:
                values = shopfloor.get('values', []) or []
                row = [str(shopfloor.get('id', '')),
                       str(shopfloor.get('shopfloor_name', '')),
                       str(shopfloor.get('space_name', ''))]
                for j in range(n_categories):
                    value = values[j] if j < len(values) else None
                    row.append(str(round2(value, 2)) if value is not None else '')
                carbon = shopfloor.get('carbon_emissions', 0.0)
...
def export(report, space_name, reporting_start_datetime_local,
    ...
    """
    exporter = ShopfloorBatchPDFExporter(language)
