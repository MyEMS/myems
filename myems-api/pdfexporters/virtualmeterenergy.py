"""
Virtual Meter Energy PDF Exporter

This module provides functionality to export virtual meter energy data to PDF format.
It generates comprehensive reports showing energy consumption for virtual meters
with detailed analysis including base period comparison and time-series data.

Note: the content mirrors the Excel exporter (excelexporters/virtualmeterenergy.py) exactly.
...
class VirtualMeterEnergyPDFExporter:
    """
    Export virtual meter energy data to PDF format.
    ...
    def _create_cover_page(self, pdf: PdfPages, name: str, period_type: str,
        ...
        fig.text(0.5, 0.50, _('Meter Data') + ' - ' + _('Virtual Meter Energy'),
    ...
    def _create_consumption_page(self, pdf: PdfPages):
        """Create the reporting period consumption summary page.

        Mirrors the Excel block at rows 7~10: one header row (name, energy category,
        TCE, TCO2E) plus the Consumption row and the Increment Rate row.
        """
        _ = self._
        ...
        table_data = [col_headers, consumption_row, increment_row]
        ...
        for i in range(1, len(table_data)):
            table[i, 0].set_facecolor(self.colors['table_header'])
    ...
    def _create_detailed_data_pages(self, pdf: PdfPages):
        ...
        chart_title = _('Reporting Period Consumption') + ' - ' + category_label
...
def export(report, name, base_period_start_datetime_local, base_period_end_datetime_local,
    ...
    """
    exporter = VirtualMeterEnergyPDFExporter(language)
