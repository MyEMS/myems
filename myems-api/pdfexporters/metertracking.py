"""
Meter Tracking PDF Exporter

This module provides functionality to export meter tracking data to PDF format.
It generates comprehensive reports listing all meters of a space tree with their
start value, end value and difference value over the reporting period.

Key Features:
- Multi-meter tracking table (ID, Name, Space, Cost Center, Energy Category,
  Description, Start Value, End Value, Difference Value)
- Data integrity rates on the cover page
- Multi-language support
- Base64 encoding for file transmission

The exported PDF file includes:
- Cover page with report metadata and integrity rates
- Tracking data table (paginated, matching Excel content)

Note: the content mirrors the Excel exporter (excelexporters/metertracking.py)
exactly - the same columns, the same rows and the same 2-decimal precision.
The tracking report is a plain table (rows = meters) and has no chart,
matching the Excel output.
"""
...
class MeterTrackingPDFExporter:
    """
    Export meter tracking data to PDF format.
    Generates a meter list table (rows = meters) matching the Excel exporter
    layout, plus a cover page carrying the data integrity rates.
    """
    ...
    def export(self,
        ...
        logger.info("Starting PDF generation for meter tracking")

        pdf_filename = self.generate_pdf(
            report, space_name, energy_category_name,
    ...
    def generate_pdf(self,
        ...
        with PdfPages(filename) as pdf:
            ...
            # Tracking data table
            self._create_tracking_data_table(pdf)

        logger.info(f"PDF generated: {filename}")
        return filename

    @staticmethod
    def _format_integrity_rate(rate) -> str:
        """Format an integrity rate (0..1) as a percentage string like Excel does."""
        if rate is None:
            return ''
        return str(round2(rate * 100.0, 2)) + '%'
    ...
    def _create_cover_page(self, pdf: PdfPages):
        ...
        # Title - _('Meter Data') + ' - ' + _('Meter Tracking') => "计量表数据 - 计量表台账"
        fig.text(0.5, 0.50, _('Meter Data') + ' - ' + _('Meter Tracking'),
                 fontsize=24, weight='bold', ha='center', va='center')

        # Info list (mirrors the Excel query-parameter block)
        energy_category_label = self.energy_category_name \
            if self.energy_category_name is not None else _('All')
        info_data = [
            [_('Space') + ':', self.space_name],
            [_('Start Datetime') + ':', self.reporting_start],
            [_('End Datetime') + ':', self.reporting_end],
            [_('Start Integrity Rate') + ':',
             self._format_integrity_rate(self.report.get('start_integrity_rate'))],
            [_('End Integrity Rate') + ':',
             self._format_integrity_rate(self.report.get('end_integrity_rate'))],
            [_('Full Integrity Rate') + ':',
             self._format_integrity_rate(self.report.get('full_integrity_rate'))],
            [_('Energy Category') + ':', energy_category_label],
        ]

        ax_table = fig.add_axes([0.25, 0.08, 0.50, 0.40])
        ax_table.axis('off')
    ...
    def _create_tracking_data_table(self, pdf: PdfPages):
        """Create the meter tracking table mirroring the Excel exporter columns:
        [ID, Name, Space, Cost Center, Energy Category, Description,
         Start Value, End Value, Difference Value].
        Paginates if there are too many meter rows. No chart and no total row,
        matching the Excel output.
        """
        _ = self._
        ...
        col_headers = [_('ID'), _('Name'), _('Space'), _('Cost Center'),
                       ...
                       _('Start Value'), _('End Value'), _('Difference Value')]
        ...
        # Column widths: description gets the largest share, values stay narrow
        col_widths = [0.04, 0.12, 0.16, 0.11, 0.10, 0.20, 0.09, 0.09, 0.09]
        ...
        for page in range(num_pages):
            ...
            fig = plt.figure(figsize=self.page_size)
            if num_pages == 1:
                fig.suptitle(self.space_name + ' - ' + _('Meter Tracking'),
            ...
            # Build table data; values rounded to 2 decimals like Excel
            table_data = [col_headers]
            for meter in page_meters:
                start_value = meter.get('start_value')
                end_value = meter.get('end_value')
                difference_value = meter.get('difference_value')
                row = [str(meter.get('id', '')),
                       str(meter.get('meter_name', '')),
                       str(meter.get('space_name', '')),
                       str(meter.get('cost_center_name', '')),
                       str(meter.get('energy_category_name', '')),
                       str(meter.get('description', '')),
                       str(round2(start_value, 2)) if start_value is not None else '',
                       str(round2(end_value, 2)) if end_value is not None else '',
                       str(round2(difference_value, 2)) if difference_value is not None else '']
...
def export(report, space_name, energy_category_name,
           reporting_start_datetime_local, reporting_end_datetime_local, language):
    """
    Export report data to PDF and return base64 encoded string.
    This function maintains the same interface as the Excel exporter.
    """
    exporter = MeterTrackingPDFExporter(language)
    return exporter.export(report, space_name, energy_category_name,
                           reporting_start_datetime_local,
                           reporting_end_datetime_local,
                           language)
