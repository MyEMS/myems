"""
Shop Floor Plan Report API

This module provides REST API endpoints for generating shop floor plan reports.
It analyzes shop floor planning data and performance against planned targets,
providing insights into plan execution and optimization opportunities.

Key Features:
- Shop floor plan analysis
...
import pdfexporters.shopfloorplan
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:shopfloorplan:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.shopfloorplan.export(result,
                                                                               ...
                                                                               language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.shopfloorplan.export(result,
                                                                               shopfloor['name'],
                                                                               base_period_start_datetime_local,
                                                                               base_period_end_datetime_local,
                                                                               reporting_period_start_datetime_local,
