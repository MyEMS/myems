"""
Offline Meter Plan Report API

This module provides REST API endpoints for generating offline meter plan reports.
It analyzes offline meter planning data and performance against planned targets,
providing insights into plan execution and optimization opportunities.

Key Features:
- Offline meter plan analysis
...
import pdfexporters.offlinemeterplan
import docxexporters.offlinemeterplan
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:offlinemeterplan:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        try:
            ...
            try:
                ...
                for row_offline_meter_periodically in rows_offline_meter_periodically:
                    ...
                    base['total_in_kgce_plan'] += actual_value * offline_meter['kgce']
                    base['total_in_kgco2e_plan'] += actual_value * offline_meter['kgco2e']
                ...
                for row_offline_meter_periodically in rows_offline_meter_periodically:
                    ...
                    reporting['total_in_kgce_plan'] += actual_value * offline_meter['kgce']
                    reporting['total_in_kgco2e_plan'] += actual_value * offline_meter['kgco2e']
                ...
                for index, value in enumerate(reporting['values_saving']):
                    if index < len(base['values_saving']) and base['values_saving'][index] != 0 and value != 0:
                        reporting['values_rates'].append((value -
                                                          base['values_saving'][index]) / base['values_saving'][index])
                    else:
        ...
        if not is_quick_mode:
            ...
            if is_export_pdf:
                result['pdf_bytes_base64'] = \
                    pdfexporters.offlinemeterplan.export(result,
                                                         offline_meter['name'],
                                                         base_period_start_datetime_local,
                                                         base_period_end_datetime_local,
                                                         reporting_period_start_datetime_local,
            ...
            if is_export_docx:
                result['docx_bytes_base64'] = \
                    docxexporters.offlinemeterplan.export(result,
                                                          offline_meter['name'],
                                                          base_period_start_datetime_local,
                                                          base_period_end_datetime_local,
                                                          reporting_period_start_datetime_local,
