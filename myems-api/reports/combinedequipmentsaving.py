...
import docxexporters.combinedequipmentsaving
import pdfexporters.combinedequipmentsaving
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:combinedequipmentsaving:' + \
        ...
        try:
            ...
            if cnx_historical is not None:
                try:
                    ...
                    logger.warning(f"Ignored exception: {e}")
            if cursor_energy_baseline is not None:
                try:
                    cursor_energy_baseline.close()
                except Exception as e:
                    logger.warning(f"Ignored exception: {e}")
            if cnx_energy_baseline is not None:
                try:
            ...
            if is_export_docx:
                try:
                    result['docx_bytes_base64'] = \
                        docxexporters.combinedequipmentsaving.export(result,
                                                                     combined_equipment['name'],
                                                                     base_period_start_datetime_local,
                                                                     base_period_end_datetime_local,
                                                                     reporting_period_start_datetime_local,
            ...
            if is_export_pdf:
                try:
                    result['pdf_bytes_base64'] = \
                        pdfexporters.combinedequipmentsaving.export(result,
                                                                    combined_equipment['name'],
                                                                    base_period_start_datetime_local,
                                                                    base_period_end_datetime_local,
                                                                    reporting_period_start_datetime_local,
