"""
Store Cost Report API

This module provides REST API endpoints for generating store cost reports.
It analyzes energy costs associated with store energy consumption, providing insights
into cost optimization opportunities and financial performance metrics.

Key Features:
- Store energy cost analysis
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:storecost:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.storecost.export(result, store['name'], base_period_start_datetime_local, base_period_end_datetime_local, reporting_period_start_datetime_local, reporting_period_end_datetime_local, period_type, language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.storecost.export(result, store['name'], base_period_start_datetime_local, base_period_end_datetime_local, reporting_period_start_datetime_local, reporting_period_end_datetime_local, period_type, language)
