...
class Reporting:
    ...
    def on_get(req, resp):
        ...
        ################################################################################################################
        # Step 5: query combined equipment associated points data (for detailed parameters)
        ################################################################################################################
        ...
        if not is_quick_mode:
            # Query points for combined equipment 1
            cursor_system.execute(
                " SELECT p.id, cep.name, p.units, p.object_type "
                " FROM tbl_combined_equipments ce, tbl_combined_equipments_parameters cep, tbl_points p "
                " WHERE ce.id = %s AND ce.id = cep.combined_equipment_id AND cep.parameter_type = 'point' "
                "       AND cep.point_id = p.id "
                " ORDER BY p.id ",
                (combined_equipment1["id"],),
            )
            ...
            # Query points for combined equipment 2
            cursor_system.execute(
                " SELECT p.id, cep.name, p.units, p.object_type "
                " FROM tbl_combined_equipments ce, tbl_combined_equipments_parameters cep, tbl_points p "
                " WHERE ce.id = %s AND ce.id = cep.combined_equipment_id AND cep.parameter_type = 'point' "
