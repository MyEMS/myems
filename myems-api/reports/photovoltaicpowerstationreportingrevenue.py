"""
Photovoltaic Power Station Reporting Revenue Report API

This module provides REST API endpoints for generating photovoltaic power station reporting revenue reports.
It analyzes revenue generation from photovoltaic power station operations with detailed reporting,
providing insights into financial performance and revenue optimization opportunities.

Key Features:
- Photovoltaic power station revenue analysis
...
Report Components:
- Photovoltaic power station revenue summary
...
import config
import excelexporters.photovoltaicpowerstationreportingrevenue
    ...
    def on_get(req, resp):
        ...
        try:
            ...
            try:
                ...
                ##################################################################################################
                # Step 3: query billing generation data
                ##################################################################################################
                timezone_offset = int(config.utc_offset[1:3]) * 60 + int(config.utc_offset[4:6])
                if config.utc_offset[0] == '-':
                    timezone_offset = -timezone_offset
        ...
        if not is_quick_mode:
            result['excel_bytes_base64'] = \
                excelexporters.photovoltaicpowerstationreportingrevenue.\
                export(result,
