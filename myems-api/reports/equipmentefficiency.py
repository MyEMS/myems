"""
Equipment Efficiency Report API

This module provides REST API endpoints for generating equipment efficiency reports.
It analyzes equipment performance metrics and efficiency indicators to provide
...
import docxexporters.equipmentefficiency
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = \
                    'report:equipmentefficiency:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        try:
            ...
            try:
                ...
                cursor_system.execute(" SELECT p.id, ep.name, p.units, p.object_type  "
                                      " FROM tbl_equipments e, tbl_equipments_parameters ep, "
                                      "      tbl_points p "
                                      " WHERE e.id = %s AND e.id = ep.equipment_id "
                ...
                if rows_points is not None and len(rows_points) > 0:
                    for row in rows_points:
                        point_list.append({"id": row[0], "name": row[1],
                                           "units": row[2], "object_type": row[3]})
                ...
                if fraction_list is not None and len(fraction_list) > 0:
                    for fraction in fraction_list:
                        ...
                        for row_numerator_meter_periodically in rows_numerator_meter_periodically:
                            current_datetime_local = row_numerator_meter_periodically[0].replace(
                        ...
                        for row_denominator_meter_periodically in rows_denominator_meter_periodically:
                            current_datetime_local = row_denominator_meter_periodically[0].replace(
                                tzinfo=timezone.utc) + timedelta(minutes=timezone_offset)
                            if period_type == 'hourly':
                                current_datetime = current_datetime_local.isoformat()[0:19]
                            elif period_type == 'daily':
                            ...
                            actual_value = Decimal(0.0) if (row_denominator_meter_periodically[1]
                                                            is None) \
                        ...
                        for i in range(len(base[fraction['id']]['denominator_timestamps'])):
                            ...
                            value = (base[fraction['id']]['numerator_values'][i] /
                                ...
                                if base[fraction['id']]['denominator_values'][i] > Decimal(0.0) \
                                else Decimal(0.0)
                        ...
                        cumulation = (base[fraction['id']]['numerator_cumulation'] /
                            ...
                            if base[fraction['id']]['denominator_cumulation'] > Decimal(0.0) \
                            else Decimal(0.0)
                ...
                if fraction_list is not None and len(fraction_list) > 0:
                    for fraction in fraction_list:
                        ...
                        reporting[fraction['id']]['denominator_name'] = (
                            fraction)['denominator_meter_name']
                        ...
                        for row_denominator_meter_periodically in rows_denominator_meter_periodically:
                            ...
                            actual_value = Decimal(0.0) \
                                if row_denominator_meter_periodically[1] is None \
                                else row_denominator_meter_periodically[1]
                        ...
                        for i in range(len(reporting[fraction['id']]['denominator_timestamps'])):
                            ...
                            value = reporting[fraction['id']]['numerator_values'][i] / \
                                ...
                                if reporting[fraction['id']]['denominator_values'][i] > Decimal(0.0) \
                                else Decimal(0.0)
                        ...
                        cumulation = (reporting[fraction['id']]['numerator_cumulation'] /
                            ...
                            if reporting[fraction['id']]['denominator_cumulation'] > Decimal(0.0) \
                            else Decimal(0.0)
            ...
            if is_export_docx:
                try:
                    result['docx_bytes_base64'] = \
                        docxexporters.equipmentefficiency.export(
                            result,
                            equipment['name'],
                            base_period_start_datetime_local,
                            base_period_end_datetime_local,
