...
import pdfexporters.equipmentenergycategory
import docxexporters.equipmentenergycategory
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:equipmentenergycategory:' + \
        ...
        if energy_category_set is not None and len(energy_category_set) > 0:
            for energy_category_id in energy_category_set:
                ...
                result['reporting_period']['total_in_kgce'] += (
                    reporting)[energy_category_id]['subtotal_in_kgce']
                result['reporting_period']['total_in_kgco2e'] += (
                    reporting)[energy_category_id]['subtotal_in_kgco2e']
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = \
                    excelexporters.equipmentenergycategory.export(
                        result,
                        equipment['name'],
            ...
            if is_export_pdf:
                result['pdf_bytes_base64'] = \
                    pdfexporters.equipmentenergycategory.export(
                        result,
                        equipment['name'],
                        base_period_start_datetime_local,
                        base_period_end_datetime_local,
            ...
            if is_export_docx:
                try:
                    result['docx_bytes_base64'] = \
                        docxexporters.equipmentenergycategory.export(
                            result,
                            equipment['name'],
                            base_period_start_datetime_local,
                            base_period_end_datetime_local,
