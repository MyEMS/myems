"""
Equipment Energy Item Report API

This module provides REST API endpoints for generating equipment energy item reports.
It analyzes energy consumption by specific energy items for equipment,
providing detailed insights into energy usage patterns and item-specific optimizations.

Key Features:
- Equipment energy consumption by energy item analysis
...
Report Components:
- Equipment energy consumption by energy item summary
...
import config
import excelexporters.equipmentenergyitem
import pdfexporters.equipmentenergyitem
import docxexporters.equipmentenergyitem
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = \
                    'report:equipmentenergyitem:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        try:
            ...
            try:
                ...
                cursor_system.execute(" SELECT ei.id, ei.name, ei.energy_category_id, "
                                      " ec.name AS energy_category_name, ec.unit_of_measure, "
                                      " ec.kgce, ec.kgco2e "
                ...
                for row_energy_item in rows_energy_items:
                    if row_energy_item[0] in energy_item_set:
                        energy_item_dict[row_energy_item[0]] = {
                            "name": row_energy_item[1],
                ...
                if rows_points is not None and len(rows_points) > 0:
                    for row in rows_points:
                        point_list.append({"id": row[0], "name": row[1], "units": row[2],
                                           "object_type": row[3]})
                ...
                if energy_item_set is not None and len(energy_item_set) > 0:
                    for energy_item_id in energy_item_set:
                        ...
                        for row_equipment_periodically in rows_equipment_periodically:
                            current_datetime_local = row_equipment_periodically[0].replace(
                ...
                if not is_quick_mode:
                    if (config.is_tariff_appended and energy_item_set is not None
                            and len(energy_item_set) > 0):
                        for energy_item_id in energy_item_set:
                            energy_category_tariff_dict = \
            ...
            if is_export_excel:
                try:
                    result['excel_bytes_base64'] = \
                        excelexporters.equipmentenergyitem.export(
                            result,
                            equipment['name'],
            ...
            if is_export_pdf:
                try:
                    result['pdf_bytes_base64'] = \
                        pdfexporters.equipmentenergyitem.export(
                            result,
                            equipment['name'],
                            base_period_start_datetime_local,
                            base_period_end_datetime_local,
            ...
            if is_export_docx:
                try:
                    result['docx_bytes_base64'] = \
                        docxexporters.equipmentenergyitem.export(
                            result,
                            equipment['name'],
                            base_period_start_datetime_local,
                            base_period_end_datetime_local,
