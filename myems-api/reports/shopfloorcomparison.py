...
class Reporting:
    ...
    # Step 5: query shopfloor associated points data
    ...
    def on_get(req, resp):
        ...
        ################################################################################################################
        # Step 5: query shopfloor associated points data (for detailed parameters)
        ################################################################################################################
        ...
        if not is_quick_mode:
            # Query points for shopfloor 1
            cursor_system.execute(
                " SELECT p.id, e.name, p.units, p.object_type "
                " FROM tbl_shopfloors e, tbl_shopfloors_points ep, tbl_points p "
                " WHERE e.id = %s AND e.id = ep.shopfloor_id "
