"""
Store Energy Item Report API

This module provides REST API endpoints for generating store energy item reports.
It analyzes energy consumption by specific energy items for stores,
providing detailed insights into energy usage patterns and item-specific optimizations.

Key Features:
- Store energy consumption by energy item analysis
...
Report Components:
- Store energy consumption by energy item summary
...
import pdfexporters.storeenergyitem
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:storeenergyitem:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.storeenergyitem.export(result, store['name'], base_period_start_datetime_local, base_period_end_datetime_local, reporting_period_start_datetime_local, reporting_period_end_datetime_local, period_type, language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.storeenergyitem.export(result, store['name'], base_period_start_datetime_local, base_period_end_datetime_local, reporting_period_start_datetime_local, reporting_period_end_datetime_local, period_type, language)
