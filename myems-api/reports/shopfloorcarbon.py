"""
Shop Floor Carbon Report API

This module provides REST API endpoints for generating shop floor carbon emissions reports.
It analyzes carbon dioxide emissions from shop floor energy consumption, providing insights
into environmental impact and carbon footprint reduction opportunities.

Key Features:
- Shop floor carbon emissions analysis
...
Report Components:
- Shop floor carbon emissions summary
...
import config
import excelexporters.shopfloorcarbon
import pdfexporters.shopfloorcarbon
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_params = {
                    "shopfloorid": shopfloor_id,
                    "shopflooruuid": shopfloor_uuid,
                ...
                cache_key = 'report:shopfloorcarbon:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
            ...
            try:
                    ...
                    for energy_category_id in energy_category_set:
                        ...
                        cursor_carbon.execute(" SELECT start_datetime_utc, actual_value "
                                              ...
                                              (shopfloor['id'],
                                               ...
                                               base_start_datetime_utc,
                                               base_end_datetime_utc))
                        rows_shopfloor_hourly = cursor_carbon.fetchall()
                ...
                if energy_category_set is not None and len(energy_category_set) > 0:
                    for energy_category_id in energy_category_set:
                        ...
                        rows_shopfloor_periodically = utilities.aggregate_hourly_data_by_period(
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.shopfloorcarbon.export(result,
                                                                                 shopfloor['name'],
                                                                                 ...
                                                                                 language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.shopfloorcarbon.export(result,
                                                                                 shopfloor['name'],
                                                                                 base_period_start_datetime_local,
                                                                                 base_period_end_datetime_local,
                                                                                 reporting_period_start_datetime_local,
