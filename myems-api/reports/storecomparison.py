...
import pdfexporters.storecomparison
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_params = {
                    "storeid1": store_id1,
                    "storeuuid1": store_uuid1,
                    "storeid2": store_id2,
                    "storeuuid2": store_uuid2,
                ...
                cache_key = 'report:storecomparison:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        if not is_quick_mode:
            if is_export_excel:
                result["excel_bytes_base64"] = excelexporters.storecomparison.export( result, store1["name"], store2["name"], energy_category["name"], reporting_period_start_datetime_local, reporting_period_end_datetime_local, period_type, language)
            if is_export_pdf:
                result["pdf_bytes_base64"] = pdfexporters.storecomparison.export( result, store1["name"], store2["name"], energy_category["name"], reporting_period_start_datetime_local, reporting_period_end_datetime_local, period_type, language)
