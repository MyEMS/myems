"""
Store Carbon Report API

This module provides REST API endpoints for generating store carbon emissions reports.
It analyzes carbon dioxide emissions from store energy consumption, providing insights
into environmental impact and carbon footprint reduction opportunities.

Key Features:
- Store carbon emissions analysis
...
Report Components:
- Store carbon emissions summary
...
import pdfexporters.storecarbon
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:storecarbon:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.storecarbon.export(result, store['name'], base_period_start_datetime_local, base_period_end_datetime_local, reporting_period_start_datetime_local, reporting_period_end_datetime_local, period_type, language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.storecarbon.export(result, store['name'], base_period_start_datetime_local, base_period_end_datetime_local, reporting_period_start_datetime_local, reporting_period_end_datetime_local, period_type, language)
