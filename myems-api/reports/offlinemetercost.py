"""
Offline Meter Cost Report API

This module provides REST API endpoints for generating offline meter cost reports.
It analyzes energy costs associated with offline meter readings, providing insights
into cost optimization opportunities and financial performance metrics.

Key Features:
- Offline meter energy cost analysis
...
Report Components:
- Offline meter energy cost summary
...
import pdfexporters.offlinemetercost
import docxexporters.offlinemetercost
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:offlinemetercost:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        try:
            ...
            try:
                ...
                query = (" SELECT start_datetime_utc, actual_value "
                         ...
                         " AND start_datetime_utc < %s "
                         " ORDER BY start_datetime_utc ")
                cursor_billing.execute(query, (offline_meter['id'],
                                               reporting_start_datetime_utc, reporting_end_datetime_utc))
                rows_offline_meter_hourly = cursor_billing.fetchall()
        ...
        if not is_quick_mode:
            ...
            if is_export_pdf:
                result['pdf_bytes_base64'] = \
                    pdfexporters.offlinemetercost.export(result,
                                                         offline_meter['name'],
                                                         base_period_start_datetime_local,
                                                         base_period_end_datetime_local,
                                                         reporting_period_start_datetime_local,
            ...
            if is_export_docx:
                result['docx_bytes_base64'] = \
                    docxexporters.offlinemetercost.export(result,
                                                          offline_meter['name'],
                                                          base_period_start_datetime_local,
                                                          base_period_end_datetime_local,
                                                          reporting_period_start_datetime_local,
