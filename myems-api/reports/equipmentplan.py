"""
Equipment Plan Report API

This module provides REST API endpoints for generating equipment plan reports.
It analyzes equipment planning data and performance against planned targets,
providing insights into plan execution and optimization opportunities.

Key Features:
- Equipment plan analysis
...
import pdfexporters.equipmentplan
import docxexporters.equipmentplan
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:equipmentplan:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        try:
            ...
            # Connect to energy plan database
            ...
            try:
                ...
                # --- Original business logic starts here ---
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.equipmentplan.export(
                    ...
                    language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.equipmentplan.export(
                    result,
                    equipment['name'],
                    base_period_start_datetime_local,
                    base_period_end_datetime_local,
                    ...
                    language)
            if is_export_docx:
                result['docx_bytes_base64'] = docxexporters.equipmentplan.export(
                    result,
                    equipment['name'],
                    base_period_start_datetime_local,
                    base_period_end_datetime_local,
