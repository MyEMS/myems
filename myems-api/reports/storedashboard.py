...
Report Components:
- Store overview KPIs (total stores, active meters, alerts)
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:storedashboard:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        try:
            ...
            total_area = sum([float(store['area']) for store in store_list])
            store_ids_list = [store['id'] for store in store_list]
            store_ids_tuple = tuple(store_ids_list)
