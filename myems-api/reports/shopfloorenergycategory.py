"""
Shop Floor Energy Category Report API

This module provides REST API endpoints for generating shop floor energy category reports.
It analyzes energy consumption by different energy categories for shop floors,
providing insights into energy usage patterns and category-specific optimizations.

Key Features:
- Shop floor energy consumption by category analysis
...
Report Components:
- Shop floor energy consumption by category summary
...
import pdfexporters.shopfloorenergycategory
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:shopfloorenergycategory:' + hashlib.sha256(
        ...
        try:
            ...
            try:
                ...
                cursor_system.execute(" SELECT swc.id "
                                      ...
                                      " WHERE sp.id = %s AND sp.id = swc.shopfloor_id ", (shopfloor['id'], ))
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.shopfloorenergycategory.export(result,
                                                              ...
                                                              language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.shopfloorenergycategory.export(result,
                                                              shopfloor['name'],
                                                              base_period_start_datetime_local,
                                                              base_period_end_datetime_local,
                                                              reporting_period_start_datetime_local,
