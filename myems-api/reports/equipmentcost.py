"""
Equipment Cost Report API

This module provides REST API endpoints for generating equipment cost reports.
It analyzes energy costs associated with equipment operations, providing insights
into cost optimization opportunities and financial performance metrics.

Key Features:
- Equipment energy cost analysis
...
import pdfexporters.equipmentcost
import docxexporters.equipmentcost
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:equipmentcost:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        try:
            ...
            try:
                ...
                cursor_billing.execute(" SELECT DISTINCT(energy_category_id) "
                                       ...
                                       "     AND start_datetime_utc < %s ",
                                       (equipment['id'], base_start_datetime_utc,
                                        base_end_datetime_utc))
                    ...
                    for energy_category_id in energy_category_set:
                        ...
                        for row_equipment_periodically in rows_equipment_periodically:
                            current_datetime_local = (
                                    row_equipment_periodically[0].replace(tzinfo=timezone.utc) +
                                    timedelta(minutes=timezone_offset))
        ...
        if energy_category_set is not None and len(energy_category_set) > 0:
            for energy_category_id in energy_category_set:
                ...
                result['reporting_period']['deeps'].append(
                    reporting[energy_category_id]['deep'])
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.equipmentcost.export(
                    result,
                    equipment['name'],
                    ...
                    language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.equipmentcost.export(
                    result,
                    equipment['name'],
                    base_period_start_datetime_local,
                    base_period_end_datetime_local,
            ...
            if is_export_docx:
                try:
                    result['docx_bytes_base64'] = \
                        docxexporters.equipmentcost.export(
                            result,
                            equipment['name'],
                            base_period_start_datetime_local,
                            base_period_end_datetime_local,
