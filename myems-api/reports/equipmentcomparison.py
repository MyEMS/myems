...
import pdfexporters.equipmentcomparison
import docxexporters.equipmentcomparison
...
class Reporting:
    ...
    def on_get(req, resp):
        ...
        # determine whether to export Excel and/or PDF
        is_export_excel = False
        if (export_excel is not None
                and len(str.strip(export_excel)) > 0
                and str.lower(str.strip(export_excel)) in ('true', 't', 'on', 'yes', 'y')):
        ...
        if (export_docx is not None
                and len(str.strip(export_docx)) > 0
                and str.lower(str.strip(export_docx)) in ('true', 't', 'on', 'yes', 'y')):
            is_export_docx = True
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_params = {
                    "equipmentid1": equipment_id1,
                    "equipmentuuid1": equipment_uuid1,
                    "equipmentid2": equipment_id2,
                    "equipmentuuid2": equipment_uuid2,
                ...
                cache_key = 'report:equipmentcomparison:' + \
        ...
        try:
            ...
            try:
                ...
                ##################################################################################
                # Step 3: query equipment input category hourly data
                # (pre-aggregated by background service)
                ##################################################################################
            ...
            if is_export_excel:
                try:
                    result['excel_bytes_base64'] = \
                        excelexporters.equipmentcomparison.export(
                            result,
                            equipment1["name"],
                            equipment2["name"],
            ...
            if is_export_pdf:
                try:
                    result['pdf_bytes_base64'] = \
                        pdfexporters.equipmentcomparison.export(
                            result,
                            equipment1["name"],
                            equipment2["name"],
                            energy_category["name"],
            ...
            if is_export_docx:
                try:
                    result['docx_bytes_base64'] = \
                        docxexporters.equipmentcomparison.export(
                            result,
                            equipment1["name"],
                            equipment2["name"],
                            energy_category["name"],
