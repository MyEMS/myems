"""
Store Statistics Report API

This module provides REST API endpoints for generating store statistics reports.
It analyzes statistical data and performance metrics for stores to provide
comprehensive insights into store performance and operational patterns.
...
import pdfexporters.storestatistics
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:storestatistics:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.storestatistics.export(result, store['name'], base_period_start_datetime_local, base_period_end_datetime_local, reporting_period_start_datetime_local, reporting_period_end_datetime_local, period_type, language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.storestatistics.export(result, store['name'], base_period_start_datetime_local, base_period_end_datetime_local, reporting_period_start_datetime_local, reporting_period_end_datetime_local, period_type, language)
