...
import pdfexporters.tenantsaving
import docxexporters.tenantsaving
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:tenantsaving:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
                ...
                if energy_category_set is not None and len(energy_category_set) > 0:
                    for energy_category_id in energy_category_set:
                        ...
                        for row_tenant_periodically in rows_tenant_periodically:
                            ...
                            baseline_value = Decimal(0.0) if row_tenant_periodically[1] is None \
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.tenantsaving.export(result,
                                                                                  ...
                                                                                  language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.tenantsaving.export(result,
                                                                                     tenant['name'],
                                                                                     base_period_start_datetime_local,
                                                                                     base_period_end_datetime_local,
                                                                                     reporting_period_start_datetime_local,
                                                                                     ...
                                                                                     language)
            if is_export_docx:
                result['docx_bytes_base64'] = docxexporters.tenantsaving.export(result,
                                                                                tenant['name'],
                                                                                base_period_start_datetime_local,
                                                                                base_period_end_datetime_local,
                                                                                reporting_period_start_datetime_local,
