"""
Shop Floor Cost Report API

This module provides REST API endpoints for generating shop floor cost reports.
It analyzes energy costs associated with shop floor energy consumption, providing insights
into cost optimization opportunities and financial performance metrics.

Key Features:
- Shop floor energy cost analysis
...
Report Components:
- Shop floor energy cost summary
...
import pdfexporters.shopfloorcost
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:shopfloorcost:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.shopfloorcost.export(result,
                                                                               ...
                                                                               language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.shopfloorcost.export(result,
                                                                               shopfloor['name'],
                                                                               base_period_start_datetime_local,
                                                                               base_period_end_datetime_local,
                                                                               reporting_period_start_datetime_local,
