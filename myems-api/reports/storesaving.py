"""
Store Saving Report API

This module provides REST API endpoints for generating store saving reports.
It analyzes energy savings achieved through store optimization and efficiency
improvements, providing insights into saving opportunities and ROI calculations.

Key Features:
- Store energy saving analysis
...
import pdfexporters.storesaving
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:storesaving:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
                ...
                if energy_category_set is not None and len(energy_category_set) > 0:
                    for energy_category_id in energy_category_set:
                        ...
                        for row_store_periodically in rows_store_periodically:
                            ...
                            baseline_value = Decimal(0.0) if row_store_periodically[1] is None \
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.storesaving.export(result, store['name'], base_period_start_datetime_local, base_period_end_datetime_local, reporting_period_start_datetime_local, reporting_period_end_datetime_local, period_type, language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.storesaving.export(result, store['name'], base_period_start_datetime_local, base_period_end_datetime_local, reporting_period_start_datetime_local, reporting_period_end_datetime_local, period_type, language)
