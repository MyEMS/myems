"""
Combined Equipment Plan Report API

This module provides REST API endpoints for generating combined equipment plan reports.
It analyzes combined equipment planning data and performance against planned targets,
providing insights into plan execution and optimization opportunities.

Key Features:
- Combined equipment plan analysis
...
Report Components:
- Combined equipment plan summary
...
import docxexporters.combinedequipmentplan
import pdfexporters.combinedequipmentplan
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:combinedequipmentplan:' + \
        ...
        try:
            ...
            if cnx_historical is not None:
                try:
                    ...
                    logger.warning(f"Ignored exception: {e}")
            if cursor_energy_plan is not None:
                try:
                    cursor_energy_plan.close()
                except Exception as e:
                    logger.warning(f"Ignored exception: {e}")
            if cnx_energy_plan is not None:
                try:
            ...
            if is_export_docx:
                try:
                    result['docx_bytes_base64'] = \
                        docxexporters.combinedequipmentplan.export(result,
                                                                   combined_equipment['name'],
                                                                   base_period_start_datetime_local,
                                                                   base_period_end_datetime_local,
                                                                   reporting_period_start_datetime_local,
            ...
            if is_export_pdf:
                try:
                    result['pdf_bytes_base64'] = \
                        pdfexporters.combinedequipmentplan.export(result,
                                                                    combined_equipment['name'],
                                                                    base_period_start_datetime_local,
                                                                    base_period_end_datetime_local,
                                                                    reporting_period_start_datetime_local,
