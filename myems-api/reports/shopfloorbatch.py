...
import pdfexporters.shopfloorbatch
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:shopfloorbatch:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        result = {'shopfloors': shopfloor_list, 'energycategories': energy_category_list, 'excel_bytes_base64': None, 'pdf_bytes_base64': None}
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.shopfloorbatch.export(result,
                                                                                    ...
                                                                                    language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.shopfloorbatch.export(result,
                                                                                space_name,
                                                                                reporting_period_start_datetime_local,
                                                                                reporting_period_end_datetime_local,
                                                                                language)
