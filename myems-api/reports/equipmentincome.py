...
import pdfexporters.equipmentincome
import docxexporters.equipmentincome
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:equipmentincome:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.equipmentincome.export(
                    result,
                    equipment['name'],
                    ...
                    language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.equipmentincome.export(
                    result,
                    equipment['name'],
                    base_period_start_datetime_local,
                    base_period_end_datetime_local,
            ...
            if is_export_docx:
                try:
                    result['docx_bytes_base64'] = \
                        docxexporters.equipmentincome.export(
                            result,
                            equipment['name'],
                            base_period_start_datetime_local,
                            base_period_end_datetime_local,
