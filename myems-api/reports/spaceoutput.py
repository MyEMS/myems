"""
Space Output Report API

This module provides REST API endpoints for generating space output reports.
It analyzes space output performance and production metrics to provide
...
import pdfexporters.spaceoutput
import docxexporters.spaceoutput
...
class Reporting:
    ...
    def on_get(req, resp):
        ...
        result['reporting_period']['total_unit'] = energy_category_dict.get(
            list(energy_category_dict.keys())[0], {}).get('unit_of_measure', '') if energy_category_dict else ''
        ...
        result['child_space']['total_unit'] = energy_category_dict.get(
            list(energy_category_dict.keys())[0], {}).get('unit_of_measure', '') if energy_category_dict else ''
        if energy_category_set is not None and len(energy_category_set) > 0:
            for energy_category_id in energy_category_set:
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = \
                    excelexporters.spaceoutput.export(result,
                                                     space['name'],
            ...
            if is_export_pdf:
                result['pdf_bytes_base64'] = \
                    pdfexporters.spaceoutput.export(result,
                                                    space['name'],
                                                    base_period_start_datetime_local,
                                                    base_period_end_datetime_local,
                                                    reporting_period_start_datetime_local,
            ...
            if is_export_docx:
                result['docx_bytes_base64'] = \
                    docxexporters.spaceoutput.export(result,
                                                     space['name'],
                                                     base_period_start_datetime_local,
                                                     base_period_end_datetime_local,
                                                     reporting_period_start_datetime_local,
