...
import pdfexporters.tenantload
import docxexporters.tenantload
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:tenantload:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.tenantload.export(result,
                                                                                ...
                                                                                language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.tenantload.export(result,
                                                                                   tenant['name'],
                                                                                   base_period_start_datetime_local,
                                                                                   base_period_end_datetime_local,
                                                                                   reporting_period_start_datetime_local,
                                                                                   ...
                                                                                   language)
            if is_export_docx:
                result['docx_bytes_base64'] = docxexporters.tenantload.export(result,
                                                                              tenant['name'],
                                                                              base_period_start_datetime_local,
                                                                              base_period_end_datetime_local,
                                                                              reporting_period_start_datetime_local,
