"""
Shop Floor Energy Item Report API

This module provides REST API endpoints for generating shop floor energy item reports.
It analyzes energy consumption by specific energy items for shop floors,
providing detailed insights into energy usage patterns and item-specific optimizations.

Key Features:
- Shop floor energy consumption by energy item analysis
...
Report Components:
- Shop floor energy consumption by energy item summary
...
import pdfexporters.shopfloorenergyitem
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:shopfloorenergyitem:' + \
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.shopfloorenergyitem.export(result,
                                                          shopfloor['name'],
                                                          ...
                                                          language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.shopfloorenergyitem.export(result,
                                                          shopfloor['name'],
                                                          base_period_start_datetime_local,
                                                          base_period_end_datetime_local,
                                                          reporting_period_start_datetime_local,
