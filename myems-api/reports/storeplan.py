"""
Store Plan Report API

This module provides REST API endpoints for generating store plan reports.
It analyzes store planning data and performance against planned targets,
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:storeplan:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
                ...
                if energy_category_set is not None and len(energy_category_set) > 0:
                    for energy_category_id in energy_category_set:
                        ...
                        for row_store_periodically in rows_store_periodically:
                            ...
                            plan_value = Decimal(0.0) if row_store_periodically[1] is None \
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.storeplan.export(result, store['name'], base_period_start_datetime_local, base_period_end_datetime_local, reporting_period_start_datetime_local, reporting_period_end_datetime_local, period_type, language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.storeplan.export(result, store['name'], base_period_start_datetime_local, base_period_end_datetime_local, reporting_period_start_datetime_local, reporting_period_end_datetime_local, period_type, language)
