"""
Tenant Cost Report API

This module provides REST API endpoints for generating tenant cost reports.
It analyzes energy costs associated with tenant energy consumption, providing insights
into cost optimization opportunities and financial performance metrics.

Key Features:
- Tenant energy cost analysis
...
import pdfexporters.tenantcost
import docxexporters.tenantcost
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:tenantcost:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.tenantcost.export(result,
                                                                                ...
                                                                                language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.tenantcost.export(result,
                                                                                   tenant['name'],
                                                                                   base_period_start_datetime_local,
                                                                                   base_period_end_datetime_local,
                                                                                   reporting_period_start_datetime_local,
                                                                                   ...
                                                                                   language)
            if is_export_docx:
                result['docx_bytes_base64'] = docxexporters.tenantcost.export(result,
                                                                              tenant['name'],
                                                                              base_period_start_datetime_local,
                                                                              base_period_end_datetime_local,
                                                                              reporting_period_start_datetime_local,
