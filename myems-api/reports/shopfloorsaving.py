"""
Shop Floor Saving Report API

This module provides REST API endpoints for generating shop floor saving reports.
It analyzes energy savings achieved through shop floor optimization and efficiency
improvements, providing insights into saving opportunities and ROI calculations.

Key Features:
- Shop floor energy saving analysis
...
import pdfexporters.shopfloorsaving
    ...
    def on_get(req, resp):
        ...
        if config.redis.get('is_enabled'):
            try:
                ...
                cache_key = 'report:shopfloorsaving:' + hashlib.sha256(cache_params_json.encode('utf-8')).hexdigest()
        ...
        if not is_quick_mode:
            if is_export_excel:
                result['excel_bytes_base64'] = excelexporters.shopfloorsaving.export(result,
                                                                                 ...
                                                                                 language)
            if is_export_pdf:
                result['pdf_bytes_base64'] = pdfexporters.shopfloorsaving.export(result,
                                                                                 shopfloor['name'],
                                                                                 base_period_start_datetime_local,
                                                                                 base_period_end_datetime_local,
                                                                                 reporting_period_start_datetime_local,
