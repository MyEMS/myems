...
class Reporting:
    ...
    # Step 7: query the points of generator
    ...
    def on_get(req, resp, id_):
        ...
        ################################################################################################################
        # Step 6: query the points of generator
        ################################################################################################################
        ...
        cursor_system.execute(" SELECT id, name, uuid, "
                              ...
                              " ORDER BY id ",
                              (microgrid_id,))
        rows_generator = cursor_system.fetchall()
        if rows_generator is not None and len(rows_generator) > 0:
            for row in rows_generator:
                current_generator = dict()
                current_generator['id'] = row[0]
                current_generator['name'] = row[1]
                current_generator['uuid'] = row[2]
                current_generator['power_point'] = (latest_value_dict.get(row[3], None),
