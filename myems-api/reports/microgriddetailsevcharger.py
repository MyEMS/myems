...
class Reporting:
    ...
    # Step 6: query the points of EVChargers
    ...
    def on_get(req, resp, id_):
        ...
        ################################################################################################################
        # Step 6: query the points of associated EVChargers
        ################################################################################################################
        ...
        cursor_system.execute(" SELECT id, name, uuid "
                              ...
                              " ORDER BY id ",
                              (microgrid_id,))
        rows_evchargers = cursor_system.fetchall()
        if rows_evchargers is not None and len(rows_evchargers) > 0:
            for row in rows_evchargers:
                current_evcharger = dict()
                current_evcharger['id'] = row[0]
                current_evcharger['name'] = row[1]
                current_evcharger['uuid'] = row[2]
                current_evcharger['points'] = list()
                evcharger_list.append(current_evcharger)
        print(evcharger_list)
        for index, evcharger in enumerate(evcharger_list):
            cursor_system.execute(" SELECT p.id "
                                  " FROM tbl_microgrids_evchargers_points bp, tbl_points p "
                                  " WHERE bp.evcharger_id = %s AND bp.point_id = p.id "
                                  " ORDER BY bp.id ",
                                  (evcharger['id'],))
            ...
            if rows_points is not None and len(rows_points) > 0:
                ...
                evcharger_list[index]['points'] = point_list
        ...
        ################################################################################################################
        resp.text = json.dumps(evcharger_list)
