...
class Reporting:
    ...
    # Step 7: query the points of evcharger
    ...
    def on_get(req, resp, id_):
        ...
        ################################################################################################################
        # Step 6: query the points of evcharger
        ################################################################################################################
        ...
        cursor_system.execute(" SELECT id, name, uuid, "
                              ...
                              " ORDER BY id ",
                              (microgrid_id,))
        rows_evcharger = cursor_system.fetchall()
        if rows_evcharger is not None and len(rows_evcharger) > 0:
            for row in rows_evcharger:
                current_evcharger = dict()
                current_evcharger['id'] = row[0]
                current_evcharger['name'] = row[1]
                current_evcharger['uuid'] = row[2]
                current_evcharger['power_point'] = (latest_value_dict.get(row[3], None),
