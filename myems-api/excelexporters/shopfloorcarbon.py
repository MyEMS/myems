"""
Shop Floor Carbon Excel Exporter

This module provides functionality to export shop floor carbon emissions data to Excel format.
It generates comprehensive reports showing carbon dioxide emissions for shop floors
...
def generate_excel(report,
    ...
    if "toppeaks" not in reporting_period_data.keys() or \
        ...
        ws['D15'] = '{:.2%}'.format(round2(reporting_period_data['toppeaks'][electricity_index], 2) / wssum) \
        ...
        ws['D16'] = '{:.2%}'.format(round2(reporting_period_data['onpeaks'][electricity_index], 2) / wssum) \
            if wssum is not None and wssum != 0 else "0.00%"
        ...
        ws['D17'] = '{:.2%}'.format(round2(reporting_period_data['midpeaks'][electricity_index], 2) / wssum) \
            if wssum is not None and wssum != 0 else "0.00%"
        ...
        ws['D18'] = '{:.2%}'.format(round2(reporting_period_data['offpeaks'][electricity_index], 2) / wssum) \
            if wssum is not None and wssum != 0 else "0.00%"
    ...
    if "subtotals" not in reporting_period_data.keys() or \
        ...
        for i in range(0, ca_len):
            ...
            ws['D' + str(current_row_number)].alignment = c_c_alignment
            ws['D' + str(current_row_number)].border = f_border
            ws['D' + str(current_row_number)] = '{:.2%}'.format(round2(reporting_period_data['subtotals'][i], 2) /
