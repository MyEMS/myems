"""
Store Batch Excel Exporter

This module provides functionality to export store batch data to Excel format.
It generates comprehensive reports showing energy consumption data for multiple stores
within a specific time period.

Key Features:
- Multi-store energy consumption comparison
...
- Store names and associated spaces
...
def generate_excel(report, space_name, reporting_start_datetime_local, reporting_end_datetime_local, language):
    ...
    for i in range(0, len(report['stores'])):
        ...
        for j in range(0, ca_len):
            ...
            ws[col + str(current_row_number)] = round2(report['stores'][i]['values'][j], 2)
        ...
        ws[carbon_col + str(current_row_number)] = round2(report['stores'][i].get('carbon_emissions', 0.0), 2)
        ...
        ws[cost_col + str(current_row_number)] = round2(report['stores'][i].get('cost', 0.0), 2)
