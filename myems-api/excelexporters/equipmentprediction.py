...
def generate_excel(report,
    ...
    if "timestamps" not in reporting_period_data.keys() or \
        ...
        if not is_base_period_timestamp_exists_flag:
            ...
            current_row_number += ca_len * 6 + real_timestamps_len + 1 + 2
            table_start_row_number = current_row_number
    ...
    if 'parameters' not in report.keys() or \
        ...
        col_index += 1
        line.title = _('Parameters') + ' - ' + parameters_data['names'][0]
        labels = Reference(ws, min_col=labels_col, min_row=chart_start_row_number + 1,
                           max_row=chart_start_row_number + len(parameters_data['timestamps'][0]))
        line_data = Reference(ws, min_col=data_col, min_row=chart_start_row_number,
                              max_row=chart_start_row_number + len(parameters_data['timestamps'][0]))
        line.add_data(line_data, titles_from_data=True)
        line.set_categories(labels)
        line_data = line.series[0]
