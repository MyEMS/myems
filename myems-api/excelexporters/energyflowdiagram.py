...
def generate_excel(report, name, reporting_start_datetime_local, reporting_end_datetime_local, language):
    ...
    source = list()
    labels = list()
    color= ['rgba(25,202,173, 0.8)', 'rgba(140,199,181, 0.8)', 'rgba(160,238,225, 0.8)', 'rgba(190,231,233, 0.8)', 'rgba(190,237,199,0.8)',
    'rgba(109,91,7,0.8)', 'rgba(27,167,4,0.8)', 'rgba(108,234,12,0.8)', 'rgba(202,217,14,0.8)', 'rgba(70,6,12,0.8)',
    'rgba(246,10,15,0.8)', 'rgba(240,5,15,0.8)']
    ...
    for link in report['links']:
        ...
        node_value[nodes.index(link['source'])] = node_value[nodes.index(link['source'])] - link['value']
        node_value[nodes.index(link['target'])] = node_value[nodes.index(link['target'])] + link['value']
    ...
    ws.add_image(img, 'B' + str(current_row_number))  # 在单元格 A1 处插入图像
