"""
Meter Carbon DOCX Exporter

This module provides functionality to export meter carbon emissions data to DOCX format.
...
class MeterCarbonDOCXExporter:
    """
    Export meter carbon emissions data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Meter Carbon'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_consumption_summary(self, doc, has_next=True):
        ...
        for j in range(1, num_cols):
            cell_carbon = table.cell(1, j)
            cell_carbon.text = str(table_data[1][j])
            _style_table_cell(cell_carbon, font_size=8)
...
def export(report,
    ...
    """
    exporter = MeterCarbonDOCXExporter(language)
