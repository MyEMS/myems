"""
Offline Meter Cost DOCX Exporter

This module provides functionality to export offline meter cost data to DOCX format.
...
class OfflineMeterCostDOCXExporter:
    """
    Export offline meter cost data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Offline Meter Cost'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
...
def export(report,
    ...
    """
    exporter = OfflineMeterCostDOCXExporter(language)
