"""
Tenant Comparison DOCX Exporter

This module provides functionality to export tenant comparison analysis of two tenants data to DOCX format.
The content mirrors the PDF exporter (pdfexporters/tenantcomparison.py).

Page order:
- Cover page
- Combined analysis section (summary tables + comparison chart)
- Detailed data section (paginated table with difference column)
- Parameters sections (for both tenant1 and tenant2)
"""
...
class TenantComparisonDOCXExporter:
    """
    Export tenant comparison data to DOCX format.
    ...
    def export(self,
        ...
        logger.info("Starting DOCX generation for TenantComparison: %s vs %s",
                    tenant1_name, tenant2_name)
        ...
        logger.info("DOCX generation completed in %.2fs for TenantComparison", elapsed)
        return result
    ...
    def _add_cover_page(self, doc, tenant1_name, tenant2_name,
                        energy_category_name,
        ...
        run = title.add_run(_('Tenant Data') + ' - ' + _('Tenant Comparison'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_parameters_section(self, doc):
        ...
        for param_key, tenant_name in [('parameters1', self.tenant1_name),
            ...
            doc.add_page_break()
            self._add_heading_styled(doc, tenant_name + ' ' + _('Parameters'), level=1)
...
def export(report, tenant1_name, tenant2_name, energy_category_name,
    ...
    Provides a unified signature matching the PDF and Excel exporters.
    """
    exporter = TenantComparisonDOCXExporter(language)
