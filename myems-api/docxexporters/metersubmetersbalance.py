"""
Meter Submeters Balance DOCX Exporter

This module provides functionality to export meter sub-meters balance data to DOCX format.
...
class MeterSubmetersBalanceDOCXExporter:
    """
    Export meter sub-meters balance data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Meter Submeters Balance'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_balance_summary(self, doc, has_next=True):
        ...
        self._add_heading_styled(doc, self.name + ' - ' + _('Reporting Period'), level=1)
        ...
        row_labels = [_('Master Meter Consumption'), _('Submeters Consumption'),
                      _('Difference'), _('Percentage Difference')]
        for r_idx in range(len(table_data)):
            for j in range(num_cols):
                cell = table.cell(r_idx + 1, j)
                cell.text = table_data[r_idx][j]
                if j == 0:
                    _style_table_cell(cell, bold=True, font_size=9)
                else:
...
def export(report, name, reporting_start_datetime_local, reporting_end_datetime_local,
    ...
    """
    exporter = MeterSubmetersBalanceDOCXExporter(language)
