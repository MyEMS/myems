"""
Meter Energy DOCX Exporter

This module provides functionality to export meter energy data to DOCX format.
...
class MeterEnergyDOCXExporter:
    """
    Export meter energy data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Meter Energy'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
...
def export(report,
    ...
    """
    exporter = MeterEnergyDOCXExporter(language)
