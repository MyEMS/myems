"""
Virtual Meter Cost DOCX Exporter

This module provides functionality to export virtual meter cost data to DOCX format.
It generates comprehensive reports showing cost analysis for virtual meters
...
- Parameter data (if available) tables + line charts with filled area
"""
...
class VirtualMeterCostDOCXExporter:
    """
    Export virtual meter cost data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Virtual Meter Cost'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_consumption_summary(self, doc, has_next=True):
        """Add reporting period costs summary table."""
        _ = self._
        reporting_data = self.report['reporting_period']
        ...
        for j in range(1, num_cols):
            cell_cost = table.cell(1, j)
            cell_cost.text = cost_row[j]
            _style_table_cell(cell_cost, font_size=8)
    ...
    def _add_detailed_data_pages(self, doc, has_next=True):
        ...
        if not self.is_base_period_exists:
            ...
            for page in range(num_pages):
                ...
                ax_chart.set_title(_('Reporting Period Costs') + ' - ' + category_label,
            ...
            for page in range(num_pages):
                ...
                ax_chart.set_title(_('Base Period Costs') + ' / ' + _('Reporting Period Costs') + ' - ' + category_label,
                                   fontsize=10, weight='bold')
                ax_chart.legend(fontsize=7, loc='upper right')
                ax_chart.grid(True, alpha=0.3)
                chart_buf = self._fig_to_bytesio(fig, self.dpi)
...
def export(report,
           ...
           language):
    """
    Export virtual meter cost report data to DOCX and return base64 encoded string.
    This function maintains the same interface as the Excel exporter.
    """
    exporter = VirtualMeterCostDOCXExporter(language)
