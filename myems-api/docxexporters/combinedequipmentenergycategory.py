"""
Combined Equipment Energy Category DOCX Exporter

This module provides functionality to export combined equipment energy category data to DOCX format.
...
class CombinedEquipmentEnergyCategoryDOCXExporter:
    """
    Export combined equipment energy category data to DOCX format.
    ...
    def generate_docx(self,
        ...
        self._add_associated_detailed_data_section(doc)
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(f"{_('Combined Equipment')} - {_('Energy Analysis')}")       
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    # ---------- Associated Equipment Detailed Data ----------
    def _add_associated_detailed_data_section(self, doc):
        _ = self._
        report = self.report

        if 'associated_report_period_list' not in report.keys() or \
                report['associated_report_period_list'] is None or \
                len(report['associated_report_period_list']) == 0:
            return

        associated_list = report['associated_report_period_list']
        ...
        for associated_reporting_period_data in associated_list:
            times_raw = associated_reporting_period_data.get('timestamps', [])
            names = associated_reporting_period_data.get('names', [])
            units = associated_reporting_period_data.get('units', [])
            values = associated_reporting_period_data.get('values', [])
            subtotals = associated_reporting_period_data.get('subtotals', [])
            eq_name = associated_reporting_period_data.get('associated_equipment_name', '')

            ca_len = len(names)
            if ca_len == 0 or not times_raw or len(times_raw) == 0:
                continue
            reporting_times = times_raw[0] if isinstance(times_raw[0], list) else times_raw
            if not reporting_times or len(reporting_times) == 0:
                continue
...
def export(report,
    ...
    """
    exporter = CombinedEquipmentEnergyCategoryDOCXExporter(language)
