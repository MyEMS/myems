"""
Combined Equipment Comparison DOCX Exporter

This module provides functionality to export combined equipment comparison data to DOCX format.
It generates comprehensive reports showing energy consumption comparison between
two combined equipments with detailed breakdown including difference analysis.

Key Features:
- Combined Equipment energy comparison analysis (two combined equipments, single energy category)
- Consumption summary for both combined equipments
- Detailed data table with difference column
- Comparison line chart (both combined equipments overlaid)
...
class CombinedEquipmentComparisonDOCXExporter:
    """
    Export combined equipment comparison data to DOCX format.
    ...
    def export(self,
               report: Dict[str, Any],
               combined_equipment1_name: str,
               combined_equipment2_name: str,
        ...
        logger.info(f"Starting DOCX generation for CombinedEquipmentComparison: {combined_equipment1_name} vs {combined_equipment2_name}")

        docx_filename = self.generate_docx(
            report, combined_equipment1_name, combined_equipment2_name, energy_category_name,
        ...
        logger.info(f"DOCX generation completed in {elapsed:.2f}s for CombinedEquipmentComparison")
        return result
    ...
    def _add_cover_page(self, doc, combined_equipment1_name, combined_equipment2_name,
                        energy_category_name,
        ...
        run = title.add_run(f"{_('Combined Equipment')} - {_('Equipment Comparison')}")
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_combined_analysis_section(self, doc):
        ...
        summary_data = [
            ['', cat_name + ' (' + unit + ')'],
            [self.combined_equipment1_name, str(total1)],
            [self.combined_equipment2_name, str(total2)],
        ...
        if timestamps1 and len(timestamps1) > 0:
            ...
            ax.plot(xs, ys1, linewidth=1.5, color='#4472C4',
                    ...
                    label=self.combined_equipment1_name)
            ax.plot(xs, ys2, linewidth=1.5, color='#ED7D31',
                    marker='s', markersize=3,
                    markevery=marker_step,
                    label=self.combined_equipment2_name)
    ...
    def _add_detailed_data_section(self, doc):
        ...
        for page in range(num_pages):
            ...
            col_headers = [
                _('Datetime'),
                self.combined_equipment1_name + ' ' + cat_name + ' (' + unit + ')',
                self.combined_equipment2_name + ' ' + cat_name + ' (' + unit + ')',
                _('Difference')
            ]
    ...
    def _add_parameters_section(self, doc):
        """Add parameter data sections for both combined_equipment1 and combined_equipment2."""
        _ = self._

        for param_key, combined_equipment_name in [('parameters1', self.combined_equipment1_name),
                                                   ('parameters2', self.combined_equipment2_name)]:
            ...
            self._add_heading_styled(doc, combined_equipment_name + ' ' + _('Parameters'), level=1)
...
def export(report,
    ...
    Extracts combined_equipment1_name, combined_equipment2_name, and energy_category_name from report data.
    """
    combined_equipment1_name = report.get('combined_equipment1', {}).get('name', name)
    combined_equipment2_name = report.get('combined_equipment2', {}).get('name', '')
    energy_category_name = report.get('energy_category', {}).get('name', '')
    exporter = CombinedEquipmentComparisonDOCXExporter(language)
    return exporter.export(report, combined_equipment1_name, combined_equipment2_name, energy_category_name,
                           reporting_start_datetime_local,
                           reporting_end_datetime_local,
                           period_type, language)
