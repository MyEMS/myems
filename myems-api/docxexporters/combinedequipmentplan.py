"""
Combined Equipment Plan DOCX Exporter

This module provides functionality to export combined equipment plan data to DOCX format.
It generates comprehensive reports showing energy plan analysis for combined equipment
with detailed breakdown by energy categories and time periods.

Key Features:
- Combined Equipment energy plan analysis (plan - actual)
...
- Cover page with logo and report metadata (Combined Equipment Data - Plan Analysis)
...
class CombinedEquipmentPlanDOCXExporter:
    """
    Export combined equipment plan data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(f"{_('Combined Equipment')} - {_('Energy Plan Analysis')}")
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_detailed_data_charts(self, doc):
        ...
        for i in range(num_categories):
            ...
            unit_i = units[i] if (units and i < len(units)) else ''
            col_name = names[i] + _('Plan') + ((' (' + unit_i + ')') if unit_i else '')
            ...
            for i in range(num_categories):
                ...
                if r_ys:
                    ...
                    ax.plot(r_xs, r_ys, linewidth=1.2, color=color,
                            ...
                            label=_('Reporting Period Plan') + ' - ' + names[i])
                ...
                if b_ys:
                    ...
                    ax.plot(b_xs, b_ys, linewidth=1.2, color=color,
                            ...
                            label=_('Base Period Plan') + ' - ' + names[i])
...
def export(report,
    ...
    """
    exporter = CombinedEquipmentPlanDOCXExporter(language)
