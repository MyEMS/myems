"""
Virtual Meter Energy DOCX Exporter

This module provides functionality to export virtual meter energy data to DOCX format.
...
class VirtualMeterEnergyDOCXExporter:
    """
    Export virtual meter energy data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Virtual Meter Energy'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_consumption_summary(self, doc, has_next=True):
        ...
        for j in range(1, num_cols):
            cell_cons = table.cell(1, j)
            cell_cons.text = consumption_row[j]
...
def export(report,
           ...
           language):
    """
    Export virtual meter energy report data to DOCX and return base64 encoded string.
    This function maintains the same interface as the Excel exporter.
    """
    exporter = VirtualMeterEnergyDOCXExporter(language)
