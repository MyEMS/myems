"""
Combined Equipment Cost DOCX Exporter

This module provides functionality to export combined equipment cost data to DOCX format.
It generates comprehensive reports showing cost analysis for combined equipments
with detailed breakdown by energy categories and time periods.

Key Features:
- Combined Equipment cost analysis
- Base period vs reporting period comparison
- Cost breakdown by energy categories
- Cost proportion analysis with charts
- Detailed data with line charts
- Child combined equipments analysis
...
class CombinedEquipmentCostDOCXExporter:
    """
    Export combined equipment cost data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(f"{_('Combined Equipment')} - {_('Cost')}")
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_detailed_data_charts_section(self, doc):
        ...
        for i in range(num_categories):
            ...
            unit_i = units[i] if (units and i < len(units)) else ''
            col_name = names[i] + _('Costs') + ((' (' + unit_i + ')') if unit_i else '')
            ...
            for i in range(num_categories):
                ...
                if r_ys:
                    ...
                    ax.plot(r_xs, r_ys, linewidth=1.2, color=color,
                            ...
                            label=_('Reporting Period Costs') + ' - ' + names[i])
                ...
                if b_ys:
                    ...
                    ax.plot(b_xs, b_ys, linewidth=1.2, color=color,
                            ...
                            label=_('Base Period Costs') + ' - ' + names[i])
...
def export(report,
    ...
    """
    exporter = CombinedEquipmentCostDOCXExporter(language)
