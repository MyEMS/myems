"""
Offline Meter Prediction DOCX Exporter

This module provides functionality to export offline meter prediction data to DOCX format.
It generates comprehensive reports showing energy prediction analysis for offline meters
with detailed breakdown and time-series data.

Key Features:
- Offline meter energy prediction analysis
...
- Offline meter prediction summary table (Prediction + Increment Rate)
...
class OfflineMeterPredictionDOCXExporter:
    """
    Export offline meter prediction data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Offline Meter Prediction'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
...
def export(report,
    ...
    """
    exporter = OfflineMeterPredictionDOCXExporter(language)
