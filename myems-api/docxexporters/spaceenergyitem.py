"""
Space Energy Item DOCX Exporter

This module provides functionality to export space energy item data to DOCX format.
...
- Child spaces data table + per-item pie charts
...
class SpaceEnergyItemDOCXExporter:
    """
    Export space energy item data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Energy Item Analysis'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_child_spaces_section(self, doc):
        """Add child spaces data table with per-item pie charts."""
        _ = self._
...
def export(report,
    ...
    """
    exporter = SpaceEnergyItemDOCXExporter(language)
