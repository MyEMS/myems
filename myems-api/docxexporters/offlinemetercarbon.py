"""
Offline Meter Carbon DOCX Exporter

This module provides functionality to export offline meter carbon emissions data to DOCX format.
...
class OfflineMeterCarbonDOCXExporter:
    """
    Export offline meter carbon emissions data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Offline Meter Carbon'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_parameters_section(self, doc):
        ...
        for pi in valid_params:
            ...
            data_table.alignment = WD_TABLE_ALIGNMENT.CENTER
            p_elem = left_cell.paragraphs[0]._element
            p_elem.getparent().remove(p_elem)
...
def export(report,
    ...
    """
    exporter = OfflineMeterCarbonDOCXExporter(language)
