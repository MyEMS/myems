"""
Combined Equipment Efficiency DOCX Exporter

This module provides functionality to export combined equipment efficiency data to DOCX format.
...
class CombinedEquipmentEfficiencyDOCXExporter:
    """
    Export combined equipment efficiency data to DOCX format.
    ...
    def _add_cover_section(self, doc, name, period_type,
        ...
        run = title.add_run(f"{_('Combined Equipment')} - {_('Efficiency')}")
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_combined_analysis_section(self, doc):
        ...
        numerator_cumulations = reporting_data.get('numerator_cumulations', [])
        denominator_cumulations = reporting_data.get('denominator_cumulations', [])
...
def export(report,
    ...
    """
    exporter = CombinedEquipmentEfficiencyDOCXExporter(language)
