"""
Equipment Data Prediction DOCX Exporter

This module provides functionality to export equipment prediction data to DOCX format.
...
Key Features:
- Equipment Data energy prediction analysis
...
class EquipmentPredictionDOCXExporter:
    """
    Export equipment prediction data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(f"{_('Equipment Data')} - {_('Energy Prediction Analysis')}")
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
...
def export(report,
    ...
    """
    exporter = EquipmentPredictionDOCXExporter(language)
