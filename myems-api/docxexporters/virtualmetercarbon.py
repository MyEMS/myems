"""
Virtual Meter Carbon DOCX Exporter

This module provides functionality to export virtual meter carbon emissions data to DOCX format.
...
class VirtualMeterCarbonDOCXExporter:
    """
    Export virtual meter carbon emissions data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Virtual Meter Carbon'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
...
def export(report,
           ...
           language):
    """
    Export virtual meter carbon report data to DOCX and return base64 encoded string.
    This function maintains the same interface as the PDF exporter.
    """
    exporter = VirtualMeterCarbonDOCXExporter(language)
