"""
Meter Batch DOCX Exporter

This module provides functionality to export meter batch data to DOCX format.
...
The batch report is a wide matrix (rows = meters, columns = dates).
"""
...
class MeterBatchDOCXExporter:
    """
    Export meter batch data to DOCX format.
    ...
    def export(self,
        ...
        logger.info("Starting DOCX generation for meter batch")
    ...
    def _add_cover_page(self, doc, has_next=True):
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Batch Analysis'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_batch_data_table(self, doc):
        """Add the main batch data table showing all meters with their daily
        ...
        for page_idx in range(num_pages):
            ...
            if page_idx == 0:
                self._add_heading_styled(
                    doc, self.space_name + ' - ' + _('Meter Batch'), level=1
                )
...
def export(report,
    ...
    """
    exporter = MeterBatchDOCXExporter(language)
