"""
Equipment Plan DOCX Exporter

This module provides functionality to export equipment plan data to DOCX format.
...
Key Features:
- Equipment energy plan analysis (plan - actual)
...
- Cover page with logo and report metadata (Equipment Data - Plan Analysis)
...
class EquipmentPlanDOCXExporter:
    """
    Export equipment plan data to DOCX format.
    ...
    def _make_pie_chart(self, values, labels, title, colors=None):
        if not values or sum(abs(v or 0) for v in values) == 0:
            return None
        fig, ax = plt.subplots(figsize=(3.2, 2.6))
        if colors is None:
            colors = self.chart_colors[:len(labels)]
        filtered = [(l, v, c) for l, v, c in zip(labels, values, colors) if abs(v or 0) > 0]
        if not filtered:
            plt.close(fig)
            return None
        f_labels, f_values, f_colors = zip(*filtered)
        abs_values = [abs(v) for v in f_values]
        if len(f_labels) > 8:
            sorted_data = sorted(zip(f_labels, abs_values, f_colors), key=lambda x: x[1], reverse=True)
            top_data = sorted_data[:7]
            other_sum = sum(v for _, v, _ in sorted_data[7:])
            f_labels = [l for l, _, _ in top_data] + [self._('Others')]
            abs_values = [v for _, v, _ in top_data] + [other_sum]
            f_colors = [c for _, _, c in top_data] + ['#999999']
        ax.pie(abs_values, labels=f_labels, autopct='%1.1f%%', colors=f_colors, startangle=90)
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(f"{_('Equipment Data')} - {_('Energy Plan Analysis')}")
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_detailed_data_charts(self, doc):
        ...
        for i in range(num_categories):
            ...
            if not is_base:
                ...
                for page in range(num_pages):
                    ...
                    col_headers = [_('Datetime'), names[i] + ' ' + _('Reporting Period Plan') + value_unit]
                ...
                for page in range(num_pages):
                    ...
                    col_headers = [
                        ...
                        _('Base Period') + ' - ' + names[i] + ' ' + _('Base Period Plan') + value_unit,
                        _('Reporting Period') + ' - ' + _('Datetime'),
                        _('Reporting Period') + ' - ' + names[i] + ' ' + _('Reporting Period Plan') + value_unit
                    ]
                    table_data = [col_headers]
                    for t_idx in range(start_row, end_row):
        ...
        for i in range(num_categories):
            ...
            if not is_base:
                ...
                ax.set_title(_('Reporting Period Plan') + ' - ' + names[i] + value_unit,
                ...
                base_prefix = _('Base Period Plan')
                report_prefix = _('Reporting Period Plan')
                fig, ax = plt.subplots(figsize=(fig_w, fig_h))
...
def export(report,
    ...
    """
    exporter = EquipmentPlanDOCXExporter(language)
