"""
Virtual Meter Comparison DOCX Exporter

This module provides functionality to export virtual meter comparison data to DOCX format.
...
- Comparison line chart (both virtual meters overlaid)
- Multi-language support
- Base64 encoding for file transmission

The exported DOCX file includes:
- Cover page with logo and report metadata
- Consumption summary table (virtual_meter1, virtual_meter2)
- Line chart of reporting period consumption (virtual_meter1, virtual_meter2)
...
class VirtualMeterComparisonDOCXExporter:
    """
    Export virtual meter comparison data to DOCX format.
    ...
    def export(self,
        ...
        logger.info(f"Starting DOCX generation for VirtualMeterComparison: {name1} vs {name2}")
        ...
        logger.info(f"DOCX generation completed in {elapsed:.2f}s for VirtualMeterComparison")
        return result
    ...
    def generate_docx(self,
        ...
        has_summary = True
        has_linechart = bool(timestamps and len(timestamps) > 0)
        has_detailed = bool(timestamps and len(timestamps) > 0)
    ...
    def _add_cover_page(self, doc, name1, name2,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Virtual Meter Comparison'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_consumption_summary(self, doc, has_next=True):
        ...
        Two separate tables - VirtualMeter1 name/Consumption with EC1(unit1) and total1,
        VirtualMeter2 name/Consumption with EC2(unit2) and total2.
        Each virtual meter keeps its own energy category name and unit.
        """
        _ = self._
...
def export(report,
           ...
           language):
    """
    Export virtual meter comparison report data to DOCX and return base64 encoded string.
    ...
    """
    exporter = VirtualMeterComparisonDOCXExporter(language)
