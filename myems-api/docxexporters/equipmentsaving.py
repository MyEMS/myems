"""
Equipment Saving DOCX Exporter

This module provides functionality to export equipment saving data to DOCX format.
...
Key Features:
- Equipment Data energy saving analysis (baseline - actual)
...
- Cover page with logo and report metadata (Equipment Data - Saving Analysis)
...
class EquipmentSavingDOCXExporter:
    """
    Export equipment saving data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(f"{_('Equipment Data')} - {_('Energy Saving Analysis')}")
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_combined_analysis(self, doc):
        ...
        for i in range(ca_len):
            ...
            headers.append(names[i] + ' (' + _('Baseline') + ' - ' + _('Actual') + ')(' + unit_i + ')')
        headers.append(_('Ton of Standard Coal') + ' (' + _('Baseline') + ' - ' + _('Actual') + ') (TCE)')
        headers.append(_('Ton of Carbon Dioxide Emissions') + ' (' + _('Baseline') + ' - ' + _('Actual') + ') (TCO2E)')
        ...
        row_labels = [_('Saving'), _('Increment Rate')]
        ...
        for i in range(ca_len):
            col = i + 1
            cell_saving = table.cell(1, col)
            val = subtotals_saving[i] if (subtotals_saving and i < len(subtotals_saving)) else None
            cell_saving.text = str(round2(val, 2)) if val is not None else ''
            _style_table_cell(cell_saving, font_size=8)
        ...
        table.cell(2, tce_col).text = inc_tce_text
        ...
        table.cell(2, tco2e_col).text = inc_tco2e_text
    ...
    def _add_detailed_data_charts(self, doc):
        ...
        for i in range(num_categories):
            ...
            if not is_base:
                ...
                for page in range(num_pages):
                    ...
                    col_headers = [_('Datetime'), names[i] + ' ' + _('Reporting Period Saving') + value_unit]
                ...
                for page in range(num_pages):
                    ...
                    col_headers = [
                        ...
                        _('Base Period') + ' - ' + names[i] + ' ' + _('Base Period Saving') + value_unit,
                        _('Reporting Period') + ' - ' + _('Datetime'),
                        _('Reporting Period') + ' - ' + names[i] + ' ' + _('Reporting Period Saving') + value_unit
                    ]
                    table_data = [col_headers]
                    for t_idx in range(start_row, end_row):
        ...
        for i in range(num_categories):
            ...
            if not is_base:
                ...
                ax.set_title(_('Reporting Period Saving') + ' - ' + names[i] + value_unit,
                             fontsize=9, fontweight='bold')
                ax.grid(True, alpha=0.3)
                all_charts.append(self._fig_to_bytesio(fig, self.dpi))
            else:
                r_data = values_saving[i] if i < len(values_saving) else []
                base_data_arr = base_values_saving
                base_prefix = _('Base Period Saving')
                report_prefix = _('Reporting Period Saving')
                fig, ax = plt.subplots(figsize=(fig_w, fig_h))
...
def export(report,
    ...
    """
    exporter = EquipmentSavingDOCXExporter(language)
