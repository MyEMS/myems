"""
Virtual Meter Saving DOCX Exporter

This module provides functionality to export virtual meter saving data to DOCX format.
...
Key Features:
- Virtual Meter energy saving analysis (base - reporting)
...
- Cover page with logo and report metadata (Meter Data - Virtual Meter Saving)
- Reporting period saving summary table (Saving / Increment Rate)
...
class VirtualMeterSavingDOCXExporter:
    """
    Export virtual meter saving data to DOCX format.
    ...
    def generate_docx(self,
        ...
        self._add_cover_page(doc, name, period_type,
                             ...
                             has_next=(has_saving or has_detailed or has_params))

        self._add_saving_summary(doc, has_next=(has_detailed or has_params))
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Virtual Meter Saving'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_saving_summary(self, doc, has_next=True):
        """Add reporting period saving summary table (Saving vs Actual saving)."""
        _ = self._
        reporting_data = self.report['reporting_period']
        ...
        self._add_heading_styled(doc, self.name + ' ' + _('Saving'), level=1)
        ...
        for j in range(1, num_cols):
            cell_saving = table.cell(1, j)
            cell_saving.text = saving_row[j]
    ...
    def _add_detailed_data_pages(self, doc, has_next=True):
        ...
        if not self.is_base_period_exists:
            ...
            for page in range(num_pages):
                ...
                ax_chart.set_title(_('Reporting Period Saving') + ' - ' + category_label,
            ...
            for page in range(num_pages):
                ...
                ax_chart.set_title(_('Base Period Saving') + ' / ' + _('Reporting Period Saving') + ' - ' + category_label,
                                   fontsize=10, weight='bold')
                ax_chart.legend(fontsize=7, loc='upper right')
                ax_chart.grid(True, alpha=0.3)
                chart_buf = self._fig_to_bytesio(fig, self.dpi)
...
def export(report,
           ...
           language):
    """
    Export virtual meter saving report data to DOCX and return base64 encoded string.
    This function maintains the same interface as the Excel exporter.
    """
    exporter = VirtualMeterSavingDOCXExporter(language)
