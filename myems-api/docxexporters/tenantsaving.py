"""
Tenant Saving DOCX Exporter

This module provides functionality to export tenant energy saving data to DOCX format.
It generates comprehensive reports showing energy saving analysis for tenants
with detailed breakdown by energy categories and time periods.

Key Features:
- Tenant energy saving analysis (baseline - actual)
...
- Cover page with logo and report metadata (Tenant Data - Energy Saving Analysis)
...
class TenantSavingDOCXExporter:
    """
    Export tenant energy saving data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Tenant Data') + ' - ' + _('Energy Saving Analysis'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
        ...
        if not self.is_base_period_exists:
            ...
            for i in range(ca_len):
                ...
                for page in range(num_pages):
                    ...
                    ax_chart.set_title(_('Reporting Period Saving') + ' - ' + names[i] + ' (' + unit_i + ')',
            ...
            for i in range(ca_len):
                ...
                for page in range(num_pages):
                    ...
                    ax_chart.set_xticklabels(tick_labels, rotation=45, ha='right', fontsize=7)
                    ax_chart.set_title(
                        _('Base Period Saving') + ' / ' + _('Reporting Period Saving') + ' - ' +
                        names[i] + ' (' + unit_i + ')',
                        fontsize=10, weight='bold')
...
def export(report,
    ...
    """
    exporter = TenantSavingDOCXExporter(language)
