"""
Combined Equipment Carbon DOCX Exporter

This module provides functionality to export combined equipment carbon emissions data to DOCX format.
It generates comprehensive reports showing carbon dioxide emissions for combined equipments
...
class CombinedEquipmentCarbonDOCXExporter:
    """
    Export combined equipment carbon emissions data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(f"{_('Combined Equipment')} - {_('Carbon')}")
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
...
def export(report,
    ...
    """
    exporter = CombinedEquipmentCarbonDOCXExporter(language)
