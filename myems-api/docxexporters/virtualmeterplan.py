"""
Virtual Meter Plan DOCX Exporter

This module provides functionality to export virtual meter plan data to DOCX format.
...
Key Features:
- Virtual Meter energy plan analysis (plan - actual)
...
- Cover page with logo and report metadata (Meter Data - Virtual Meter Plan)
...
class VirtualMeterPlanDOCXExporter:
    """
    Export virtual meter plan data to DOCX format.
    ...
    def generate_docx(self,
        ...
        self._add_cover_page(doc, name, period_type,
                             ...
                             has_next=(has_plan or has_detailed or has_params))
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Virtual Meter Plan'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
...
def export(report,
           ...
           language):
    """
    Export virtual meter plan report data to DOCX and return base64 encoded string.
    This function maintains the same interface as the Excel exporter.
    """
    exporter = VirtualMeterPlanDOCXExporter(language)
