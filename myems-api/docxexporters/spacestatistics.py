"""
Space Statistics DOCX Exporter

This module provides functionality to export space statistics data to DOCX format.
...
- Per unit area derived statistics
...
- Statistics section (reporting period statistics + per unit area)
- Detailed data charts (line charts per energy category)
...
class SpaceStatisticsDOCXExporter:
    """
    Export space statistics data to DOCX format.
    ...
    def _add_statistics_section(self, doc):
        ...
        for r_idx in range(1, num_data_rows):
            for c_idx in range(num_cols):
                c = table.cell(r_idx, c_idx)
                c.text = str(table_data[r_idx][c_idx]) if table_data[r_idx][c_idx] is not None else ''
                is_green = (c_idx == 0)
                _style_table_cell(c, is_green=is_green, bold=is_green)

        doc.add_paragraph('')

        area = self.report.get('space', {}).get('area', None)
        per_area_title = self.name + ' ' + _('Per Unit Area')
        if area is not None:
            per_area_title += '    ' + str(area) + 'M\u00b2'

        self._add_heading_styled(doc, per_area_title, level=1)
        ...
        for i in range(ca_len):
            unit_i = units[i] if (units and i < len(units)) else ''
            per_area_headers.append(names[i] + ((' (' + unit_i + '/M\u00b2)') if unit_i else '/M\u00b2'))
        ...
        num_per_area_rows = len(per_area_table_data)
        per_area_table = doc.add_table(rows=num_per_area_rows, cols=num_cols)
        per_area_table.alignment = WD_TABLE_ALIGNMENT.CENTER

        for j, h in enumerate(per_area_headers):
            c = per_area_table.cell(0, j)
            c.text = h
            _style_table_cell(c, is_header=True, bold=True)

        for r_idx in range(1, num_per_area_rows):
            for c_idx in range(num_cols):
                c = per_area_table.cell(r_idx, c_idx)
                c.text = str(per_area_table_data[r_idx][c_idx]) if per_area_table_data[r_idx][c_idx] is not None else ''
                is_green = (c_idx == 0)
                _style_table_cell(c, is_green=is_green, bold=is_green)
    ...
    def _add_detailed_data_charts(self, doc):
        ...
        def _safe_sum(raw):
            try:
                vals = [(v if isinstance(v, (int, float)) else 0) if v is not None else 0
        ...
        for i in range(num_categories):
            ...
            r_total = subtotals[i] if (subtotals and i < len(subtotals)) else _safe_sum(r_data)
            b_data = base_values[i] if i < len(base_values) else []
            b_total = base_subtotals[i] if (base_subtotals and i < len(base_subtotals)) else _safe_sum(b_data)
...
def export(report,
    ...
    """
    exporter = SpaceStatisticsDOCXExporter(language)
