"""
Virtual Meter Batch DOCX Exporter

This module provides functionality to export virtual meter batch data to DOCX format.
...
The batch report is a wide matrix (rows = virtual meters, columns = dates).
"""
...
class VirtualMeterBatchDOCXExporter:
    """
    Export virtual meter batch data to DOCX format.
    ...
    def export(self,
        ...
        logger.info("Starting DOCX generation for virtual meter batch")
    ...
    def _add_cover_page(self, doc, has_next=True):
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Virtual Meter Batch Analysis'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
        ...
        for page_idx in range(num_pages):
            ...
            if page_idx == 0:
                title_text = (self.space_name
                              + ' - '
                              + _('Virtual Meter Batch Analysis'))
            ...
            for local_i, meter in enumerate(page_meters):
                ...
                c_name.text = str(meter.get('virtual_meter_name', ''))
                ...
                c_ec = table.cell(r_idx, 3)
                c_ec.text = str(meter.get('energy_category_name', ''))
                _style_table_cell(c_ec, bold=True, font_size=font_size)

                for d_idx, day in enumerate(date_list):
                    col = 4 + d_idx
                ...
                total_col = 4 + n_dates
                c_total = table.cell(r_idx, total_col)
                subtotal = meter.get('subtotal')
                c_total.text = str(round2(subtotal, 2)) if subtotal is not None else ''
                _style_table_cell(c_total, bold=True, font_size=font_size)
...
def export(report,
           ...
           language):
    """
    Export virtual meter batch report data to DOCX and return base64 encoded string.
    This function maintains the same interface as the Excel exporter.
    """
    exporter = VirtualMeterBatchDOCXExporter(language)
