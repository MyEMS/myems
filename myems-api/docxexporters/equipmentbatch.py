"""
Equipment Batch DOCX Exporter

This module provides functionality to export equipment batch data to DOCX format.
...
class EquipmentBatchDOCXExporter:
    """
    Export equipment batch data to DOCX format.
    ...
    def export(self,
        ...
        logger.info(f"Starting DOCX generation for Equipment Batch")
        ...
        logger.info(f"DOCX generation completed in {elapsed:.2f}s for Equipment Batch")
        return result
    ...
    def _add_cover_page(self, doc):
        ...
        run = title.add_run(f"{_('Equipment Data')} - {_('Batch Analysis')}")
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
...
def export(report,
    ...
    """
    exporter = EquipmentBatchDOCXExporter(language)
