"""
Offline Meter Saving DOCX Exporter

This module provides functionality to export offline meter saving data to DOCX format.
...
- Cover page with logo and report metadata (Meter Data - Offline Meter Saving)
...
class OfflineMeterSavingDOCXExporter:
    """
    Export offline meter saving data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Offline Meter Saving'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
...
def export(report,
    ...
    """
    exporter = OfflineMeterSavingDOCXExporter(language)
