"""
Meter Saving DOCX Exporter

This module provides functionality to export meter saving data to DOCX format.
...
- Cover page with logo and report metadata (Meter Data - Meter Saving)
...
class MeterSavingDOCXExporter:
    """
    Export meter saving data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Meter Saving'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_saving_summary(self, doc, has_next=True):
        """Add reporting period saving summary table."""
        _ = self._
        reporting_data = self.report['reporting_period']
...
def export(report,
    ...
    """
    exporter = MeterSavingDOCXExporter(language)
