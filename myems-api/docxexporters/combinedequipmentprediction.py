"""
Combined Equipment Prediction DOCX Exporter

This module provides functionality to export combined equipment prediction data to DOCX format.
It generates comprehensive reports showing energy prediction analysis for combined equipments
with detailed breakdown including time-of-use analysis and energy category proportions.

Key Features:
- Combined Equipment energy prediction analysis
...
class CombinedEquipmentPredictionDOCXExporter:
    """
    Export combined equipment prediction data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(f"{_('Combined Equipment')} - {_('Energy Prediction Analysis')}")
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
...
def export(report,
    ...
    """
    exporter = CombinedEquipmentPredictionDOCXExporter(language)
