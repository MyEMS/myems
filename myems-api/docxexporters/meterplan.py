"""
Meter Plan DOCX Exporter

This module provides functionality to export meter plan data to DOCX format.
It generates comprehensive reports showing energy plan analysis for meters
with detailed breakdown and time-series data.

Key Features:
- Meter energy plan analysis (plan - actual)
...
- Cover page with logo and report metadata (Meter Data - Meter Plan)
...
class MeterPlanDOCXExporter:
    """
    Export meter plan data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Meter Plan'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
...
def export(report,
    ...
    """
    exporter = MeterPlanDOCXExporter(language)
