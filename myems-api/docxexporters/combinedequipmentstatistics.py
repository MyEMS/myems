"""
Combined Equipment Statistics DOCX Exporter

This module provides functionality to export combined equipment statistics data to DOCX format.
It generates comprehensive reports showing statistical analysis for combined equipments
with detailed breakdown by energy categories and time periods.

Key Features:
- Combined Equipment statistical analysis (mean, median, min, max, stdev, variance)
...
- Statistics section (reporting period statistics)
...
class CombinedEquipmentStatisticsDOCXExporter:
    """
    Export combined equipment statistics data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(f"{_('Combined Equipment')} - {_('Statistics')}")
...
def export(report,
    ...
    """
    exporter = CombinedEquipmentStatisticsDOCXExporter(language)
