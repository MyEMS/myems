"""
Combined Equipment Energy Item DOCX Exporter

This module provides functionality to export combined equipment energy item data to DOCX format.
It generates comprehensive reports showing energy consumption breakdown by specific energy items
for combined equipments with detailed analysis and visualizations.

Key Features:
- Combined Equipment energy consumption by energy items
- Base period vs reporting period comparison
- Energy item proportion analysis by category group with pie charts
- Detailed data charts per energy item
- Child combined equipments data with charts
...
- Child combined equipments data table + per-item pie charts
...
class CombinedEquipmentEnergyItemDOCXExporter:
    """
    Export combined equipment energy item data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(f"{_('Combined Equipment')} - {_('Energy Item Analysis')}")
    ...
    def _add_child_combined_equipments_section(self, doc):
        ...
        reporting_data = self.report['reporting_period']
        names_array = child['child_combined_equipment_names_array']
        child_names = names_array[0]
        child_ids = child['child_combined_equipment_ids_array'][0]
        energy_item_names = child['energy_item_names']
        child_units = child.get('units', [])
        subtotals_array = child['subtotals_array']
        item_len = len(energy_item_names)
        combined_equipment_len = len(child_names)

        self._add_heading_styled(doc, self.name + ' ' + _('Child Combined Equipments Data'), level=1)

        num_cols = 2 + item_len
        table = doc.add_table(rows=combined_equipment_len + 1, cols=num_cols)
        table.alignment = WD_TABLE_ALIGNMENT.CENTER
        ...
        c1 = table.cell(0, 1)
        c1.text = _('Child Combined Equipment')
        ...
        for i in range(combined_equipment_len):
            r_idx = i + 1
        ...
        for j in range(item_len):
            values = []
            for s in range(combined_equipment_len):
...
def export(report,
    ...
    """
    exporter = CombinedEquipmentEnergyItemDOCXExporter(language)
