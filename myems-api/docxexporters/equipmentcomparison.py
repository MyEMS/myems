"""
Equipment Data Comparison DOCX Exporter

This module provides functionality to export equipment comparison data to DOCX format.
It generates comprehensive reports showing energy consumption comparison between
two equipment with detailed breakdown including difference analysis.

Key Features:
- Equipment Data energy comparison analysis (two equipment, single energy category)
- Consumption summary for both equipment
- Detailed data table with difference column
- Comparison line chart (both equipment overlaid)
...
class EquipmentComparisonDOCXExporter:
    """
    Export equipment comparison data to DOCX format.
    ...
    def export(self,
        ...
        logger.info(f"Starting DOCX generation for EquipmentComparison: {equipment1_name} vs {equipment2_name}")
        ...
        logger.info(f"DOCX generation completed in {elapsed:.2f}s for EquipmentComparison")
        return result
    ...
    def generate_docx(self,
        ...
        if "reporting_period1" not in report.keys() or \
            ...
            self._add_cover_page(doc, equipment1_name, equipment2_name,
    ...
    def _add_cover_page(self, doc, equipment1_name, equipment2_name,
                        energy_category_name,
        ...
        run = title.add_run(f"{_('Equipment Data')} - {_('Equipment Comparison')}")
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
        ...
        info_data = [
            [_('Equipment Data') + '1:', equipment1_name],
            [_('Equipment Data') + '2:', equipment2_name],
    ...
    def _add_combined_analysis_section(self, doc):
        ...
        self._add_heading_styled(doc, self.equipment1_name + ' & ' + self.equipment2_name + ' - ' +
                                 _('Reporting Period Consumption'), level=1)

        summary_data = [
            ['', cat_name + ' (' + unit + ')'],
            [self.equipment1_name, str(total1)],
            [self.equipment2_name, str(total2)],
        ...
        if timestamps1 and len(timestamps1) > 0:
            ...
            ax.plot(xs, ys1, linewidth=1.5, color='#4472C4',
                    ...
                    label=self.equipment1_name)
            ax.plot(xs, ys2, linewidth=1.5, color='#ED7D31',
                    marker='s', markersize=3,
                    markevery=marker_step,
                    label=self.equipment2_name)
    ...
    def _add_detailed_data_section(self, doc):
        ...
        for page in range(num_pages):
            ...
            col_headers = [
                _('Datetime'),
                self.equipment1_name + ' ' + cat_name + ' (' + unit + ')',
                self.equipment2_name + ' ' + cat_name + ' (' + unit + ')',
                _('Difference')
            ]
...
def export(report,
    ...
    Extracts equipment1_name, equipment2_name, and energy_category_name from report data.
    """
    equipment1_name = report.get('equipment1', {}).get('name', name)
    equipment2_name = report.get('equipment2', {}).get('name', '')
    energy_category_name = report.get('energy_category', {}).get('name', '')
    exporter = EquipmentComparisonDOCXExporter(language)
