"""
Equipment Energy Category DOCX Exporter

This module provides functionality to export equipment energy category data to DOCX format.
...
class EquipmentEnergyCategoryDOCXExporter:
    """
    Export equipment energy category data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(f"{_('Equipment Data')} - {_('Energy Category Analysis')}")
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
...
def export(report,
    ...
    """
    exporter = EquipmentEnergyCategoryDOCXExporter(language)
