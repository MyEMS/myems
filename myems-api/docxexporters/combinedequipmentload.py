"""
Combined Equipment Load DOCX Exporter

This module provides functionality to export combined equipment load data to DOCX format.
...
Key Features:
- Combined equipment load analysis (average load, maximum load, load factor)
...
class CombinedEquipmentLoadDOCXExporter:
    """
    Export combined equipment load data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(f"{_('Combined Equipment')} - {_('Load')}")
...
def export(report,
    ...
    """
    exporter = CombinedEquipmentLoadDOCXExporter(language)
