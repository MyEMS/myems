"""
Combined Equipment Saving DOCX Exporter

This module provides functionality to export combined equipment saving data to DOCX format.
It generates comprehensive reports showing energy saving analysis for combined equipments
with detailed breakdown by energy categories and time periods.

Key Features:
- Combined Equipment energy saving analysis (baseline - actual)
...
- Cover page with logo and report metadata (Combined Equipment Data - Saving Analysis)
...
class CombinedEquipmentSavingDOCXExporter:
    """
    Export combined equipment saving data to DOCX format.
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(f"{_('Combined Equipment')} - {_('Energy Saving Analysis')}")
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_combined_analysis(self, doc):
        ...
        row_labels = [_('Saving'), _('Per Unit Area'), _('Increment Rate')]
    ...
    def _add_detailed_data_charts(self, doc):
        ...
        base_times = None
        base_values_saving = None
        base_subtotals_saving = None
        if is_base:
        ...
        for i in range(num_categories):
            ...
            b_data = base_values_saving[i] if (base_values_saving and i < len(base_values_saving)) else []
            r_len = len(r_data)
            b_len = len(b_data) if is_base else 0

            unit_i = units[i] if (units and i < len(units)) else ''
            col_name = names[i] + _('Saving') + ((' (' + unit_i + ')') if unit_i else '')
            ...
            for i in range(num_categories):
                ...
                if r_ys:
                    ...
                    ax.plot(r_xs, r_ys, linewidth=1.2, color=color,
                            ...
                            label=_('Reporting Period Saving') + ' - ' + names[i])
                ...
                if b_ys:
                    ...
                    ax.plot(b_xs, b_ys, linewidth=1.2, color=color,
                            ...
                            label=_('Base Period Saving') + ' - ' + names[i])
...
def export(report,
    ...
    """
    exporter = CombinedEquipmentSavingDOCXExporter(language)
