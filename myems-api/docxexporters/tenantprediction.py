"""
Tenant Prediction DOCX Exporter

This module provides functionality to export tenant prediction data to DOCX format.
It generates comprehensive reports showing energy prediction analysis for tenants
with detailed breakdown by energy categories and time periods.

Key Features:
- Tenant energy prediction analysis
- Base period vs reporting period comparison
- Prediction breakdown by energy categories (TCE/TCO2E)
- Detailed data with line charts (actual vs prediction)
- Multi-language support
- Base64 encoding for file transmission

The exported DOCX file includes:
- Cover page with logo and report metadata (Tenant Data - Energy Prediction Analysis)
- Combined analysis page (prediction summary table + TCE/TCO2E breakdown tables with pie charts)
...
class TenantPredictionDOCXExporter:
    """
    Export tenant prediction data to DOCX format.
    ...
    def generate_docx(self,
        ...
        self._add_working_non_working_days_tables(doc)
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Tenant Data') + ' - ' + _('Energy Prediction Analysis'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_combined_analysis(self, doc):
        """Reporting Period Consumption title, summary table top + three table+pie columns below."""
        _ = self._
        ...
        # 2. Top summary table (5 rows: header + 4 data rows, cols = 1 + categories + 2 TCE/TCO2E)
        num_cols = ca_len + 3
        ...
        # ---- Column 1: TOU (table top + pie bottom) ----
        ...
        # ---- Column 2: TCE breakdown (table header 'TCE' + pie) ----
        ...
        th_tce0 = tce_tbl.cell(0, 0)
        th_tce0.text = _('Energy Category')
        _style_table_cell(th_tce0, is_header=True, bold=True, font_size=8)
        th_tce1 = tce_tbl.cell(0, 1)
        th_tce1.text = 'TCE'
        _style_table_cell(th_tce1, is_header=True, bold=True, font_size=8)
        for i in range(ca_len):
        ...
        # ---- Column 3: TCO2E breakdown (table header 'TCO2E' + pie) ----
        ...
        th_co2e0 = co2e_tbl.cell(0, 0)
        th_co2e0.text = _('Energy Category')
        _style_table_cell(th_co2e0, is_header=True, bold=True, font_size=8)
        th_co2e1 = co2e_tbl.cell(0, 1)
        th_co2e1.text = 'TCO2E'
        _style_table_cell(th_co2e1, is_header=True, bold=True, font_size=8)
        for i in range(ca_len):
    ...
    def _draw_working_days_table(self, doc, section_title, names, units,
        ...
        data_table = doc.add_table(rows=ca_len + 1, cols=3)
    ...
    def _add_working_non_working_days_tables(self, doc):
        ...
        # Base Period table (only if comparison exists)
        ...
        # Reporting Period table (always attempt)
        ...
        if (self.is_base_period_exists and r_names) or r_names:
            doc.add_page_break()
    ...
    def _add_detailed_data_charts(self, doc, has_next=True):
        """Add detailed data pages: table (up to 25 rows/page) on top, single big line chart below."""
        _ = self._
        ...
        if not self.is_base_period_exists:
            ...
            for page in range(num_pages):
                ...
                for j in range(ca_len):
                    unit_j = units[j] if (units and j < len(units)) else ''
                    col_headers.append(names[j] + ' (' + unit_j + ')')

                table_data = [col_headers]
                total_row = [_('Total')]
                for j in range(ca_len):
                    total_row.append(str(round2(subtotals[j], 2))
                                     if (subtotals and j < len(subtotals)) else '')
                ...
                tbl = table_cell.add_table(rows=len(table_data), cols=num_cols)
                tbl.alignment = WD_TABLE_ALIGNMENT.CENTER

                for r in range(len(table_data)):
                    for c in range(num_cols):
                        c_cell = tbl.cell(r, c)
                        c_cell.text = table_data[r][c]
                        if r == 0:
                            _style_table_cell(c_cell, is_header=True, bold=True, font_size=7)
                        elif r == len(table_data) - 1:
                            _style_table_cell(c_cell, bold=True, font_size=7)
                        else:
                            _style_table_cell(c_cell, font_size=6)

                fig, ax = plt.subplots(figsize=(8.5, 2.8))
                for j in range(ca_len):
                    color = self.chart_colors[j % len(self.chart_colors)]
                    raw = values[j] if j < len(values) else []
                    ys = [None] * page_rows
                    for t in range(page_rows):
                        g_idx = start_row + t
                        if j < len(values) and g_idx < len(raw) and raw[g_idx] is not None:
                            try:
                                ys[t] = float(raw[g_idx])
                            except (TypeError, ValueError):
                                pass
                    xs = [t for t, v in enumerate(ys) if v is not None]
                    yv = [v for v in ys if v is not None]
                    label_j = names[j]
                    if xs:
                        ax.plot(xs, yv, linewidth=1.2, color=color,
                                marker='o', markersize=3, markevery=marker_step,
                                label=label_j)

                step = max(1, page_rows // 10)
                ax.set_xticks(range(0, page_rows, step))
                ax.set_xticklabels(
                    [reporting_times[start_row + t]
                ...
                if ca_len:
                    ax.legend(fontsize=7, loc='upper right',
                              ncol=min(4, max(1, (ca_len + 1) // 2)))
                ax.grid(True, alpha=0.3)
                fig.tight_layout()
                chart_buf = self._fig_to_bytesio(fig, self.dpi)

                cp = chart_cell.paragraphs[0]
                cp.alignment = WD_ALIGN_PARAGRAPH.CENTER
                run_c = cp.add_run()
                run_c.add_picture(chart_buf, width=Inches(8.5))
            ...
            for page in range(num_pages):
                ...
                num_cols = base_ca_len + ca_len + 2
                col_headers = [_('Base Period') + ' - ' + _('Datetime')]
                for j in range(base_ca_len):
                    bu = base_units[j] if (base_units and j < len(base_units)) else ''
                    col_headers.append(_('Base Period') + ' - ' + base_names[j] + ' (' + bu + ')')
                col_headers.append(_('Reporting Period') + ' - ' + _('Datetime'))
                for j in range(ca_len):
                    uj = units[j] if (units and j < len(units)) else ''
                    col_headers.append(_('Reporting Period') + ' - ' + names[j] + ' (' + uj + ')')

                table_data = [col_headers]
                total_row = [_('Total')]
                for j in range(base_ca_len):
                    total_row.append(str(round2(base_subtotals[j], 2))
                                     if (base_subtotals and j < len(base_subtotals)) else '')
                ...
                for t_idx in range(start_row, end_row):
                    ...
                    for j in range(base_ca_len):
                        if (j < len(base_values) and t_idx < len(base_values[j])
                                and base_values[j][t_idx] is not None):
                            row.append(str(round2(base_values[j][t_idx], 2)))
                        else:
                            row.append('')
                    row.append(reporting_times[t_idx]
                               if t_idx < len(reporting_times) else '')
                    for j in range(ca_len):
                        if (j < len(values) and t_idx < len(values[j])
                                and values[j][t_idx] is not None):
                            row.append(str(round2(values[j][t_idx], 2)))
                        else:
                            row.append('')
                    table_data.append(row)
                ...
                fig, ax = plt.subplots(figsize=(8.5, 2.8))
                total_cols = max(base_ca_len, ca_len)
                for j in range(total_cols):
                    color = self.chart_colors[j % len(self.chart_colors)]
                    # Reporting period (solid)
                    if j < ca_len:
                        raw = values[j] if j < len(values) else []
                        ys_r = [None] * page_rows
                        for t in range(page_rows):
                            g_idx = start_row + t
                            if (j < len(values) and g_idx < len(raw)
                                    and raw[g_idx] is not None):
                                try:
                                    ys_r[t] = float(raw[g_idx])
                                except (TypeError, ValueError):
                                    pass
                        xs_r = [t for t, v in enumerate(ys_r) if v is not None]
                        yv_r = [v for v in ys_r if v is not None]
                        if xs_r:
                            name_r = names[j] if j < len(names) else _('Category') + str(j)
                            ax.plot(xs_r, yv_r, linewidth=1.2, color=color,
                                    marker='o', markersize=3, markevery=marker_step,
                                    label=_('Reporting Period') + ' - ' + name_r)
                    # Base period (dashed)
                    if j < base_ca_len:
                        braw = base_values[j] if j < len(base_values) else []
                        ys_b = [None] * page_rows
                        for t in range(page_rows):
                            g_idx = start_row + t
                            if (j < len(base_values) and g_idx < len(braw)
                                    and braw[g_idx] is not None):
                                try:
                                    ys_b[t] = float(braw[g_idx])
                                except (TypeError, ValueError):
                                    pass
                        xs_b = [t for t, v in enumerate(ys_b) if v is not None]
                        yv_b = [v for v in ys_b if v is not None]
                        if xs_b:
                            name_b = (base_names[j]
                                      if j < len(base_names)
                                      else _('Category') + str(j))
                            ax.plot(xs_b, yv_b, linewidth=1.2, color=color,
                                    linestyle='--', marker='s', markersize=3,
                                    markevery=marker_step,
                                    label=_('Base Period') + ' - ' + name_b)

                step = max(1, page_rows // 10)
                ax.set_xticks(range(0, page_rows, step))
                ax.set_xticklabels(
                    [reporting_times[start_row + t]
                     if (start_row + t) < len(reporting_times) else ''
                     for t in range(0, page_rows, step)],
                    rotation=45, ha='right', fontsize=7)
                ax.set_title(
...
def export(report,
    ...
    """
    exporter = TenantPredictionDOCXExporter(language)
