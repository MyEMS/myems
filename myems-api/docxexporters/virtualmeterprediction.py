"""
Virtual Meter Prediction DOCX Exporter

This module provides functionality to export virtual meter prediction data to DOCX format.
...
Key Features:
- Virtual Meter energy prediction analysis
...
- Cover page with logo and report metadata (Meter Data - Virtual Meter Prediction)
- Reporting period prediction summary table (Prediction / Increment Rate)
...
class VirtualMeterPredictionDOCXExporter:
    """
    Export virtual meter prediction data to DOCX format.
    ...
    def generate_docx(self,
        ...
        has_prediction_data = \
            ...
            "values" in report['reporting_period'].keys() and \
        ...
        self._add_cover_page(doc, name, period_type,
                             ...
                             has_next=(has_prediction or has_detailed or has_params))

        self._add_prediction_summary(doc, has_next=(has_detailed or has_params))
    ...
    def _add_cover_page(self, doc, name, period_type,
        ...
        run = title.add_run(_('Meter Data') + ' - ' + _('Virtual Meter Prediction'))
        run.font.size = Pt(24)
        run.font.bold = True
        run.font.name = 'Arial'
        r = run._element
    ...
    def _add_prediction_summary(self, doc, has_next=True):
        """Add reporting period prediction summary table (Prediction vs Actual saving)."""
        _ = self._
        reporting_data = self.report['reporting_period']
        ...
        self._add_heading_styled(doc, self.name + ' ' + _('Prediction'), level=1)
        ...
        row_labels = [_('Prediction'), _('Increment Rate')]
        ...
        for j in range(1, num_cols):
            cell_prediction = table.cell(1, j)
            cell_prediction.text = prediction_row[j]
            _style_table_cell(cell_prediction, font_size=8)
    ...
    def _add_detailed_data_pages(self, doc, has_next=True):
        ...
        if not self.is_base_period_exists:
            ...
            for page in range(num_pages):
                ...
                ax_chart.set_title(_('Reporting Period Prediction') + ' - ' + category_label,
            ...
            for page in range(num_pages):
                ...
                ax_chart.set_title(_('Base Period Prediction') + ' / ' + _('Reporting Period Prediction') + ' - ' + category_label,
                                   fontsize=10, weight='bold')
                ax_chart.legend(fontsize=7, loc='upper right')
                ax_chart.grid(True, alpha=0.3)
                chart_buf = self._fig_to_bytesio(fig, self.dpi)
...
def export(report,
           ...
           language):
    """
    Export virtual meter prediction report data to DOCX and return base64 encoded string.
    This function maintains the same interface as the Excel exporter.
    """
    exporter = VirtualMeterPredictionDOCXExporter(language)
