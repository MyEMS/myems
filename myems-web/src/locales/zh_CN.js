...
export default {
    translation: {
        ...
        ABOUT_MORE: 'MyEMS 开源能源管理系统适用于工厂、商业建筑、园区、工业生产能耗的全方位采集、监测和分析。',
        ...
        'Meter Count': '计量表数量',
        'Virtual Meter Count': '虚拟表数量',
        'Offline Meter Count': '离线表数量',
        'Virtual Meter List': '虚拟表列表',
        'Offline Meter List': '离线表列表',
        ...
        'Current Period': '当前周期',
        'Energy Consumption by Category': '按类别能耗分布',
        'Cost Distribution by Category': '按类别成本分布',
        'Monthly Energy & Cost Trends': '月度能耗与成本趋势',
        'Top 5 Energy Consuming Stores': '能耗最高的前5家门店',
        'Energy Category Details': '能源类别详情',
        'Store Performance Overview': '门店绩效概览',
        'YoY Change': '同比变化',
        'Per m²': '每平方米',
        'Usage Intensity': '使用强度',
        'Dashboard data loaded successfully': '仪表板数据加载成功',
        'Failed to load dashboard data': '加载仪表板数据失败',
        'Store Name': '门店名称',
        'Store Area': '门店面积',
        ...
        'last period': '上期',
        // Shopfloor Dashboard
        'Total Shopfloors': '车间总数',
        'Shopfloor Performance Overview': '车间绩效概览',
        'Shopfloor Name': '车间名称',
        'Shopfloor Area': '车间面积',
        // Dashboard
        ...
        'AI analysis initial prompt':
            '请仅依据下列报表 JSON 数据：概括主要结论与异常，对比基期与报告期（如有），并给出节能与运维优化建议。请用纯文本分段说明，不要使用 Markdown（不要用 #、**、```、列表符号等）。',
        'AI analysis loading': 'AI 正在分析报表数据…',
        'AI analysis follow up placeholder': '针对本报表继续提问（可选）',
        Send: '发送',
        'AI analysis copy': '复制',
        'AI analysis copied': '已复制到剪贴板',
        'AI analysis copy failed': '复制失败',
        'AI analysis max messages hint': '已达到本会话对话条数上限（最多 5 条）。',
        Datetime: '日期时间',
  ...
  Please copy it to the input box below.':
        ...
        'API.DEEPSEEK_INVALID_RESPONSE': 'AI 分析返回格式无效',
        'API.DEEPSEEK_NOT_CONFIGURED': '服务端未配置 AI 分析（DeepSeek）',
        'API.DEEPSEEK_REQUEST_FAILED': 'AI 分析请求失败',
        'API.DEEPSEEK_UPSTREAM_ERROR': 'AI 分析服务暂时不可用',
        'API.DEEPSEEK_TOO_MANY_MESSAGES': '本会话消息条数已超过上限（最多 5 条）',
        'API.INVALID_MESSAGE_CONTENT': '无效的消息内容',
        'API.INVALID_MESSAGE_ROLE': '无效的消息角色',
        'API.INVALID_MESSAGES': '无效的消息列表',
        'API.INVALID_REPORT_CONTEXT': '无效的报表上下文',
