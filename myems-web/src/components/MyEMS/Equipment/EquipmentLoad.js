...
const EquipmentLoad = ({ setRedirect, setRedirectUrl, t }) => {
  ...
  const handleExport = (e, type) => {
    ...
    if (type === 'excel') {
      ...
      const fileName = 'equipmentload.pdf';
      ...
      const fileName = 'equipmentload.docx';
  ...
  const buildSmartAnalysisContext = useCallback(() => {
    ...
    if (equipmentReportingData && typeof equipmentReportingData === 'object') {
      Object.keys(equipmentReportingData).forEach(k => {
        const arr = equipmentReportingData[k];
        reportingTrendValues[k] = Array.isArray(arr) ? arr.slice(0, 200) : arr;
      });
    }
    const baseTrendValues = {};
    if (equipmentBaseData && typeof equipmentBaseData === 'object') {
      Object.keys(equipmentBaseData).forEach(k => {
        const arr = equipmentBaseData[k];
    ...
    return {
      ...
      maximumLoadTrend: {
        ...
        optionLabels: equipmentReportingOptions
      },
