...
const EquipmentPrediction = ({ setRedirect, setRedirectUrl, t }) => {
  ...
  useEffect(() => {
    ...
    fetch(APIBaseURL + '/equipments', {
      ...
      .then(json => {
        if (isResponseOK) {
          let equipmentTree = json.map(item => ({
            value: item.id,
            label: item.name,
            children: []
          }));
          setCascaderOptions(equipmentTree);

          loadWithEquipmentTree(equipmentTree);
        } else {
  ...
  const loadWithEquipmentTree = equipmentTree => {
    if (uuid !== null && uuid) {
      let url =
        APIBaseURL +
        ...
        '&language=' +
        language;
      loadData(url, equipmentTree);
    } else {
      if (equipmentTree && equipmentTree.length > 0) {
        let equipmentID = equipmentTree[0].value;
        let url =
          APIBaseURL +
          '/reports/equipmentprediction?' +
          'equipmentid=' +
