...
const ShopfloorLoad = ({ setRedirect, setRedirectUrl, t }) => {
    ...
    fetch(APIBaseURL + '/spaces/' + selectedSpaceID + '/shopfloors', {
      ...
      .then(json => {
        if (isResponseOK) {
          ...
          handleAPIError(json, setRedirect, setRedirectUrl, t, toast);
          // re-enable submit button and hide spinner on API error
          setSubmitButtonDisabled(false);
      ...
      .catch(err => {
        console.log(err);
        // re-enable submit button and hide spinner on network/parse error
        setSubmitButtonDisabled(false);
        setSpinnerHidden(true);
      });
  };
  ...
  const handleExport = (e, type) => {
    ...
    if (type === 'excel' && excelBytesBase64) {
      ...
      const fileName = 'shopfloorload.pdf';
  ...
  const buildSmartAnalysisContext = useCallback(() => {
    ...
    if (shopfloorReportingData && typeof shopfloorReportingData === 'object') {
      Object.keys(shopfloorReportingData).forEach(k => {
        const arr = shopfloorReportingData[k];
        reportingTrendValues[k] = Array.isArray(arr) ? arr.slice(0, 200) : arr;
      });
    }
    const baseTrendValues = {};
    if (shopfloorBaseData && typeof shopfloorBaseData === 'object') {
      Object.keys(shopfloorBaseData).forEach(k => {
        const arr = shopfloorBaseData[k];
    ...
    return {
      ...
      maximumLoadTrend: {
        ...
        optionLabels: shopfloorReportingOptions
      },
            ...
            <Row form>
              ...
              <Col xs="auto">
                <FormGroup>
                  ...
                  <div>
                    <CustomInput type="checkbox" id="exportExcel" name="exportExcel"
                      label="Excel" bsSize="sm" inline
                      checked={exportExcel} onChange={({ target }) => setExportExcel(target.checked)} />
                    <CustomInput type="checkbox" id="exportPdf" name="exportPdf"
                      label="PDF" bsSize="sm" inline
                      checked={exportPdf} onChange={({ target }) => setExportPdf(target.checked)} />
                  </div>
                </FormGroup>
              </Col>
              <Col xs="auto">
              ...
              <Col xs="auto">
                ...
                <UncontrolledDropdown hidden={exportButtonHidden}>
                  <DropdownToggle size="sm" color="falcon-default" caret>{t('Export')}</DropdownToggle>
                  <DropdownMenu right>
                    {excelBytesBase64 ? (<DropdownItem onClick={e => handleExport(e, 'excel')}>EXCEL</DropdownItem>) : null}
                    {pdfBytesBase64 ? (<DropdownItem onClick={e => handleExport(e, 'pdf')}>PDF</DropdownItem>) : null}
                  </DropdownMenu>
                </UncontrolledDropdown>
              </Col>
