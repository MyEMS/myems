...
const Dashboard = ({setRedirect, setRedirectUrl, t}) => {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${APIBaseURL}/reports/shopfloordashboard?${params}`, {
        method: 'GET',
        headers: {
  ...
  // Get sorted shopfloors
  const getSortedShopfloors = () => {
    if (!sortConfig.key) {
      return allShopfloors.sort((a, b) => b.total_energy - a.total_energy);
    }

    const sortedShopfloors = [...allShopfloors];
    sortedShopfloors.sort((a, b) => {
      let aValue, bValue;
  ...
  // Prepare bar chart data for top shopfloors
  const prepareTopShopfloorsBarData = () => {
    const labels = topShopfloors.map(shopfloor => shopfloor.name);
    const data = topShopfloors.map(shopfloor => ({
      name: shopfloor.name,
      subtotal: shopfloor.total_energy,
      unit: 'kWh',
      increment_rate: null,
                    ...
                    {getSortedShopfloors()
                      ...
                      return (
                        <tr key={shopfloor.id}>
                          ...
                          <td>
                            <Link to={{ pathname: '/shopfloor/energycategory?uuid=' + shopfloor.uuid }} target="_blank">
