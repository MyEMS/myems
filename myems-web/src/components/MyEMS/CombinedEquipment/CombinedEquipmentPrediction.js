...
const CombinedEquipmentPrediction = ({ setRedirect, setRedirectUrl, t }) => {
  ...
  useEffect(() => {
    if (uuid !== null && uuid) {
      let url =
        APIBaseURL +
        '/reports/combinedequipmentprediction?' +
  ...
  const handleExport = e => {
    ...
    const fileName = 'combinedequipmentprediction.xlsx';
  ...
  const buildSmartAnalysisContext = useCallback(() => {
    ...
    return {
      reportType: 'combined_equipment_prediction',
      ...
      <div>
        <Breadcrumb>
          ...
          <BreadcrumbItem active onClick={() => window.location.reload()}>
            <Link to="/combinedequipment/prediction">{t('Prediction')}</Link>
          </BreadcrumbItem>
        </Breadcrumb>
      </div>
...
export default withTranslation()(withRedirect(CombinedEquipmentPrediction));