...
const CombinedEquipmentLoad = ({ setRedirect, setRedirectUrl, t }) => {
                ...
                json['associated_equipment']['energy_category_names'].forEach((currentValue, energyCategoryIndex) => {
                  if (json['associated_equipment']['sub_averages_array'][energyCategoryIndex][equipmentIndex] != null) {
                    associated_equipment_value['a' + 2 * energyCategoryIndex] =
                      json['associated_equipment']['sub_averages_array'][energyCategoryIndex][equipmentIndex];
                  } else {
                  ...
                  if (json['associated_equipment']['sub_maximums_array'][energyCategoryIndex][equipmentIndex] != null) {
                    associated_equipment_value['a' + (2 * energyCategoryIndex + 1)] =
                      json['associated_equipment']['sub_maximums_array'][energyCategoryIndex][equipmentIndex];
                  } else {
  ...
  const handleExport = (e, type) => {
    ...
    if (type === 'excel' && excelBytesBase64) {
      ...
      const fileName = 'combinedequipmentload.docx';
      ...
      const fileName = 'combinedequipmentload.pdf';
  ...
  const buildSmartAnalysisContext = useCallback(() => {
    ...
    if (combinedEquipmentReportingData && typeof combinedEquipmentReportingData === 'object') {
      Object.keys(combinedEquipmentReportingData).forEach(k => {
        const arr = combinedEquipmentReportingData[k];
        reportingTrendValues[k] = Array.isArray(arr) ? arr.slice(0, 200) : arr;
      });
    }
    const baseTrendValues = {};
    if (combinedEquipmentBaseData && typeof combinedEquipmentBaseData === 'object') {
      Object.keys(combinedEquipmentBaseData).forEach(k => {
        const arr = combinedEquipmentBaseData[k];
    ...
    return {
      reportType: 'combined_equipment_load',
      reportTitle: t('Load'),
      ...
      maximumLoadTrend: {
        ...
        optionLabels: combinedEquipmentReportingOptions
      },
