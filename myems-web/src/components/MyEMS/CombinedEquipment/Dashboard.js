...
import {Col, Row, Card, CardBody, BreadcrumbItem, Breadcrumb} from 'reactstrap';
...
const Dashboard = ({setRedirect, setRedirectUrl, t}) => {
  ...
  const [topCombinedEquipments, setTopCombinedEquipments] = useState([]);
  const [allCombinedEquipments, setAllCombinedEquipments] = useState([]);
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${APIBaseURL}/reports/combinedequipmentdashboard?${params}`, {
        method: 'GET',
        headers: {
      ...
      setTopCombinedEquipments(json.top_combined_equipments || []);
      setAllCombinedEquipments(json.combined_equipments || []);
  ...
  // Get sorted combined equipments
  const getSortedCombinedEquipments = () => {
    if (!sortConfig.key) {
      return allCombinedEquipments.sort((a, b) => b.total_energy - a.total_energy);
    }

    const sortedCombinedEquipments = [...allCombinedEquipments];
    sortedCombinedEquipments.sort((a, b) => {
      let aValue, bValue;
    ...
    return sortedCombinedEquipments;
  };
              ...
              <CardBody>
                ...
                <div className="table-responsive">
                  <table className="table table-hover">
                    ...
                    <tbody>
                    {getSortedCombinedEquipments()
                    .map((combinedEquipment) => {
                      return (
                        <tr key={combinedEquipment.id}>
                          ...
                          {energyData.energy_category_ids && energyData.energy_category_ids.map((ecId, index) => {
                            const categoryEnergy = combinedEquipment.energy_by_category && combinedEquipment.energy_by_category[ecId]
                              ? combinedEquipment.energy_by_category[ecId]
                              : 0;
                            return (
                          ...
                          <td className="text-right">
                            {combinedEquipment.total_cost !== null && combinedEquipment.total_cost !== undefined && combinedEquipment.total_cost > 0
                                ? combinedEquipment.total_cost.toFixed(2)
                                : '-'}
                          </td>
                          <td className="text-right">
                            {combinedEquipment.total_carbon !== null && combinedEquipment.total_carbon !== undefined && combinedEquipment.total_carbon > 0
                                ? combinedEquipment.total_carbon.toFixed(2)
                                : '-'}
                          </td>
                        </tr>
                      );
