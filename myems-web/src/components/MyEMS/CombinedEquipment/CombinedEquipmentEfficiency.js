...
const CombinedEquipmentEfficiency = ({ setRedirect, setRedirectUrl, t }) => {
  ...
  const handleSubmit = e => {
    ...
    fetch(
      APIBaseURL +
        '/reports/combinedequipmentefficiency?' +
        'combinedequipmentid=' +
        ...
        if (isResponseOK) {
          ...
          json['reporting_period_efficiency']['names'].forEach((currentValue, index) => {
            ...
            cardSummaryItem['numerator_cumulation'] =
              json['reporting_period_efficiency']['numerator_cumulations'][index];
            ...
            cardSummaryItem['denominator_cumulation'] =
              json['reporting_period_efficiency']['denominator_cumulations'][index];
            ...
            json['associated_equipment']['associated_equipment_names_array'][0].forEach(
              (currentEquipmentName, equipmentIndex) => {
                ...
                json['associated_equipment']['energy_category_names'].forEach((currentValue, energyCategoryIndex) => {
                  associated_equipment_value['a' + energyCategoryIndex] = json['associated_equipment'][
                    'subtotals_array'
                  ][energyCategoryIndex][equipmentIndex].toFixed(2);
                });
  ...
  const handleExport = (e, type) => {
    ...
    if (type === 'excel' && excelBytesBase64) {
      ...
      const fileName = 'combinedequipmentefficiency.docx';
      ...
      const fileName = 'combinedequipmentefficiency.pdf';
  ...
  const buildSmartAnalysisContext = useCallback(() => {
    ...
    return {
      reportType: 'combined_equipment_efficiency',
      ...
      combinedEquipmentEfficiencyIndicator,
      periodType,
      comparisonType,
      reportingPeriod: {
      ...
      <div style={{ visibility: resultDataHidden ? 'hidden' : 'visible', display: resultDataHidden ? 'none' : '' }}>
        {cardSummaryList.map(cardSummaryItem => (
          <div className="card-deck" key={cardSummaryItem['name']}>
            ...
            <CardSummary
              key={cardSummaryItem['numerator_name']}
              rate={cardSummaryItem['increment_rates_num']}
              title={t('Reporting Period Output CATEGORY UNIT', {
                CATEGORY: cardSummaryItem['name'] + '-' + cardSummaryItem['numerator_name'],
                UNIT: '(' + cardSummaryItem['numerator_unit'] + ')'
              })}
              color="success"
            >
              {cardSummaryItem['numerator_cumulation'] && (
            ...
            <CardSummary
              key={cardSummaryItem['denominator_name']}
              rate={cardSummaryItem['increment_rates_den']}
              title={t('Reporting Period Consumption CATEGORY UNIT', {
                CATEGORY: cardSummaryItem['name'] + '-' + cardSummaryItem['denominator_name'],
                UNIT: '(' + cardSummaryItem['denominator_unit'] + ')'
              })}
              color="success"
            >
              {cardSummaryItem['denominator_cumulation'] && (
        ...
        <div ref={chartContainerRef}>
          <MultiTrendChart
            reportingTitle={{
              ...
              NAME: combinedEquipmentBaseAndReportingNames,
