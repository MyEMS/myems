...
const CombinedEquipmentEnergyCategory = ({ setRedirect, setRedirectUrl, t }) => {
  ...
  useEffect(() => {
    if(uuid === null || !uuid) {
      ...
      fetch(APIBaseURL + '/reports/combinedequipmentenergycategory?' +
        ...
        if (isResponseOK) {
          console.log(json);
          setCombinedEquipmentList([{'value': json['combined_equipment']['id'], 'label': json['combined_equipment']['name']}]);
  ...
  return (
    <Fragment>
      <div>
        <Breadcrumb>
          <BreadcrumbItem>{t('Combined Equipment Data')}</BreadcrumbItem><BreadcrumbItem active onClick={ () => window.location.reload() }>
