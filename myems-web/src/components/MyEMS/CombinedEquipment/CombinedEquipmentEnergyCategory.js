...
const CombinedEquipmentEnergyCategory = ({ setRedirect, setRedirectUrl, t }) => {
  ...
  const handleExport = (e, type) => {
    ...
    if (type === 'excel' && excelBytesBase64) {
      ...
      const fileName = 'combinedequipmentenergycategory.pdf';
  ...
  const buildSmartAnalysisContext = useCallback(() => {
    ...
    const maxAssocCharts = 15;
    const associatedEquipmentCharts = [];
    for (
      let i = 0;
      i < Math.min(associatedEquipmentNamesOfAssociatedReportPeriodList.length, maxAssocCharts);
      i++
    ) {
      const dataObj = associatedEquipmentLineChartDataList[i];
      const values = {};
      ...
      associatedEquipmentCharts.push({
        name: associatedEquipmentNamesOfAssociatedReportPeriodList[i],
        labels: associatedEquipmentLineChartLabelsList[i],
        optionLabels: associatedEquipmentLineChartOptionsList[i],
        values
      });
    }
    const combinedLabel =
      combinedEquipmentList.find(c => String(c.value) === String(selectedCombinedEquipment))?.label ?? null;
    const spaceDisplayName = Array.isArray(selectedSpaceName)
      ? selectedSpaceName.join('/')
      : selectedSpaceName ?? null;
    return {
      reportType: 'combined_equipment_energy_category',
    ...
    combinedEquipmentBaseAndReportingUnits,
    associatedEquipmentNamesOfAssociatedReportPeriodList,
    associatedEquipmentLineChartLabelsList,
    associatedEquipmentLineChartDataList,
    associatedEquipmentLineChartOptionsList,
