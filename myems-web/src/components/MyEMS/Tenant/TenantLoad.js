...
const TenantLoad = ({ setRedirect, setRedirectUrl, t }) => {
  ...
  const handleExport = (e, type) => {
    ...
    if (type === 'excel' && excelBytesBase64) {
      ...
      const fileName = 'tenantload.docx';
  ...
  const buildSmartAnalysisContext = useCallback(() => {
    ...
    if (tenantReportingData && typeof tenantReportingData === 'object') {
      Object.keys(tenantReportingData).forEach(k => {
        const arr = tenantReportingData[k];
        reportingTrendValues[k] = Array.isArray(arr) ? arr.slice(0, 200) : arr;
      });
    }
    const baseTrendValues = {};
    if (tenantBaseData && typeof tenantBaseData === 'object') {
      Object.keys(tenantBaseData).forEach(k => {
        const arr = tenantBaseData[k];
    ...
    return {
      ...
      maximumLoadTrend: {
        ...
        optionLabels: tenantReportingOptions
      },
