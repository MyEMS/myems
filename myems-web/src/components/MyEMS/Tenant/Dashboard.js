...
const Dashboard = ({setRedirect, setRedirectUrl, t}) => {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${APIBaseURL}/reports/tenantdashboard?${params}`, {
        method: 'GET',
        headers: {
      ...
      setTopTenants(json.top_tenants || []);
  ...
  const getSortedTenants = () => {
    if (!sortConfig.key) {
      return allTenants.sort((a, b) => b.total_energy - a.total_energy);
    }
    ...
    sortedTenants.sort((a, b) => {
      ...
      switch (sortConfig.key) {
        case 'name':
          ...
          break;
        case 'tenant_type':
          aValue = a.tenant_type || '';
          bValue = b.tenant_type || '';
          break;
        case 'area':
  ...
  // Prepare bar chart data for top tenants
  const prepareTopTenantsBarData = () => {
    const labels = topTenants.map(tenant => tenant.name);
    const data = topTenants.map(tenant => ({
      name: tenant.name,
      subtotal: tenant.total_energy,
      unit: 'kWh',
      increment_rate: null,
  ...
  const topTenantsBarData = prepareTopTenantsBarData();
                ...
                <div className="table-responsive">
                  <table className="table table-hover">
                    ...
                    <tr>
                      ...
                      <th 
                        onClick={() => handleSort('tenant_type')}
                        style={{cursor: 'pointer'}}
                      >
                        {t('Tenant Type')}
                        <FontAwesomeIcon 
                          icon={sortConfig.key === 'tenant_type' ? (sortConfig.direction === 'asc' ? faSortUp : faSortDown) : faSort}
                          className="ml-1"
                        />
                      </th>
                      <th 
                      ...
                      return (
                        <tr key={tenant.id}>
                          ...
                          {energyData.energy_category_ids && energyData.energy_category_ids.map((ecId, index) => {
                            const categoryEnergy = tenant.energy_by_category && tenant.energy_by_category[ecId] 
                              ? tenant.energy_by_category[ecId] 
                              : 0;
                            return (
                          ...
                          <td className="text-right">
                            {tenant.total_cost !== null && tenant.total_cost !== undefined && tenant.total_cost > 0
                                ? tenant.total_cost.toFixed(2)
                                : '-'}
                          </td>
                          <td className="text-right">
                            {tenant.total_carbon !== null && tenant.total_carbon !== undefined && tenant.total_carbon > 0
                                ? tenant.total_carbon.toFixed(2)
                                : '-'}
                          </td>
                        </tr>
                      );
