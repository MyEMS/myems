import React, { Fragment, useEffect, useState, useContext, useCallback, useRef } from 'react';
import {
  Row,
  Col,
  Spinner
...
const SpaceEnergyCategory = ({ setRedirect, setRedirectUrl, t }) => {
  ...
  const defaultReportingStart = current_moment.clone().startOf('month').toDate();
  const defaultReportingEnd = current_moment.toDate();
        ...
        .then(json => {
          if (isResponseOK) {
            ...
            let url =
                ...
                moment(defaultReportingStart).format('YYYY-MM-DDTHH:mm:ss') +
                '&reportingperiodenddatetime=' +
                moment(defaultReportingEnd).format('YYYY-MM-DDTHH:mm:ss') +
                '&language=' +
                language;
            loadData(url);
          } else {
  ...
  }, [spaceUUID, language, defaultReportingStart, defaultReportingEnd]);
  ...
  const buildSmartAnalysisContext = useCallback(() => {
    ...
    return {
      ...
      periodType: defaultPeriodType,
      reportingPeriod: {
        start: moment(defaultReportingStart).format('YYYY-MM-DDTHH:mm:ss'),
        end: moment(defaultReportingEnd).format('YYYY-MM-DDTHH:mm:ss')
      },
      cardSummaryList,
      totalInTCE,
      totalInTCO2E,
