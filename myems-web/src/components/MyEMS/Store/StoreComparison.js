...
const StoreComparison = ({ setRedirect, setRedirectUrl, t }) => {
  ...
  const handleExport = (e, type) => {
    ...
    if (type === 'excel' && excelBytesBase64) {
      ...
      const fileName = 'StoreComparison.pdf';
  ...
  const buildSmartAnalysisContext = useCallback(() => {
    ...
    return {
      reportType: 'store_comparison',
      reportTitle: t('Store Comparison'),
      ...
      reportingPeriodTotals: {
        store1: reportingPeriodEnergyConsumptionInCategory1,
        store2: reportingPeriodEnergyConsumptionInCategory2,
        difference: reportingPeriodEnergyConsumptionInDifference
      },
      store1Trend: {
        labels: sliceNestedArrays(storeChartLabels1, 200),
        values: sliceChartValues(storeChartData1)
      },
      store2Trend: {
        labels: sliceNestedArrays(storeChartLabels2, 200),
        values: sliceChartValues(storeChartData2)
      },
