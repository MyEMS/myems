...
const StoreLoad = ({ setRedirect, setRedirectUrl, t }) => {
  ...
  const buildSmartAnalysisContext = useCallback(() => {
    ...
    if (storeReportingData && typeof storeReportingData === 'object') {
      Object.keys(storeReportingData).forEach(k => {
        const arr = storeReportingData[k];
        reportingTrendValues[k] = Array.isArray(arr) ? arr.slice(0, 200) : arr;
      });
    }
    const baseTrendValues = {};
    if (storeBaseData && typeof storeBaseData === 'object') {
      Object.keys(storeBaseData).forEach(k => {
        const arr = storeBaseData[k];
        baseTrendValues[k] = Array.isArray(arr) ? arr.slice(0, 200) : arr;
      });
    }
    const storeLabel = storeList.find(s => String(s.value) === String(selectedStore))?.label ?? null;
    const spaceDisplayName = Array.isArray(selectedSpaceName)
      ? selectedSpaceName.join('/')
      : selectedSpaceName ?? null;
    return {
      reportType: 'store_load',
      reportTitle: t('Load'),
      ...
      maximumLoadTrend: {
        ...
        optionLabels: storeReportingOptions
      },
