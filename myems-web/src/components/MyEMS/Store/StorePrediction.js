...
const StorePrediction = ({ setRedirect, setRedirectUrl, t }) => {
  ...
  const [workingDaysConsumptionTableColumns, setWorkingDaysConsumptionTableColumns] = useState([
    { dataField: 'name', text: t('Prediction'), sort: true }
  ]);
  ...
  const loadData = useCallback(
    url => {
      ...
      fetch(url, {
        ...
        .then(json => {
          ...
          if (isResponseOK) {
            if (uuid !== null && uuid) {
              setStoreList([{ value: json['store']['id'], label: json['store']['name'] }]);
              setSelectedStore(json['store']['id']);
            }
            ...
            if (!isBasePeriodTimestampExists(json['base_period'])) {
              ...
              const reportingTimestamps = json['reporting_period']['timestamps'] || [];
              if (reportingTimestamps.length > 0 && reportingTimestamps[0]) {
                reportingTimestamps[0].forEach((currentTimestamp, timestampIndex) => {
                  let detailed_value = {};
              ...
              json['reporting_period']['names'].forEach((currentValue, index) => {
                ...
                working_days_table_value['b0'] =
                  json['store']['working_calendars'].length > 0
    ...
    [
      ...
      setStoreBaseLabels,
      setStoreBaseData,
      setStoreBaseAndReportingNames,
      setStoreBaseAndReportingUnits,
      setStoreBaseSubtotals,
      setStoreReportingLabels,
      setStoreReportingData,
      setStoreReportingSubtotals,
      setStoreReportingRates,
      setStoreReportingOptions,
  ...
  const handleExport = (e, type) => {
    ...
    if (type === 'excel' && excelBytesBase64) {
      ...
      const fileName = 'storeprediction.xlsx';
      ...
      const fileName = 'storeprediction.pdf';
      ...
      <div>
        <Breadcrumb>
          ...
          <BreadcrumbItem active onClick={() => window.location.reload()}>
            <Link to="/store/prediction">{t('Prediction')}</Link>
          </BreadcrumbItem>
        </Breadcrumb>
      </div>
...
export default withTranslation()(withRedirect(StorePrediction));
