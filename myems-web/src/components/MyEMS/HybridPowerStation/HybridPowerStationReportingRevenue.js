...
const HybridPowerStationReportingRevenue = ({ setRedirect, setRedirectUrl, t }) => {
            ...
            .then(json => {
              if (isResponseOK) {
                ...
                if (json[0].length > 0) {
                  ...
                  setSelectedHybridPowerStation(stationID);
                  // enable submit button
                  setSubmitButtonDisabled(false);
                  // automatically submit with station
                  let url =
                    APIBaseURL +
                    '/reports/hybridpowerstationreportingrevenue?' +
                    'id=' +
                    stationID +
                    '&periodtype=' +
                    periodType +
    ...
    fetch(url, {
      ...
      .then(json => {
        if (isResponseOK) {
          ...
          setHybridPowerStationReportingNames(base_and_reporting_names);

          let base_and_reporting_units = {};
          json['reporting_period']['units'].forEach((currentValue, index) => {
            base_and_reporting_units['a' + index] = '(' + currentValue + ')';
          });
          setHybridPowerStationReportingUnits(base_and_reporting_units);

          let reporting_timestamps = {};
          json['reporting_period']['timestamps'].forEach((currentValue, index) => {
            reporting_timestamps['a' + index] = currentValue;
          });
          setHybridPowerStationReportingLabels(reporting_timestamps);

          let reporting_values = {};
          json['reporting_period']['values'].forEach((currentValue, index) => {
            reporting_values['a' + index] = currentValue;
          });
          setHybridPowerStationReportingData(reporting_values);

          let reporting_subtotals = {};
          json['reporting_period']['subtotals'].forEach((currentValue, index) => {
            reporting_subtotals['a' + index] = currentValue.toFixed(2);
          });
          setHybridPowerStationReportingSubtotals(reporting_subtotals);
          ...
          setHybridPowerStationReportingOptions(options);
  ...
  const handleExport = e => {
    ...
    const fileName = 'hybridPowerStationreproting.xlsx';
...
export default withTranslation()(withRedirect(HybridPowerStationReportingRevenue));
