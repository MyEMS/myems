...
const HybridPowerStationTable = ({ setIsSelected, hybridPowerStationList, t }) => {
  ...
  const nameFormatter = (nameuuid) => (
    <Link to={"/singlehybridpowerstation/details?uuid="+nameuuid.substring(nameuuid.length-36, nameuuid.length)} className="font-weight-semi-bold" target="_blank">
...
const selectRow = onSelect => ({
  ...
  const options = {
    custom: true,
    sizePerPage: 6,
    totalSize: hybridPowerStationList.length
  };
            ...
            <Row noGutters className="px-1 py-3">
              <Col className="pl-3 fs--1">
                ...
                <ButtonIcon
                  ...
                  onClick={() => handleViewAll(paginationProps, hybridPowerStationList.length)}
                >
                  {t('View all')}
                </ButtonIcon>
              </Col>
...
export default withTranslation()(HybridPowerStationTable);
