...
const VirtualMeterBatch = ({ setRedirect, setRedirectUrl, t }) => {
  ...
  const handleExport = (e, type) => {
    ...
    if (type === 'excel' && excelBytesBase64) {
      ...
      const fileName = 'virtualmeterbatch.pdf';
      ...
      const fileName = 'virtualmeterbatch.docx';
  ...
  const buildSmartAnalysisContext = useCallback(() => {
    ...
    return {
      reportType: 'virtual_meter_batch',
      reportTitle: t('Virtual Meter Batch Analysis'),
    ...
    <Fragment>
      <div>
        <Breadcrumb>
          ...
          <BreadcrumbItem active>{t('Virtual Meter Batch Analysis')}</BreadcrumbItem>
        </Breadcrumb>
      </div>
