...
const MeterCarbon = ({ setRedirect, setRedirectUrl, t }) => {
  ...
  const handleSubmit = e => {
    ...
    let isResponseOK = false;
    fetch(APIBaseURL + '/reports/metercarbon?' +
  ...
  return (
    <Fragment>
      <div>
        <Breadcrumb>
          <BreadcrumbItem>{t('Meter Data')}</BreadcrumbItem><BreadcrumbItem active>{t('Meter Carbon')}</BreadcrumbItem>
        </Breadcrumb>
      </div>
      ...
      <div className="card-deck">
        <CardSummary rate={reportingPeriodEnergyCarbonRate} title={t('Reporting Period Carbon Dioxide Emissions CATEGORY UNIT', { 'CATEGORY': meterEnergyCategory['name'], 'UNIT': '(' + meterEnergyCategory['unit'] + ')' })}
          color="success"  >
