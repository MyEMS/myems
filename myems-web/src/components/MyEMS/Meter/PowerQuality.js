...
const PowerQuality = ({ setRedirect, setRedirectUrl, t }) => {
    ...
    fetch(
      ...
      .then(json => {
        if (isResponseOK) {
          ...
          // show export button when at least one file was generated
  ...
  const handleExport = (e, type) => {
    ...
    if (type === 'excel' && excelBytesBase64) {
      ...
      const fileName = 'powerquality.pdf';
      ...
      const fileName = 'powerquality.docx';
  ...
  const buildSmartAnalysisContext = useCallback(() => {
    ...
    return {
      ...
      analysisResults: analysisResults.slice(0, 80)
    };
  }, [
    t,
    selectedSpaceName,
