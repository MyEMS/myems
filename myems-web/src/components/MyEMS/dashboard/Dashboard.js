...
const Dashboard = ({ setRedirect, setRedirectUrl, t }) => {
        ...
        body: null
          ...
          .then(json => {
            if (isResponseOK) {
              ...
              let rootLongitude = null;
              let rootLatitude = null;


              if (json[0]['longitude'] !== undefined && json[0]['longitude'] !== null) {
                const lng = parseFloat(json[0]['longitude']);
                if (!isNaN(lng) && lng !== 0) {
                  rootLongitude = lng;
                }
              }
              if (json[0]['latitude'] !== undefined && json[0]['latitude'] !== null) {
                const lat = parseFloat(json[0]['latitude']);
                if (!isNaN(lat) && lat !== 0) {
                  rootLatitude = lat;
                }
              }

              if (!rootLongitude || !rootLatitude || isNaN(rootLongitude) || isNaN(rootLatitude)) {

                for (const childSpace of json[0]['children']) {
                  if (childSpace['latitude'] && childSpace['longitude']) {
                    const lat = parseFloat(childSpace['latitude']);
                    const lng = parseFloat(childSpace['longitude']);
              ...
              if (!rootLongitude || !rootLatitude || isNaN(rootLongitude) || isNaN(rootLatitude)) {
                ...
                rootLongitude = 116.397;
                rootLatitude = 39.908;
              }


              setRootLongitude(rootLongitude);
              setRootLatitude(rootLatitude);
              ...
              if (json[0]['children']) {
                for (const childSpace of json[0]['children']) {
                  ...
                  if (childSpace['latitude'] !== undefined && childSpace['latitude'] !== null &&
                      childSpace['longitude'] !== undefined && childSpace['longitude'] !== null) {
                    const lat = parseFloat(childSpace['latitude']);
                    const lng = parseFloat(childSpace['longitude']);
  ...
  return (
      <Fragment>
        ...
        <div className="card-deck">
          {settings.showOnlineMap && isMapDataLoaded ? (
