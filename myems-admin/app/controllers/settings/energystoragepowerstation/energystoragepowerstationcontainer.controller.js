...
app.controller('EnergyStoragePowerStationContainerController', function (
    ...
    $scope.getContainersByEnergyStoragePowerStationID = function (id) {
        let headers = { "User-UUID": $scope.cur_user.uuid, "Token": $scope.cur_user.token };
        EnergyStoragePowerStationContainerService.getContainersByEnergyStoragePowerStationID(id, headers, function (response) {
            if (angular.isDefined(response.status) && response.status === 200) {
    ...
    $scope.changeEnergyStoragePowerStation=function(item,model){
        ...
        $scope.getContainersByEnergyStoragePowerStationID($scope.currentEnergyStoragePowerStation.id);
    };
    ...
    $scope.pairEnergyStorageContainer = function (dragEl, dropEl) {
        var energystoragecontainerid = angular.element('#' + dragEl).scope().energystoragecontainer.id;
        var energystoragepowerstationid = $scope.currentEnergyStoragePowerStation.id;
        let headers = { "User-UUID": $scope.cur_user.uuid, "Token": $scope.cur_user.token };
        EnergyStoragePowerStationContainerService.addPair(energystoragepowerstationid, energystoragecontainerid, headers, function (response) {
            if (angular.isDefined(response.status) && response.status === 201) {
                toaster.pop({
                    type: "success",
                    title: $translate.instant("TOASTER.SUCCESS_TITLE"),
                    body: $translate.instant("TOASTER.BIND_ENERGY_STORAGE_CONTAINER_SUCCESS"),
    ...
    $scope.deleteEnergyStorageContainerPair = function (dragEl, dropEl) {
        ...
        var energystoragepowerstationcontainerid = angular.element('#' + dragEl).scope().energystoragepowerstationcontainer.id;
        var energystoragepowerstationid = $scope.currentEnergyStoragePowerStation.id;
        let headers = { "User-UUID": $scope.cur_user.uuid, "Token": $scope.cur_user.token };
        EnergyStoragePowerStationContainerService.deletePair(energystoragepowerstationid, energystoragepowerstationcontainerid, headers, function (response) {
            if (angular.isDefined(response.status) && response.status === 204) {
                toaster.pop({
                    type: "success",
                    title: $translate.instant("TOASTER.SUCCESS_TITLE"),
                    body: $translate.instant("TOASTER.UNBIND_ENERGY_STORAGE_CONTAINER_SUCCESS"),
                    showCloseButton: true,
                });
                $scope.getContainersByEnergyStoragePowerStationID($scope.currentEnergyStoragePowerStation.id);
            } else {
