...
app.controller('EnergyStorageContainerCommandController', function (
    $scope,
    $window,
    $timeout,
    $translate,
    ...
    $scope.getCommandsByEnergyStorageContainerID = function (id) {
        let headers = { "User-UUID": $scope.cur_user.uuid, "Token": $scope.cur_user.token };
        EnergyStorageContainerCommandService.getCommandsByEnergyStorageContainerID(id, headers, function (response) {
            if (angular.isDefined(response.status) && response.status === 200) {
    ...
    $scope.changeEnergyStorageContainer=function(item,model){
  	  ...
  	  $scope.getCommandsByEnergyStorageContainerID($scope.currentEnergyStorageContainer.id);
    };
    ...
    $scope.pairCommand = function (dragEl, dropEl) {
		...
		let headers = { "User-UUID": $scope.cur_user.uuid, "Token": $scope.cur_user.token };
        EnergyStorageContainerCommandService.addPair(energystoragecontainerid, commandid, headers, function (response) {
    ...
    $scope.deleteCommandPair = function (dragEl, dropEl) {
        ...
        var energystoragecontainercommandid = angular.element('#' + dragEl).scope().energystoragecontainercommand.id;
        var energystoragecontainerid = $scope.currentEnergyStorageContainer.id;
		let headers = { "User-UUID": $scope.cur_user.uuid, "Token": $scope.cur_user.token };
        EnergyStorageContainerCommandService.deletePair(energystoragecontainerid, energystoragecontainercommandid, headers, function (response) {
