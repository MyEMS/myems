...
app.controller('ModalBindEnergyStorageContainerBatteryCtrl', function(
	$scope,
	$uibModalInstance,
	toaster,
	$translate,
	...
	EnergyStorageContainerBatteryService.getPointsByBMSID($scope.energystoragecontainerid, $scope.energystoragecontainerbattery.id, headers, function (response) {
		if (angular.isDefined(response.status) && response.status === 200) {
    ...
    $scope.pairPoint = function (dragEl, dropEl) {
		...
		let headers = { "User-UUID": params.user_uuid, "Token": params.token };
        EnergyStorageContainerBatteryService.addPair(params.energystoragecontainerid, params.energystoragecontainerbattery.id, pointid, headers, function (response) {
            if (angular.isDefined(response.status) && response.status === 201) {
                toaster.pop({
                    type: "success",
                    title: $translate.instant("TOASTER.SUCCESS_TITLE"),
                ...
                let headers = { "User-UUID": params.user_uuid, "Token": params.token };
				EnergyStorageContainerBatteryService.getPointsByBMSID(params.energystoragecontainerid, params.energystoragecontainerbattery.id, headers, function (response) {
					if (angular.isDefined(response.status) && response.status === 200) {
						$scope.boundpoints = response.data;
					} else {
						$scope.boundpoints = [];
    ...
    $scope.deletePointPair = function (dragEl, dropEl) {
		...
		let headers = { "User-UUID": params.user_uuid, "Token": params.token };
        EnergyStorageContainerBatteryService.deletePair(params.energystoragecontainerid, params.energystoragecontainerbattery.id, pointid, headers, function (response) {
            if (angular.isDefined(response.status) && response.status === 204) {
                toaster.pop({
                    type: "success",
                    title: $translate.instant("TOASTER.SUCCESS_TITLE"),
