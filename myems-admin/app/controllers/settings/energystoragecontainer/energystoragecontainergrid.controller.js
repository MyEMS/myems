...
app.controller('EnergyStorageContainerGridController', function(
	...
	$scope.bindEnergyStorageContainerGridPoint = function(energystoragecontainergrid) {
	var modalInstance = $uibModal.open({
		templateUrl: 'views/settings/energystoragecontainer/energystoragecontainergridpoint.model.html',
  ...
  app.controller('ModalBindEnergyStorageContainerGridCtrl', function(
	$scope,
	$uibModalInstance,
	toaster,
	$translate,
	...
	EnergyStorageContainerGridService.getPointsByGridID($scope.energystoragecontainerid, $scope.energystoragecontainergrid.id, headers, function (response) {
		if (angular.isDefined(response.status) && response.status === 200) {
    ...
    $scope.pairPoint = function (dragEl, dropEl) {
		...
		let headers = { "User-UUID": params.user_uuid, "Token": params.token };
        EnergyStorageContainerGridService.addPair(params.energystoragecontainerid, params.energystoragecontainergrid.id, pointid, headers, function (response) {
            if (angular.isDefined(response.status) && response.status === 201) {
                toaster.pop({
                    type: "success",
                    title: $translate.instant("TOASTER.SUCCESS_TITLE"),
                ...
                let headers = { "User-UUID": params.user_uuid, "Token": params.token };
				EnergyStorageContainerGridService.getPointsByGridID(params.energystoragecontainerid, params.energystoragecontainergrid.id, headers, function (response) {
					if (angular.isDefined(response.status) && response.status === 200) {
						$scope.boundpoints = response.data;
					} else {
						$scope.boundpoints = [];
    ...
    $scope.deletePointPair = function (dragEl, dropEl) {
		...
		let headers = { "User-UUID": params.user_uuid, "Token": params.token };
        EnergyStorageContainerGridService.deletePair(params.energystoragecontainerid, params.energystoragecontainergrid.id, pointid, headers, function (response) {
            if (angular.isDefined(response.status) && response.status === 204) {
                toaster.pop({
                    type: "success",
                    title: $translate.instant("TOASTER.SUCCESS_TITLE"),
