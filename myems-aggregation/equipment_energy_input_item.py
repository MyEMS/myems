"""
MyEMS Equipment Energy Input Item Aggregation Service

This module handles the aggregation of energy input consumption data for equipment by energy items.
It processes energy consumption data from various sources (meters, virtual meters, offline meters)
associated with each equipment and aggregates them into hourly energy consumption by items.
...
def main(logger):
    """
    Main function for equipment energy input item aggregation service.

    This function runs continuously and processes energy aggregation for all equipment
    by energy items. It uses multiprocessing to handle multiple equipment in parallel
...
def worker(equipment):
    """
    Worker function to process energy aggregation for a single equipment by energy items.
    ...
    while len(aggregated_values) > 0:
        ...
        try:
            query = (" INSERT INTO tbl_equipment_input_item_hourly "
                         "             (equipment_id , energy_item_id , start_datetime_utc , actual_value) "
                         " VALUES  (%s, %s, %s, %s)")
