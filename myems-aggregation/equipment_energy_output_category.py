"""
MyEMS Equipment Energy Output Category Aggregation Service

This module handles the aggregation of energy output consumption data for equipment by energy categories.
...
3. For each equipment, retrieves associated output sources (meters, virtual meters, offline meters)
...
def main(logger):
    """
    Main function for equipment energy output category aggregation service.

    This function runs continuously and processes energy aggregation for all equipment
    based on energy output. It uses multiprocessing to handle multiple equipment in parallel
...
def worker(equipment):
    """
    Worker function to process energy aggregation for a single equipment by energy output categories.
