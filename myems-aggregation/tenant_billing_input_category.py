"""
MyEMS Tenant Billing Input Category Aggregation Service

This module handles the calculation of billing costs for tenants based on energy input consumption
and tariff structures. It processes energy consumption data from various sources associated with
each tenant and calculates the corresponding billing amounts using time-of-use tariffs.

The service follows a systematic approach:
1. Retrieves all tenants from the system database
2. For each tenant, determines the latest processed billing data timestamp
...
ensuring accurate billing calculations for all tenants in the system.
"""
...
def main(logger):
    """
    Main function for tenant billing input category aggregation service.

    This function runs continuously and processes billing calculations for all tenants.
    ...
    3. For each tenant, processes billing calculations
    ...
    while True:
        ...
        # Process each tenant for billing calculations
        for tenant in tenant_list:
            ...
            try:
                # Find the latest processed billing timestamp for this tenant
                ...
                # Break the tenant processing loop
                break
            ...
            # Query energy consumption data for this tenant since the last processed timestamp
            ...
            if len(billing_dict) > 0:
                try:
                    ...
                    query = (" INSERT INTO tbl_tenant_input_category_hourly "
                                 "             (tenant_id , energy_category_id , start_datetime_utc , actual_value) "
                                 " VALUES  (%s, %s, %s, %s)")
        ...
        # End of tenant processing loop - close all database connections
        if cursor_system_db:
            cursor_system_db.close()
        if cnx_system_db:
            cnx_system_db.close()
