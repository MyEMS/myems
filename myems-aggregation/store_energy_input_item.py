"""
MyEMS Store Energy Input Item Aggregation Service

This module handles the aggregation of energy input consumption data for stores by energy items.
It processes energy consumption data from various sources (meters, virtual meters, offline meters, equipment, spaces)
associated with each store and aggregates them into hourly energy consumption by items.
...
def main(logger):
    """
    Main function for store energy input item aggregation service.

    This function runs continuously and processes energy aggregation for all stores
    by energy items. It uses multiprocessing to handle multiple stores in parallel
...
def worker(store):
    """
    Worker function to process energy aggregation for a single store by energy items.
    ...
    while len(aggregated_values) > 0:
        ...
        try:
            query = (" INSERT INTO tbl_store_input_item_hourly "
                         "             (store_id , energy_item_id , start_datetime_utc , actual_value) "
                         " VALUES  (%s, %s, %s, %s)")
