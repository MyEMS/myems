"""
MyEMS Combined Equipment Energy Input Category Aggregation Service

This module handles the aggregation of energy input consumption data for combined equipment by energy categories.
...
def main(logger):
    """
    Main function for combined equipment energy input category aggregation service.

    This function runs continuously and processes energy aggregation for all combined equipment.
    It uses multiprocessing to handle multiple combined equipment in parallel for better performance.
...
def worker(combined_equipment):
    """
    Worker function to process energy aggregation for a single combined equipment.
