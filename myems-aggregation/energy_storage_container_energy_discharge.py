...
########################################################################################################################
# PROCEDURES
# Step 1: get all energy storage containers
...
def main(logger):
    ...
    while True:
        ...
        try:
            ...
            logger.error("Error in step 1.1 of energy_storage_container_energy_discharge.main " + str(e))
            if cursor_system_db:
        ...
        try:
            ...
            if rows_energy_storage_containers is None or len(rows_energy_storage_containers) == 0:
                print("There isn't any energy storage containers ")
                # sleep and continue the outer loop to reconnect the database
                time.sleep(60)
                continue
            ...
            logger.error("Error in step 1.2 of energy_storage_container_energy_discharge.main " + str(e))
            # sleep and continue the outer loop to reconnect the database
            time.sleep(60)
            continue
        finally:
        ...
        print("Got all energy storage containers in MyEMS System Database")

        # shuffle the energy storage container list for randomly calculating the hourly values
        random.shuffle(energy_storage_container_list)

        ################################################################################################################
        # Step 2: Create multiprocessing pool to call worker in parallel
        ################################################################################################################
        p = Pool(processes=config.pool_size)
        error_list = p.map(worker, energy_storage_container_list)
        p.close()
        p.join()
...
#   Step 3: for each discharge meter in list, get energy data from energy database
...
def worker(energy_storage_container):
    ...
    print("Step 1: get all discharge meters associated with the energy storage container " +
          str(energy_storage_container['name']))

    discharge_meter_list = list()
    cnx_system_db = None
    cursor_system_db = None
    try:
        cnx_system_db = mysql.connector.connect(**config.myems_system_db)
        ...
        error_string = "Error in step 1.1 of energy_storage_container_energy_discharge.worker " + str(e)
        if cursor_system_db:
            cursor_system_db.close()
        if cnx_system_db:
            cnx_system_db.close()
    ...
    try:
        cursor_system_db.execute(" SELECT m.id, m.name, m.energy_category_id "
                                 " FROM tbl_meters m, tbl_energy_storage_containers_batteries mb "
                                 " WHERE m.id = mb.discharge_meter_id "
                                 "       AND m.is_counted = 1 "
                                 "       AND mb.energy_storage_container_id = %s ",
                                 (energy_storage_container['id'],))
        rows_discharge_meters = cursor_system_db.fetchall()
    ...
    ####################################################################################################################
    # stop to the next energy storage container if this energy storage container is empty
    ####################################################################################################################
    if discharge_meter_list is None or len(discharge_meter_list) == 0:
        print("This is an empty energy storage container ")
        return None
    ...
    try:
        ...
        error_string = "Error in step 2.1 of energy_storage_container_energy_discharge.worker " + str(e)
        if cursor_energy_db:
    ...
    try:
        ...
        cursor_energy_db.execute(query, (energy_storage_container['id'],))
        ...
        error_string = "Error in step 2.2 of energy_storage_container_energy_discharge.worker " + str(e)
        if cursor_energy_db:
            cursor_energy_db.close()
        if cnx_energy_db:
            cnx_energy_db.close()
    ...
    try:
        if discharge_meter_list is not None and len(discharge_meter_list) > 0:
            for meter in discharge_meter_list:
        ...
        error_string = "Error in step 3.1 of energy_storage_container_energy_discharge.worker " + str(e)
        if cursor_energy_db:
            cursor_energy_db.close()
        if cnx_energy_db:
            cnx_energy_db.close()
    ...
    if energy_meter_hourly is None or len(energy_meter_hourly) == 0:
        ...
        # continue the for tenant loop to the next energy storage container
        print("continue the for energy storage container loop to the next energy storage container")
        if cursor_energy_db:
            cursor_energy_db.close()
        if cnx_energy_db:
            cnx_energy_db.close()
    ...
    try:
        ...
        error_string = "Error in step 5 of energy_storage_container_energy_discharge.worker " + str(e)
        if cursor_energy_db:
            cursor_energy_db.close()
        if cnx_energy_db:
            cnx_energy_db.close()
    ...
    while len(aggregated_values) > 0:
        ...
        try:
            add_values = (" INSERT INTO tbl_energy_storage_container_discharge_hourly "
            ...
            error_string = "Error in step 6.1 of energy_storage_container_energy_discharge.worker " + str(e)
            print(error_string)
            if cursor_energy_db:
                cursor_energy_db.close()
            if cnx_energy_db:
