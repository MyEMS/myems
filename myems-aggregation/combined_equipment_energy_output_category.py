"""
MyEMS Combined Equipment Energy Output Category Aggregation Service

This module handles the aggregation of energy output consumption data for combined equipment by energy categories.
It processes energy consumption data from various sources (meters, virtual meters, offline meters, equipment)
associated with each combined equipment and aggregates them into hourly energy consumption by categories.
...
3. For each combined equipment, retrieves associated output sources (meters, virtual meters, offline meters, equipment)
4. Determines the time range for data aggregation
5. Fetches energy consumption data from all output sources
...
def main(logger):
    """
    Main function for combined equipment energy output category aggregation service.

    This function runs continuously and processes energy aggregation for all combined equipment
    based on energy output. It uses multiprocessing to handle multiple combined equipment in parallel
...
def worker(combined_equipment):
    """
    Worker function to process energy aggregation for a single combined equipment by energy output categories.

    This function handles the complete energy aggregation process for one combined equipment,
    including retrieving associated output sources, fetching energy data, and saving
