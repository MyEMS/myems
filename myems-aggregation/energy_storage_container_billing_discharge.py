...
# for each energy storage container in list:
...
def main(logger):
    ...
    while True:
        ...
        try:
            ...
            logger.error("Error in step 1.1 of energy_storage_container_billing_discharge " + str(e))
            if cursor_system_db:
        ...
        try:
            ...
            if rows_energy_storage_containers is None or len(rows_energy_storage_containers) == 0:
                print("Step 1.2: There isn't any energy storage containers. ")
                if cursor_system_db:
                    cursor_system_db.close()
                if cnx_system_db:
                    cnx_system_db.close()
            ...
            for row in rows_energy_storage_containers:
                energy_storage_container_list.append({"id": row[0], "name": row[1], "cost_center_id": row[2]})

        except Exception as e:
            logger.error("Error in step 1.2 of energy_storage_container_billing_discharge " + str(e))
            if cursor_system_db:
                cursor_system_db.close()
            if cnx_system_db:
                cnx_system_db.close()
        ...
        print("Step 1.2: Got all energy storage containers from MyEMS System Database")
        ...
        try:
            ...
            logger.error("Error in step 1.3 of energy_storage_container_billing_discharge " + str(e))
            if cursor_energy_db:
        ...
        try:
            ...
            logger.error("Error in step 1.4 of energy_storage_container_billing_discharge " + str(e))
        ...
        for energy_storage_container in energy_storage_container_list:
            ...
            try:
                ...
                logger.error("Error in step 2 of energy_storage_container_billing_discharge " + str(e))
                # break the for energy storage container loop
                break
            ...
            query = (" SELECT start_datetime_utc, actual_value "
                     " FROM tbl_energy_storage_container_discharge_hourly "
                     " WHERE energy_storage_container_id = %s AND start_datetime_utc >= %s "
                     " ORDER BY id ")
            cursor_energy_db.execute(query, (energy_storage_container['id'], start_datetime_utc, ))
            ...
            if rows_hourly is None or len(rows_hourly) == 0:
                ...
                # continue the for energy storage container loop
                continue
            ...
            tariff_dict = dict()
            tariff_dict[1] = tariff.get_energy_category_tariffs(energy_storage_container['cost_center_id'],
                                                                1,
                                                                start_datetime_utc,
                                                                end_datetime_utc)
            ...
            if len(billing_dict) > 0:
                try:
                    ...
                    logger.error("Error in step 6 of energy_storage_container_billing_discharge " + str(e))
                    # break the for energy storage container loop
                    break

        # end of for energy storage container loop
        if cursor_system_db:
            cursor_system_db.close()
        if cnx_system_db:
            cnx_system_db.close()
