"""
MyEMS Cleaning Service - Digital Values Cleaning Module

This module handles the cleaning of digital values (binary sensor readings like on/off states,
open/closed positions, alarm statuses, etc.) from the historical database. It removes old
digital value records based on the configured retention policy to maintain database performance
and storage efficiency.

The cleaning process runs on a schedule (every 8 hours by default) and deletes digital values
...
def job(logger):
    """
    Execute the digital values cleaning job.

    This function connects to the historical database and deletes digital values
...
def process(logger):
    """
    Main process function for digital values cleaning.

    This function manages the scheduling and execution of digital values cleaning.
